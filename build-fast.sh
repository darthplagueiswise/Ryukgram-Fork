...
copy_bundle_assets() {
    ...
    find "$SRC" -maxdepth 1 -type f \( -iname '*.png' -o -iname '*.jpg' -o -iname '*.pdf' \) \
        -exec cp {} "$DEST/" \;
    if [ -f "$SRC/secret_src/ryg_mc_names.bin" ]; then
        python3 scripts/gen_mc_names.py "$SRC/secret_src/ryg_mc_names.bin" \
            "$DEST/ryg_mc_names.bin" >/dev/null || true
    fi
}
...
build_full_bundle() {
    ...
    if [ -d "modules/ffmpegkit/ffmpegkit.framework" ]; then
        ...
        for lib in $LIBS; do
            mv "$BUNDLE_PATH/${lib}.framework" "$BUNDLE_PATH/${lib}_ryg.framework"
            install_name_tool -id "@rpath/${lib}_ryg.framework/${lib}" \
                "$BUNDLE_PATH/${lib}_ryg.framework/${lib}"
        done
        for target in "$BUNDLE_PATH/ffmpegkit.framework/ffmpegkit" \
                      "$BUNDLE_PATH"/libav*_ryg.framework/libav* \
                      "$BUNDLE_PATH"/libsw*_ryg.framework/libsw*; do
            [ -f "$target" ] || continue
            for lib in $LIBS; do
                install_name_tool -change \
                    "@rpath/${lib}.framework/${lib}" \
                    "@rpath/${lib}_ryg.framework/${lib}" \
