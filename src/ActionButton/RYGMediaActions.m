...
static NSString *rygCurrentFilenameStem = nil;
...
static void rygSetPendingMetadata(RYGGallerySaveMetadata *m) {
    dispatch_sync(rygPendingMetadataQueue(), ^{ rygPendingGalleryMetadata = m; });
}
...
static NSString *rygSanitizeFilenameComponent(NSString *s) {
	if (!s.length) return @"";
...
static void rygSaveVideoToPhotosURLForJob(NSURL *url, RYGDownloadJob *job) {
	...
	[PHPhotoLibrary requestAuthorization:^(PHAuthorizationStatus status) {
		...
		if (useAlbum) {
			[RYGPhotoAlbum saveFileToAlbum:url completion:^(BOOL ok, NSError *error) {
...
+ (NSString *)contextLabelForContext:(RYGActionContext)ctx {
	switch (ctx) {
		case RYGActionContextFeed: return @"feed";
		case RYGActionContextReels: return @"reels";
		case RYGActionContextStories: return @"stories";
	}
	return @"media";
}

+ (NSString *)filenameStemForMedia:(id)media contextLabel:(NSString *)ctxLabel {
	return [self filenameStemForUsername:rygUsernameForMedia(media) contextLabel:ctxLabel];
}

+ (NSString *)filenameStemForUsername:(NSString *)username contextLabel:(NSString *)ctxLabel {
	NSString *user = rygSanitizeFilenameComponent(username);
	NSString *ctx = rygSanitizeFilenameComponent(ctxLabel);
...
+ (NSString *)currentFilenameStem {
	return rygCurrentFilenameStem;
}

+ (void)setCurrentFilenameStem:(NSString *)stem {
	rygCurrentFilenameStem = stem.copy;
}
...
+ (void)downloadAudioRepresentation:(RYGDashRepresentation *)audio action:(DownloadAction)action {
	...
	RYGDownloadCenter *center = [RYGDownloadCenter shared];
	NSString *stem = self.currentFilenameStem.length ? self.currentFilenameStem : NSUUID.UUID.UUIDString;
	NSString *outPath = [RYGTempFiles claimNamedFile:[stem stringByAppendingPathExtension:@"m4a"] ttl:900 tag:@"audio"].path;
...
+ (void)downloadHDMedia:(id)media action:(DownloadAction)action fromView:(UIView *)sourceView {
	...
	BOOL isVideo = [RYGUtils getVideoUrlForMedia:(IGMedia *)media] != nil;
	if (!isVideo) {
				...
				case saveToPhotos: {
					...
					if ([galleryMode isEqualToString:@"gallery_only"]) {
						...
						if ([galleryMode isEqualToString:@"mirror"]) {
							NSError *e = nil;
							rygSaveFileToGalleryURL(outputURL, RYGGalleryMediaTypeVideo, &e);
						}
	...
	void (^start)(void) = ^{
		[PHPhotoLibrary requestAuthorization:^(PHAuthorizationStatus status) {
			...
			saveNext = ^{
				...
				if (useAlbum) {
					[RYGPhotoAlbum saveFileToAlbum:file completion:step];
					return;
				}
...
+ (NSArray<RYGAction *> *)actionsForContext:(RYGActionContext)ctx media:(id)media fromView:(UIView *)sourceView includeDisabled:(BOOL)includeDisabled {
	...
	void (^stamp)(id) = ^(id targetMedia) {
		[RYGMediaActions setCurrentFilenameStem:[RYGMediaActions filenameStemForMedia:targetMedia contextLabel:ctxLabel]];
	...
	RYGAction *(^resolve)(NSString *) = ^RYGAction *(NSString *aid) {
		...
		if ([aid isEqualToString:RYGAID_DownloadWithMusic]) {
			if ([RYGUtils getVideoUrlForMedia:(IGMedia *)media]) return nil;
		...
		if ([aid isEqualToString:RYGAID_BulkDownloadShare]) {
			...
			return [RYGAction actionWithTitle:RYGLocalized(@"Download and share all") icon:@"square.and.arrow.up.on.square" handler:^{
				if (isCarousel) {
					[RYGMediaActions setCurrentFilenameStem:[RYGMediaActions filenameStemForMedia:parentMedia contextLabel:ctxLabel]];
					[RYGMediaActions downloadAllAndShareMedia:parentMedia];
					return;
				}

				[RYGMediaActions setCurrentFilenameStem:[RYGMediaActions filenameStemForMedia:storyMedias.firstObject contextLabel:ctxLabel]];
		...
		if ([aid isEqualToString:RYGAID_BulkDownloadGallery]) {
			...
			return [RYGAction actionWithTitle:RYGLocalized(@"Download all to Gallery") icon:@"ig_icon_photo_gallery_prism_outline_24" handler:^{
				...
				[RYGMediaActions setCurrentFilenameStem:[RYGMediaActions filenameStemForMedia:medias.firstObject contextLabel:ctxLabel]];
