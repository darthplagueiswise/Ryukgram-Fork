...
static NSString *RYGCleanExportName(NSURL *source, RYGGalleryMediaType mediaType, RYGGallerySaveMetadata *metadata, NSString *ext) {
	NSString *name = RYGFileNameForMedia(source, mediaType, metadata);
...
- (void)saveCurrentToPhotos {
	...
	if (source.isFileURL) {
		...
		RYGGalleryMediaType mt = item.videoURL ? RYGGalleryMediaTypeVideo : RYGGalleryMediaTypeImage;
		NSURL *copy = [RYGTempFiles claimNamedFile:RYGCleanExportName(source, mt, metadata, ext) ttl:300 tag:@"save"];
...
- (void)sharePhotoItem:(RYGMediaViewerItem *)item currentPage:(UIViewController *)page {
	...
	[RYGImageCache loadDataFromURL:item.photoURL completion:^(NSData *data) {
		...
		if (data.length) {
			...
			tempURL = [RYGTempFiles claimNamedFile:RYGCleanExportName(item.photoURL, RYGGalleryMediaTypeImage, item.metadata, ext) ttl:600 tag:@"share"];
