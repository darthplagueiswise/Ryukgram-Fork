...
- (NSArray<UIMenuElement *> *)shareMenuChildren {
	...
	if (!self.currentItem.audioURL) {
		[children addObject:[UIAction actionWithTitle:SCILocalized(@"Save to Photos")
	...
	if ([SCIUtils getBoolPref:@"sci_gallery_enabled"]) {
		[children addObject:[UIAction actionWithTitle:SCILocalized(@"Save to Gallery")
	...
	[children addObject:[UIAction actionWithTitle:SCILocalized(@"Share")
...
- (void)saveCurrentToPhotos {
	...
	if (!source) {
		SCINotifyError(SCI_NOTIF_DOWNLOAD, SCILocalized(@"Save failed"), SCILocalized(@"Nothing to save"));
		return;
	}
	...
	SCIDownloadDelegate *dl = [[SCIDownloadDelegate alloc] initWithAction:saveToPhotos showProgress:YES];
	dl.pendingGallerySaveMetadata = metadata;
...
- (void)saveCurrentToGallery {
	...
	[SCIImageCache loadDataFromURL:source completion:^(NSData *data) {
		...
		NSURL *tempURL = [SCITempFiles claimWithExt:useExt ttl:300 tag:@"save"];
...
- (void)presentActivityWithItems:(NSArray *)items cleanupURL:(NSURL *)cleanupURL {
	...
	if (cleanupURL) {
		controller.completionWithItemsHandler = ^(__unused UIActivityType type, __unused BOOL completed, __unused NSArray *returnedItems, __unused NSError *error) {
			[SCITempFiles releaseURL:cleanupURL];
		};
	}
