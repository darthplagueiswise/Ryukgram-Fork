...
- (void)loadRows:(BOOL)force {
    ...
    dispatch_async(dispatch_get_global_queue(QOS_CLASS_USER_INITIATED, 0), ^{
        ...
        if (force) [engine reloadFromRuntime]; else [engine prepare];
        NSArray<NSString *> *alternatives = RYGTypedAlternatives(domainQuery);
        NSMutableArray<NSDictionary *> *rows = [NSMutableArray array];
        for (RYGMCConfig *config in [engine allConfigsIncludingMappingOnly] ?: @[]) {
            ...
            for (RYGMCParam *param in config.params ?: @[]) {
                NSString *blob = RYGTypedNormalize([NSString stringWithFormat:@"%@ %@ %u %u %llu %@ %@",
                    configName ?: @"", param.name ?: @"", config.number, param.paramIndex, param.paramID,
                    param.typeName ?: @"", param.isRuntimeBacked ? @"runtime linked" : @"mapping only"]);
        ...
        [rows sortUsingComparator:^NSComparisonResult(NSDictionary *left, NSDictionary *right) {
            RYGMCConfig *lc = left[@"config"], *rc = right[@"config"];
            NSComparisonResult result = [lc.displayName localizedCaseInsensitiveCompare:rc.displayName];
            if (result != NSOrderedSame) return result;
            RYGMCParam *lp = left[@"param"], *rp = right[@"param"];
            NSString *ln = lp.name ?: @"", *rn = rp.name ?: @"";
            result = [ln localizedCaseInsensitiveCompare:rn];
            if (result != NSOrderedSame) return result;
            return lp.paramIndex < rp.paramIndex ? NSOrderedAscending : (lp.paramIndex > rp.paramIndex ? NSOrderedDescending : NSOrderedSame);
        }];
        dispatch_async(dispatch_get_main_queue(), ^{
            __strong typeof(weakSelf) self = weakSelf;
            if (!self || generation != self.loadGeneration) return;
            self.loading = NO; self.allRows = rows.copy;
            [self.tableView.refreshControl endRefreshing]; [self applySearch];
        });
    });
}

- (void)updateSearchResultsForSearchController:(UISearchController *)searchController { (void)searchController; [self applySearch]; }
...
- (void)applySearch {
    ...
    if (self.visibleRows.count) self.tableView.backgroundView = nil;
        ...
        label.text = @"No MobileConfig candidate matches this domain.";
        label.textAlignment = NSTextAlignmentCenter; label.textColor = UIColor.secondaryLabelColor; label.numberOfLines = 0;
        self.tableView.backgroundView = label;
    }
    [self.tableView reloadData];
}

- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section { (void)tableView; (void)section; return self.visibleRows.count; }
- (NSString *)tableView:(UITableView *)tableView titleForHeaderInSection:(NSInteger)section {
    (void)tableView; (void)section;
    return [NSString stringWithFormat:@"%lu candidate%@", (unsigned long)self.visibleRows.count, self.visibleRows.count == 1 ? @"" : @"s"];
}
- (NSString *)tableView:(UITableView *)tableView titleForFooterInSection:(NSInteger)section {
    (void)tableView; (void)section;
    return @"Candidates come from the union of id_name_mapping.json and the current iOS runtime table. Only runtime-linked rows are editable. Mapping-only candidates remain visible for discovery without inventing a PID or type.";
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    BOOL overridden = linked && [engine overrideStateFor:param] == RYGMCOverrideSet;
    id effective = linked ? (overridden ? [engine overrideValueFor:param] : [engine liveValueFor:param]) : nil;

    cell.textLabel.text = param.name.length ? param.name : [NSString stringWithFormat:@"Parameter %u", param.paramIndex];
    cell.textLabel.numberOfLines = 2; cell.textLabel.font = [UIFont systemFontOfSize:14.5 weight:UIFontWeightSemibold];
    cell.detailTextLabel.numberOfLines = 3; cell.detailTextLabel.textColor = UIColor.secondaryLabelColor;
    if (linked) {
        cell.detailTextLabel.text = [NSString stringWithFormat:@"%@ · #%u:%u · %@\n%@ %@",
            config.displayName, config.number, param.paramIndex, param.typeName ?: @"typed",
            overridden ? @"Override" : @"Native", RYGTypedValueString(effective)];
    } else {
        cell.detailTextLabel.text = [NSString stringWithFormat:@"%@ · #%u:%u\nMapping only · not linked to this iOS runtime",
            config.displayName, config.number, param.paramIndex];
    }
    cell.accessoryView = nil; cell.accessoryType = UITableViewCellAccessoryNone;
    cell.selectionStyle = linked && param.type != RYGMCTypeBool ? UITableViewCellSelectionStyleDefault : UITableViewCellSelectionStyleNone;

    if (linked && param.type == RYGMCTypeBool) {
    ...
    } else if (linked) cell.accessoryType = UITableViewCellAccessoryDisclosureIndicator;
    return cell;
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    RYGMCParam *param = self.visibleRows[(NSUInteger)indexPath.row][@"param"];
    ...
    [alert addTextFieldWithConfigurationHandler:^(UITextField *field) {
        field.text = RYGTypedValueString(current); field.clearButtonMode = UITextFieldViewModeWhileEditing;
    ...
    [alert addAction:[UIAlertAction actionWithTitle:@"Apply" style:UIAlertActionStyleDefault handler:^(__unused UIAlertAction *action) {
        BOOL valid = NO; id value = [weakSelf parsedValue:alert.textFields.firstObject.text ?: @"" forParam:param valid:&valid];
        if (!valid || !value || ![engine setOverride:value for:param]) [RYGUtils showErrorHUDWithDescription:@"Value does not match the resolved runtime type"];
...
- (UISwipeActionsConfiguration *)tableView:(UITableView *)tableView trailingSwipeActionsConfigurationForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    UIContextualAction *native = [UIContextualAction contextualActionWithStyle:UIContextualActionStyleDestructive title:@"Native" handler:^(__unused UIContextualAction *action, __unused UIView *sourceView, void (^completion)(BOOL)) {
        [RYGMobileConfig.shared clearOverrideFor:param]; [weakSelf.tableView reloadData]; completion(YES);
    }];
