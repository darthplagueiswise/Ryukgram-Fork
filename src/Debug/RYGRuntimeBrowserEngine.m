...
static BOOL RYGImagePathMatches(NSString *left, NSString *right) {
    ...
    const struct mach_header *leftHeader = RYGDyldHeaderForImagePath(left);
    const struct mach_header *rightHeader = RYGDyldHeaderForImagePath(right);
    if (leftHeader && rightHeader) return leftHeader == rightHeader;
...
static BOOL RYGIsIntegerRegisterType(const char *type) {
    type = RYGSkipTypeQualifiers(type);
    return type && *type && strchr("BcCsSiIlLqQ", *type) != NULL;
}
...
static BOOL RYGClassDefinesOrContributesToImage(Class cls, NSString *imagePath) {
    ...
    for (NSUInteger pass = 0; pass < 2; pass++) {
        ...
        for (unsigned int index = 0; methods && index < count; index++) {
            if (RYGMethodIMPBelongsToImage(methods[index], imagePath)) {
                hit = YES;
                break;
            }
        }
...
static void RYGCountMembersForClass(Class cls,
    ...
    for (NSUInteger pass = 0; pass < 2; pass++) {
        ...
        Method *methods = owner ? class_copyMethodList(owner, &count) : NULL;
        for (unsigned int index = 0; methods && index < count; index++) {
            if (classDefinedHere || RYGMethodIMPBelongsToImage(methods[index], imagePath)) methodCounts[pass]++;
        }
        if (methods) free(methods);

        if (classDefinedHere) {
            unsigned int countProperties = 0;
            objc_property_t *list = owner ? class_copyPropertyList(owner, &countProperties) : NULL;
            properties += countProperties;
            if (list) free(list);
        }
    }

    if (instanceCount) *instanceCount = methodCounts[0];
    if (classCount) *classCount = methodCounts[1];
    if (propertyCount) *propertyCount = properties;
}

static NSNumber *RYGOverrideForKey(NSString *key) {
    @synchronized(RYGRuntimeBrowserEngine.class) {
        return gRYGOverrides[key];
    }
}
...
static void RYGForgetInstalledKey(NSString *key) {
    @synchronized(RYGRuntimeBrowserEngine.class) {
        [gRYGInstalledKeys removeObject:key];
    }
}
...
static RYGRuntimeBoolMethod *RYGBoolRow(Class cls,
                                        Method method,
                                        BOOL classMethod,
                                        NSString *imagePath) {
...
+ (NSArray<NSString *> *)runtimeImagePaths {
    ...
    for (uint32_t index = 0; index < _dyld_image_count(); index++) {
        ...
        BOOL bundledDylib = [path hasPrefix:[bundle stringByAppendingString:@"/"]] &&
...
+ (NSArray<RYGRuntimeClassRow *> *)classesForImagePath:(NSString *)imagePath {
    ...
    // Categories and sideload path rewrites can make objc_copyClassNamesForImage
    // incomplete. The fallback keeps Class values in a raw C buffer and maps
    // them to the selected dyld image through class_getImageName / dladdr.
    ...
    for (NSString *name in ordered) {
        ...
        row.imagePath = imagePath;
        row.className = name;
        RYGCountMembersForClass(cls,
                                imagePath,
                                &row->_instanceMethodCount,
                                &row->_classMethodCount,
                                &row->_propertyCount);
        [rows addObject:row];
    }
    return rows.copy;
...
+ (NSArray<RYGRuntimeMemberRow *> *)membersForClassName:(NSString *)className
    ...
    for (NSUInteger pass = 0; pass < 2; pass++) {
        ...
        for (unsigned int index = 0; methods && index < methodCount; index++) {
            ...
            const char *types = method_getTypeEncoding(method);
            row.typeEncoding = types ? [NSString stringWithUTF8String:types] : @"";
            row.hookableBool = RYGMethodHasSupportedBoolABI(method);
            row.argumentKind = row.hookableBool ? RYGArgumentKindForMethod(method) : RYGRuntimeArgumentNone;
            [rows addObject:row];
        }
        if (methods) free(methods);
...
+ (NSInteger)dyldIndexForImagePath:(NSString *)imagePath {
    ...
    for (uint32_t index = 0; index < _dyld_image_count(); index++) {
        if (_dyld_get_image_header(index) == target) return (NSInteger)index;
    }
    return NSNotFound;
}
...
+ (NSNumber *)observedNativeValueForKey:(NSString *)overrideKey {
    return RYGObservedForKey(overrideKey);
}

+ (NSNumber *)overrideForKey:(NSString *)overrideKey {
    return RYGOverrideForKey(overrideKey);
}
