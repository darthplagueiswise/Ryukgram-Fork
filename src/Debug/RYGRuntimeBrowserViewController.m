...
#pragma mark - Class detail: hookable BOOL methods only
...
- (void)viewDidLoad {
    ...
    self.searchController.searchBar.placeholder = @"Hookable BOOL method or ABI";
...
- (void)applyFilter {
    ...
    NSPredicate *predicate = [NSPredicate predicateWithBlock:^BOOL(RYGRuntimeMethodRow *row, NSDictionary *bindings) {
        (void)bindings;
        return RYGRuntimeTextMatches([NSString stringWithFormat:@"%@ %@ %@", row.className ?: @"", row.selectorName ?: @"", row.typeEncoding ?: @""], tokens);
    }];
    self.visibleInstanceMethods = tokens.count ? [self.instanceMethods filteredArrayUsingPredicate:predicate] : self.instanceMethods;
    self.visibleClassMethods = tokens.count ? [self.classMethods filteredArrayUsingPredicate:predicate] : self.classMethods;
    [self.tableView reloadData];
}

- (NSArray<RYGRuntimeMethodRow *> *)rowsForSection:(NSInteger)section {
    return section == 0 ? (self.visibleInstanceMethods ?: @[]) : (self.visibleClassMethods ?: @[]);
}
...
- (NSString *)tableView:(UITableView *)tableView titleForHeaderInSection:(NSInteger)section { (void)tableView; return section == 0 ? @"Instance BOOL Methods" : @"Class BOOL Methods"; }
...
- (UIButton *)outputButtonForMethod:(RYGRuntimeBoolMethod *)method {
    ...
    UIMenu *overrideMenu = [UIMenu menuWithTitle:@"Override" image:nil identifier:nil options:UIMenuOptionsSingleSelection children:@[nativeAction, on, off]];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (method) cell.accessoryView = [self outputButtonForMethod:method];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    else [RYGUtils showErrorHUDWithDescription:@"Runtime ABI changed; hook was not installed"];
}
...
- (void)viewDidLoad {
    ...
    self.tableView.rowHeight = UITableViewAutomaticDimension; self.tableView.estimatedRowHeight = 52.0;
    ...
    self.searchController.searchResultsUpdater = self;
    self.searchController.obscuresBackgroundDuringPresentation = NO;
    self.searchController.searchBar.placeholder = @"Class or hookable BOOL method";
    self.navigationItem.searchController = self.searchController;
    self.navigationItem.hidesSearchBarWhenScrolling = NO;
...
- (void)rebuildImageMenu {
    ...
    for (NSString *path in self.images) {
        ...
        UIAction *action = [UIAction actionWithTitle:[RYGRuntimeBrowserEngine shortNameForImagePath:path] image:[UIImage systemImageNamed:mainImage ? @"app" : @"shippingbox"] identifier:nil handler:^(__unused UIAction *item) {
    ...
    if (configuration) {
        configuration.title = [NSString stringWithFormat:@"Image: %@", self.selectedImagePath.length ? [RYGRuntimeBrowserEngine shortNameForImagePath:self.selectedImagePath] : @"None"];
        configuration.baseForegroundColor = UIColor.labelColor;
        if (@available(iOS 26.0, *)) [configuration setDefaultContentInsets];
...
- (void)modeChanged:(UISegmentedControl *)sender {
    ...
    self.searchController.searchBar.placeholder = self.modeControl.selectedSegmentIndex == RYGRuntimeTopModeClasses ? @"Class or hookable BOOL method" : @"Mach-O symbol";
    [self scanSelectedImage];
}
...
- (void)scanSelectedImage {
    ...
    if (!path.length) {
        [self.spinner stopAnimating]; self.emptyLabel.text = @"No loaded executable/framework image is available."; self.tableView.backgroundView = self.emptyLabel; return;
    }
    ...
    dispatch_async(dispatch_get_global_queue(QOS_CLASS_USER_INITIATED, 0), ^{
        if (mode == RYGRuntimeTopModeClasses) {
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                ...
                self.classRows = rows; [self.spinner stopAnimating]; [self applySearchFilter];
            });
        } else {
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                __strong typeof(weakSelf) self = weakSelf;
                if (!self || generation != self.scanGeneration || ![self.selectedImagePath isEqualToString:path]) return;
                self.symbolRows = rows; [self.spinner stopAnimating]; [self applySearchFilter];
            });
        }
    });
}
...
- (void)applySearchFilter {
    ...
    if (self.modeControl.selectedSegmentIndex == RYGRuntimeTopModeMachO) {
        ...
        self.visibleSymbolRows = tokens.count ? [self.symbolRows filteredArrayUsingPredicate:[NSPredicate predicateWithBlock:^BOOL(RYGMachOSymbol *row, NSDictionary *bindings) {
            (void)bindings; return RYGRuntimeTextMatches([NSString stringWithFormat:@"%@ %@", row.name ?: @"", row.kind ?: @""], tokens);
        }]] : self.symbolRows;
    ...
    NSArray<NSString *> *tokens = RYGRuntimeTokens(query);
    if (!tokens.count) {
        self.visibleClassRows = self.classRows ?: @[]; self.memberMatchCounts = @{};
        self.emptyLabel.text = @"No class with a hookable BOOL method was found in this image.";
        self.tableView.backgroundView = self.visibleClassRows.count ? nil : self.emptyLabel; [self.tableView reloadData]; return;
    }
    ...
    dispatch_async(dispatch_get_global_queue(QOS_CLASS_USER_INITIATED, 0), ^{
        ...
        for (RYGRuntimeClassRow *row in classes) {
            ...
            if (classHit || matchCount) { [hits addObject:row]; if (matchCount) counts[row.className] = @(matchCount); }
        }
        dispatch_async(dispatch_get_main_queue(), ^{
            __strong typeof(weakSelf) self = weakSelf;
            if (!self || generation != self.searchGeneration || ![self.selectedImagePath isEqualToString:path]) return;
            self.visibleClassRows = hits.copy; self.memberMatchCounts = counts.copy; [self.spinner stopAnimating];
            self.emptyLabel.text = @"No hookable BOOL method matches this search."; self.tableView.backgroundView = hits.count ? nil : self.emptyLabel; [self.tableView reloadData];
        });
    });
}
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    ...
    return self.modeControl.selectedSegmentIndex == RYGRuntimeTopModeClasses ? self.visibleClassRows.count : self.visibleSymbolRows.count;
}

- (NSString *)tableView:(UITableView *)tableView titleForHeaderInSection:(NSInteger)section {
    (void)tableView; (void)section;
    return self.modeControl.selectedSegmentIndex == RYGRuntimeTopModeClasses ? [NSString stringWithFormat:@"%lu classes with hookable BOOLs", (unsigned long)self.visibleClassRows.count] : [NSString stringWithFormat:@"%lu Mach-O symbols", (unsigned long)self.visibleSymbolRows.count];
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    cell.accessoryType = UITableViewCellAccessoryDisclosureIndicator; cell.detailTextLabel.textColor = UIColor.secondaryLabelColor;
    if (self.modeControl.selectedSegmentIndex == RYGRuntimeTopModeClasses) {
        ...
        cell.textLabel.text = row.className; cell.textLabel.font = [UIFont monospacedSystemFontOfSize:13.0 weight:UIFontWeightMedium];
        NSNumber *matches = self.memberMatchCounts[row.className];
        cell.detailTextLabel.text = matches ? [NSString stringWithFormat:@"%@ matching BOOL methods", matches] : [NSString stringWithFormat:@"%lu instance · %lu class BOOL methods", (unsigned long)row.instanceMethodCount, (unsigned long)row.classMethodCount];
    } else {
        RYGMachOSymbol *row = self.visibleSymbolRows[(NSUInteger)indexPath.row];
        cell.textLabel.text = row.name; cell.textLabel.font = [UIFont monospacedSystemFontOfSize:12.0 weight:UIFontWeightMedium];
        cell.detailTextLabel.text = [NSString stringWithFormat:@"%@ · 0x%llx", row.kind ?: @"symbol", (unsigned long long)row.address];
    }
    return cell;
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (self.modeControl.selectedSegmentIndex == RYGRuntimeTopModeClasses) {
        RYGRuntimeClassRow *row = self.visibleClassRows[(NSUInteger)indexPath.row];
        RYGRuntimeClassDetailViewController *detail = [[RYGRuntimeClassDetailViewController alloc] initWithClassRow:row initialQuery:self.searchController.searchBar.text];
