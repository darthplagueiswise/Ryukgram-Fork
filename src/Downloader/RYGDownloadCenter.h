...
/// Duplicate-check identities, written to the ledger once the job finishes.
@property (nonatomic, copy, nullable) NSArray<NSString *> *duplicateKeys;
/// Gallery rows this job created, so a later duplicate prompt can offer to drop them.
@property (nonatomic, copy, nullable) NSArray<NSString *> *galleryFileIDs;
- (void)noteGalleryFileID:(nullable NSString *)identifier;
