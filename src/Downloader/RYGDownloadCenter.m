...
- (void)noteGalleryFileID:(NSString *)identifier {
    if (!identifier.length) return;
    NSMutableArray *ids = [self.galleryFileIDs mutableCopy] ?: [NSMutableArray array];
    if (![ids containsObject:identifier]) [ids addObject:identifier];
    self.galleryFileIDs = ids;
}
...
- (void)markJobFinished:(RYGDownloadJob *)job {
    ...
    job.cancelBlock = nil;
    [RYGDownloadLedger recordKeys:job.duplicateKeys label:job.title galleryFileIDs:job.galleryFileIDs];
    [self trimTerminal];
    [self persistHistory];
    [self pump];
}
...
- (void)retryJob:(RYGDownloadJob *)job {
    ...
    if (rb) {
        [self removeJob:job];
        [RYGDownloadLedger withBypass:rb];
        return;
    }
    ...
    if (!builder) return;
    [self removeJob:job];
    [RYGDownloadLedger withBypass:^{ builder(info); }];
}
