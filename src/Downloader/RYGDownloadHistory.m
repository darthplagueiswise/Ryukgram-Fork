...
+ (RYGDownloadJob *)jobFromDict:(NSDictionary *)d {
	...
	job.title = RYGJSONString(d[@"title"]) ?: RYGLocalized(@"Download");
	job.subtitle = RYGJSONString(d[@"subtitle"]);
	job.successText = RYGJSONString(d[@"successText"]);
	job.mediaKind = (RYGDownloadMediaKind)[RYGJSONScalar(d[@"mediaKind"]) integerValue];
	job.kind = (RYGDownloadJobKind)[RYGJSONScalar(d[@"kind"]) integerValue];
	job.bytesExpected = [RYGJSONScalar(d[@"bytes"]) longLongValue];
	...
	// Usually already gone: HD output is a 15-min scratch file, plain output lives in Caches.
