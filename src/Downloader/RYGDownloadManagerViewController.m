...
- (NSString *)tableView:(UITableView *)tv titleForHeaderInSection:(NSInteger)s {
	...
	if (s == 4) return RYGLocalized(@"Duplicates");
...
- (NSString *)tableView:(UITableView *)tv titleForFooterInSection:(NSInteger)s {
	...
	if (s == 4) return RYGLocalized(@"Keeps a list of what you already grabbed and asks first when the same media comes up again. Only IDs are stored, never files, and carousels ask once for the whole post.");
...
- (UITableViewCell *)tableView:(UITableView *)tv cellForRowAtIndexPath:(NSIndexPath *)ip {
	...
	if (ip.section == 0) {
		...
		cell.accessoryView = sw;
	} else if (ip.section == 4 && ip.row == 0) {
		cell.textLabel.text = RYGLocalized(@"Check for duplicates");
		UISwitch *sw = [UISwitch new];
		sw.on = [RYGUtils getBoolPref:@"dl_duplicate_check"];
		[sw addTarget:self action:@selector(duplicateCheckToggled:) forControlEvents:UIControlEventValueChanged];
		cell.accessoryView = sw;
	} else if (ip.section == 4) {
		cell.textLabel.text = RYGLocalized(@"Clear duplicate list");
...
- (void)tableView:(UITableView *)tv didSelectRowAtIndexPath:(NSIndexPath *)ip {
	...
	if (ip.section == 4 && ip.row == 1) [RYGDownloadManagerViewController presentClearDuplicatesConfirmation];
}

- (void)duplicateCheckToggled:(UISwitch *)sw {
	[[NSUserDefaults standardUserDefaults] setObject:@(sw.on) forKey:@"dl_duplicate_check"];
}
...
+ (void)presentClearDuplicatesConfirmation {
	...
	NSUInteger count = [RYGDownloadLedger count];
	NSString *message = count > 0
		? [NSString stringWithFormat:RYGLocalized(@"%lu entries will be forgotten. Downloaded files are untouched."), (unsigned long)count]
		: RYGLocalized(@"Nothing to clear");

	UIAlertController *alert = [UIAlertController alertControllerWithTitle:RYGLocalized(@"Clear duplicate list")
	                                                              message:message
	                                                       preferredStyle:UIAlertControllerStyleAlert];
	[alert addAction:[UIAlertAction actionWithTitle:RYGLocalized(@"Cancel") style:UIAlertActionStyleCancel handler:nil]];
	[alert addAction:[UIAlertAction actionWithTitle:RYGLocalized(@"Clear") style:UIAlertActionStyleDestructive handler:^(__unused id a) {
		[RYGDownloadLedger clearAll];
		rygDLHaptic();
	}]];
	[top presentViewController:alert animated:YES completion:nil];
}
