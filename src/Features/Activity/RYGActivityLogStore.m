...
static RYGActivityLogEvent *eventFromDict(NSDictionary *e, NSString *pk, NSString *un, NSString *pic) {
    ...
    ev.type = (RYGActivityType)[RYGJSONScalar(e[@"t"]) unsignedIntegerValue];
    ev.pk = pk;
    ev.username = un;
    ev.profilePicURL = pic;
    ev.at = [NSDate dateWithTimeIntervalSince1970:[RYGJSONScalar(e[@"ts"]) doubleValue]];
    return ev;
}
...
+ (void)appendType:(RYGActivityType)type pk:(NSString *)pk username:(NSString *)username picURL:(NSString *)picURL ownerPK:(NSString *)ownerPK {
    ...
    dispatch_async(lsQ(), ^{
        ...
        NSMutableDictionary *rec = [(RYGJSONDict(root[pk]) ?: @{}) mutableCopy];
        if (username.length) rec[@"username"] = username;
        if (picURL.length) rec[@"pic"] = picURL;
        NSMutableArray *events = [(RYGJSONArray(rec[@"events"]) ?: @[]) mutableCopy];
        ...
        for (NSDictionary *e in events) if (RYGJSONDict(e) && [RYGJSONScalar(e[@"ts"]) doubleValue] >= cutoff) [kept addObject:e];
...
+ (NSArray<RYGActivityLogEvent *> *)eventsForPK:(NSString *)pk ownerPK:(NSString *)ownerPK {
    ...
    dispatch_sync(lsQ(), ^{
        NSDictionary *rec = RYGJSONDict(load(owner)[pk]);
        NSString *un = lsStr(rec[@"username"]), *pic = lsStr(rec[@"pic"]);
        for (NSDictionary *e in RYGJSONArray(rec[@"events"]) ?: @[]) if (RYGJSONDict(e)) [out addObject:eventFromDict(e, pk, un, pic)];
    });
...
+ (void)applyUsername:(NSString *)username picURL:(NSString *)picURL forPK:(NSString *)pk ownerPK:(NSString *)ownerPK {
    ...
    dispatch_async(lsQ(), ^{
        NSMutableDictionary *root = load(owner);
        NSDictionary *cur = RYGJSONDict(root[pk]);
        if (!cur) return;
...
+ (void)deleteEventsMatchingMask:(RYGActivityType)mask forPK:(NSString *)pk ownerPK:(NSString *)ownerPK {
    ...
    dispatch_async(lsQ(), ^{
        ...
        NSMutableDictionary *rec = [cur mutableCopy];
        NSMutableArray *kept = [NSMutableArray array];
        for (NSDictionary *e in RYGJSONArray(rec[@"events"]) ?: @[])
            if (RYGJSONDict(e) && !([RYGJSONScalar(e[@"t"]) unsignedIntegerValue] & mask)) [kept addObject:e];
        rec[@"events"] = kept;
        root[pk] = rec;
        save(owner);
        post();
...
+ (void)deleteEventOfType:(RYGActivityType)type atTimestamp:(double)ts forPK:(NSString *)pk ownerPK:(NSString *)ownerPK {
    ...
    dispatch_async(lsQ(), ^{
        ...
        BOOL removed = NO;
        for (NSDictionary *e in RYGJSONArray(rec[@"events"]) ?: @[]) {
            if (!removed && RYGJSONDict(e)
                && [RYGJSONScalar(e[@"t"]) unsignedIntegerValue] == type && [RYGJSONScalar(e[@"ts"]) doubleValue] == ts) { removed = YES; continue; }
            [kept addObject:e];
        }
        if (!removed) return;
        rec[@"events"] = kept;
