...
// Call-end = VPIO disposed and NOT recreated. IG re-creates it mid-call, so debounce:
// a new call unit within the window cancels the pending stop.
...
// WebRTC recreates the VPIO unit repeatedly mid-call — capture rides across it (the
// new unit's callbacks keep writing). Only a real call-end stops us, so no unitLost.
...
- (void)toggleWithVideo:(BOOL)video meta:(RYGCallRecording *)meta {
	if (self.isRecording) [self stop];
	else [self startWithVideo:video meta:meta];
}
...
- (void)startWithVideo:(BOOL)video meta:(RYGCallRecording *)meta {
	...
	self.groupActive = NO;
	if (video) {
		// One compositor for every video call — it adapts layout to the live cameras
		// (full / PiP / grid), so no up-front 1:1-vs-group guess is needed.
