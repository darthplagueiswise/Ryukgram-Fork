...
static NSTimeInterval seenThresholdForGroup(NSDictionary *st, NSString *key) {
	NSTimeInterval global = [RYGJSONScalar(st[@"lastSeenAt"]) doubleValue];
	NSDictionary *gs = RYGJSONDict(st[@"groupSeenAt"]);
	NSTimeInterval grp = [RYGJSONScalar(gs[key]) doubleValue];
...
+ (void)markAllSeenForOwnerPK:(NSString *)ownerPK {
	...
	sync_(^{
		...
		if (newest <= [RYGJSONScalar(st[@"lastSeenAt"]) doubleValue]) return;
