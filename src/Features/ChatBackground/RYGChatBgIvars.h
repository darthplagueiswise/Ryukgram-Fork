...
// rygStrIvar / rygAnyIvar pattern in RYGDeletedMessagesCapture.
...
static inline id _Nullable RYGBgIvarValue(id _Nullable obj, const char *name) {
...
static inline NSString *_Nullable RYGBgStringIvar(id _Nullable obj, const char *name) {
    id v = RYGBgIvarValue(obj, name);
...
static inline NSString *_Nullable RYGBgReadTidFromContainer(id _Nullable container) {
    ...
    for (int i = 0; kTidIvars[i]; i++) {
        id v = RYGBgIvarValue(container, kTidIvars[i]);
        ...
        if (v) {
            ...
            for (int j = 0; kInnerIds[j]; j++) {
                NSString *s = RYGBgStringIvar(v, kInnerIds[j]);
...
static inline id _Nullable RYGBgFindThreadKey(id _Nullable obj) {
    if (!obj) return nil;
    unsigned int n = 0;
