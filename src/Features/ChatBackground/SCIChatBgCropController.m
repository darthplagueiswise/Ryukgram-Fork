...
@implementation SCIChatBgCropController {
...
- (void)viewDidLoad {
	...
	_hint = [UILabel new];
	_hint.text = SCILocalized(@"Pinch + drag to position");
	...
	_cancelBtn = [self buttonWithTitle:SCILocalized(@"Cancel") action:@selector(cancelTapped) bold:NO];
	_useBtn = [self buttonWithTitle:SCILocalized(@"Use") action:@selector(useTapped) bold:YES];
