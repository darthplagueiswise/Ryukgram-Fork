...
static void *kSCIReadyCtx = &kSCIReadyCtx;

@implementation SCIChatBgVideoView
...
- (instancetype)initWithFrame:(CGRect)frame {
	if ((self = [super initWithFrame:frame])) {
		...
		[NSNotificationCenter.defaultCenter addObserver:self selector:@selector(sci_appPaused)
			name:UIApplicationDidEnterBackgroundNotification object:nil];
		[NSNotificationCenter.defaultCenter addObserver:self selector:@selector(sci_appResumed)
...
- (void)dealloc {
	...
	if (_observingLayer) {
		[self.playerLayer removeObserver:self forKeyPath:@"readyForDisplay" context:kSCIReadyCtx];
		_observingLayer = NO;
	}
...
- (void)loadPlayer {
	...
	if (!self.observingLayer) {
		[self.playerLayer addObserver:self forKeyPath:@"readyForDisplay" options:NSKeyValueObservingOptionNew context:kSCIReadyCtx];
		self.observingLayer = YES;
	}

	[NSNotificationCenter.defaultCenter addObserver:self selector:@selector(sci_loop:)
		name:AVPlayerItemDidPlayToEndTimeNotification object:self.item];
}

- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context {
	if (context != kSCIReadyCtx) {
...
- (void)sci_loop:(NSNotification *)note {
...
- (void)sci_appPaused { [self teardownPlayer]; }
- (void)sci_appResumed { [self play]; }
