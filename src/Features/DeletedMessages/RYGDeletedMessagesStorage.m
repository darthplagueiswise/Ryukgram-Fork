...
+ (void)mergeImportedStoreAtPath:(NSString *)importedDir {
	...
	rygSync(^{
		...
		NSString *srcMediaRoot = [importedDir stringByAppendingPathComponent:kRYGDMMediaDir];
		for (NSString *owner in [rygFM() contentsOfDirectoryAtPath:srcMediaRoot error:nil] ?: @[]) {
			...
			NSString *dstOwnerDir = rygMediaDir(owner);
