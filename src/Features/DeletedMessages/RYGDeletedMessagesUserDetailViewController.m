...
- (void)applyMessage:(RYGDeletedMessage *)m ownerPK:(NSString *)ownerPK playing:(BOOL)playing outgoing:(BOOL)outgoing {
	...
	NSString *time = [RYGDeletedMessagesDate stringForDate:(m.deletedAt ?: m.capturedAt ?: m.sentAt)];
...
- (NSString *)cleanFilenameForFileURL:(NSURL *)src metadata:(RYGGallerySaveMetadata *)md {
	RYGGalleryMediaType mt = RYGGalleryMediaTypeForExtension(src.pathExtension);
	NSString *ext = src.pathExtension;
	NSString *name = RYGFileNameForMedia(src, mt, md);
