...
- (void)confirmTitle:(NSString *)title message:(NSString *)message action:(dispatch_block_t)action {
	...
	[a addAction:[UIAlertAction actionWithTitle:SCILocalized(@"Clear")
...
- (NSString *)tableView:(UITableView *)tv titleForHeaderInSection:(NSInteger)section {
	return section == 0 ? SCILocalized(@"This account") : SCILocalized(@"Manage");
}

- (NSString *)tableView:(UITableView *)tv titleForFooterInSection:(NSInteger)section {
	return section == 1
		? SCILocalized(@"Clearing media keeps the records (text, sender, timestamp). Clearing the log removes everything for this account.")
		: nil;
}
...
- (UITableViewCell *)tableView:(UITableView *)tv cellForRowAtIndexPath:(NSIndexPath *)ip {
	...
	if (ip.section == 0 && ip.row == 0) {
		...
		[self configureCell:cell
					  title:SCILocalized(@"Media on disk")
		...
		[self configureCell:cell
					  title:SCILocalized(@"Clear media files")
					 detail:[self.class formatBytes:self.mediaBytes]
		...
		[self configureCell:cell
					  title:SCILocalized(@"Clear log for this account")
					 detail:nil
					 symbol:@"trash"
...
- (void)confirmClearMedia {
	...
	[self confirmTitle:SCILocalized(@"Clear media files")
...
- (void)confirmClearAll {
	...
	__weak typeof(self) ws = self;
	[self confirmTitle:SCILocalized(@"Clear log for this account")
			   message:SCILocalized(@"Removes every preserved deleted message and its captured media for the current account. This cannot be undone.")
				action:^{
		[SCIDeletedMessagesStorage resetForOwnerPK:ws.ownerPK];
	}];
}
...
- (void)clearMediaFiles {
	...
	for (SCIDeletedMessage *m in all) {
		for (NSString *rel in @[m.mediaPath ?: @"", m.thumbnailPath ?: @""]) {
			NSString *abs = [SCIDeletedMessagesStorage absolutePathForRelativePath:rel ownerPK:self.ownerPK];
	...
	[SCIDeletedMessagesStorage saveMessages:all forOwnerPK:self.ownerPK];
	[self reload];
}
