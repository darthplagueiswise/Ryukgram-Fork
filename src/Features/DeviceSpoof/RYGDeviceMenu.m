...
+ (void)presentCustomMachineIDFrom:(UIViewController *)host onChange:(void (^)(void))onChange {
    if (!host) return;
    NSString *pinned = [RYGDeviceIdentity pinnedMachineID];
    UIAlertController *a = [UIAlertController
        alertControllerWithTitle:RYGLocalized(@"Manual machine ID")
        message:RYGLocalized(@"Only paste one Instagram issued. The server checks its format, so a made-up value is rejected.")
        preferredStyle:UIAlertControllerStyleAlert];
    [a addTextFieldWithConfigurationHandler:^(UITextField *tf) {
        tf.autocorrectionType = UITextAutocorrectionTypeNo;
        tf.text = pinned ?: @"";
    ...
    [a addAction:[UIAlertAction actionWithTitle:RYGLocalized(@"Apply")
        style:UIAlertActionStyleDefault handler:^(__unused UIAlertAction *x) {
            [RYGDeviceIdentity setCustomMachineID:a.textFields.firstObject.text];
            if (onChange) onChange();
            RYGToast(RYGLocalized(@"Machine ID set"), RYGLocalized(@"Relaunch to apply"));
        }]];
    [a addAction:[UIAlertAction actionWithTitle:RYGLocalized(@"Clear and re-register")
        style:UIAlertActionStyleDestructive handler:^(__unused UIAlertAction *x) {
            [RYGDeviceIdentity clearMachineID];
            if (onChange) onChange();
            RYGToast(RYGLocalized(@"Machine ID cleared"), RYGLocalized(@"Relaunch to apply"));
        }]];
    [a addAction:[UIAlertAction actionWithTitle:RYGLocalized(@"Cancel")
        style:UIAlertActionStyleCancel handler:nil]];
    [host presentViewController:a animated:YES completion:nil];
...
+ (void)copyMachineID {
    NSString *mid = [RYGDeviceIdentity pinnedMachineID];
    if (!mid.length) {
        RYGToast(RYGLocalized(@"No machine ID yet"), nil);
        return;
    }
    [UIPasteboard generalPasteboard].string = mid;
...
+ (void)presentMaskEverythingConfirmFrom:(UIViewController *)host {
    if (!host) return;
    UIAlertController *a = [UIAlertController
        alertControllerWithTitle:RYGLocalized(@"Mask everything & relaunch?")
        message:RYGLocalized(@"Rolls every identifier, blocks Apple attestation, and clears your saved logins and cookies. You sign in again afterwards.")
        preferredStyle:UIAlertControllerStyleAlert];
    [a addAction:[UIAlertAction actionWithTitle:RYGLocalized(@"Mask everything")
        style:UIAlertActionStyleDestructive handler:^(__unused UIAlertAction *x) {
            [RYGDeviceIdentity maskEverythingAndTerminate];
        }]];
    [a addAction:[UIAlertAction actionWithTitle:RYGLocalized(@"Cancel")
        style:UIAlertActionStyleCancel handler:nil]];
    [host presentViewController:a animated:YES completion:nil];
