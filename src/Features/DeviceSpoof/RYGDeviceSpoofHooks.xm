...
// spoofing is enabled, guards the keychain reads those getters don't cover, and
// optionally blocks Apple device attestation.
...
// The getter hooks miss IG's direct keychain reads, and IG rewrites those items
// when it re-registers. Answering from the pin is what outlives both.
static OSStatus (*orig_SecItemCopyMatching)(CFDictionaryRef, CFTypeRef *);
static OSStatus new_SecItemCopyMatching(CFDictionaryRef query, CFTypeRef *result) {
    if (![RYGDeviceIdentity spoofingEnabled] || [RYGDeviceIdentity rawKeychainAccessInProgress])
        return orig_SecItemCopyMatching(query, result);

    NSString *pinned = [RYGDeviceIdentity pinnedValueForKeychainQuery:(__bridge NSDictionary *)query];
    if (!pinned) return orig_SecItemCopyMatching(query, result);


    if (result) *result = (__bridge_retained CFTypeRef)[pinned dataUsingEncoding:NSUTF8StringEncoding];
...
%ctor {
    @autoreleasepool {
        ...
        [RYGDeviceIdentity enforcePinnedIdentity];
