...
static NSString *new_foaGetDeviceId(id self, SEL _cmd) {
    if ([SCIDeviceIdentity spoofingEnabled]) return [SCIDeviceIdentity spoofedDeviceID];
...
static NSString *new_fdid(id self, SEL _cmd, id cfg, id file, id line) {
    if ([SCIDeviceIdentity spoofingEnabled]) return [SCIDeviceIdentity spoofedFDID];
...
static NSUUID *new_idfv(id self, SEL _cmd) {
    if ([SCIDeviceIdentity spoofingEnabled]) {
        NSUUID *u = [[NSUUID alloc] initWithUUIDString:[SCIDeviceIdentity spoofedFDID]];
        if (u) return u;
    }
...
static NSError *sciAttestUnsupported(void) {
    return [NSError errorWithDomain:@"com.apple.devicecheck.error" code:1 userInfo:nil];
}
static BOOL sciAttestBlocked(void) {
    return [SCIDeviceIdentity spoofingEnabled] &&
           [SCIUtils getBoolPref:SCIDeviceSpoofBlockDeviceCheckKey];
}
...
static void new_dcGenerate(id self, SEL _cmd, void (^handler)(NSData *, NSError *)) {
    if (sciAttestBlocked()) { if (handler) handler(nil, sciAttestUnsupported()); return; }
