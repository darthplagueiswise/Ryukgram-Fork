...
// This layer covers only selectors that actually exist at runtime and have an
// exact zero-argument BOOL ABI. It never adds methods to a class and never treats
// the ig_is_employee* DATA descriptors as callable functions.
...
static void ETDInstallExistingIdentitySelectors(void) {
    static NSArray<NSString *> *getters;
    static NSArray<NSString *> *setters;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        // isEmployee itself is covered by SCIEmployeeInternal.x and exact
        // consumer hooks. These aliases extend test-user/dogfood/internal
        // surfaces only when the current runtime genuinely declares them.
        getters = @[
            @"isTestUser",
            @"isTestAccount",
            @"isEmployeeOrTestUser",
            @"isEmployeeOrTestAccount",
            @"isDogfooder",
            @"isDogfood",
            @"isDogfooding",
            @"isInternalUser",
    ...
    free(classes);
    ETDLOG("runtime identity aliases installed/active=%lu classes=%d",
        (unsigned long)installed, count);
}

#pragma mark - MobileConfig descriptor identity gates

static NSString *const kETDDescriptorOwnerKey =
    @"sci_employee_test_dogfood_descriptor_owner";
static NSString *const kETDDescriptorPreviousKey =
    @"sci_employee_test_dogfood_descriptor_previous";
static BOOL sETDSyncingDescriptors = NO;

static NSArray<NSString *> *ETDDescriptorSymbols(void) {
...
static void ETDSyncDescriptorForces(void) {
    @synchronized (SCICSymbolStub.class) {
        if (sETDSyncingDescriptors) return;
        sETDSyncingDescriptors = YES;

        NSUserDefaults *defaults = NSUserDefaults.standardUserDefaults;
        BOOL owner = [defaults boolForKey:kETDDescriptorOwnerKey];
        if (ETDMasterOn()) {
            if (!owner) {
                NSMutableDictionary *previous = [NSMutableDictionary dictionary];
                for (NSString *symbol in ETDDescriptorSymbols()) {
                    NSNumber *value = [SCICSymbolStub
                        forceForParamDescriptorSymbol:symbol];
                    previous[symbol] = value ?: NSNull.null;
                }
                [defaults setObject:previous forKey:kETDDescriptorPreviousKey];
                [defaults setBool:YES forKey:kETDDescriptorOwnerKey];
            }
            for (NSString *symbol in ETDDescriptorSymbols()) {
                [SCICSymbolStub setParamDescriptorForce:@YES forSymbol:symbol];
            }
        } else if (owner) {
            NSDictionary *previous = [defaults dictionaryForKey:kETDDescriptorPreviousKey];
            for (NSString *symbol in ETDDescriptorSymbols()) {
            ...
            [defaults removeObjectForKey:kETDDescriptorPreviousKey];
            [defaults removeObjectForKey:kETDDescriptorOwnerKey];
        }

        sETDSyncingDescriptors = NO;
    }
}

static void ETDInstall(void) {
    ETDInstallExistingIdentitySelectors();
...
static void SCIEmployeeTestDogfoodRuntimeHooksCtor(void) {
    @autoreleasepool {
        // Install method hooks regardless of the initial preference. Every
        // replacement reads the consolidated master live, so enabling it later
        // does not depend on re-hooking an already-created user/session object.
        ETDInstall();
