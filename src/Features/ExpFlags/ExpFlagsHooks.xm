// Vanilla-derived developer hooks. Installed only when ryg_exp_flags_enabled.
// MetaLocalExperiment is overridable by experiment-name substring.
// MobileConfig getters are observed only here; the mapped editor remains separate.
...
static id overrideGroupFor(NSString *expName, id origGroup) {
    ...
    for (NSString *key in [RYGExpFlags allOverriddenNames]) {
        if (![lower containsString:key.lowercaseString]) continue;
        RYGExpFlagOverride o = [RYGExpFlags overrideForName:key];
        if (o == RYGExpFlagOverrideTrue)  return @"test";
        if (o == RYGExpFlagOverrideFalse) return nil;
    }
    return origGroup;
}
...
static id new_groupName(id self, SEL _cmd) {
    ...
    [RYGExpFlags recordExperimentName:name group:[orig isKindOfClass:[NSString class]] ? orig : nil];
...
static BOOL new_mcBool(id self, SEL _cmd, unsigned long long pid) {
    BOOL v = orig_mcBool ? orig_mcBool(self, _cmd, pid) : NO;
    [RYGExpFlags recordMCParamID:pid type:RYGExpMCTypeBool defaultValue:v ? @"YES" : @"NO"];
    return v;
}
...
static BOOL new_mcBool_def(id self, SEL _cmd, unsigned long long pid, BOOL def) {
    [RYGExpFlags recordMCParamID:pid type:RYGExpMCTypeBool defaultValue:def ? @"YES" : @"NO"];
    return orig_mcBool_def ? orig_mcBool_def(self, _cmd, pid, def) : def;
}
static long long (*orig_mcInt)(id, SEL, unsigned long long);
static long long new_mcInt(id self, SEL _cmd, unsigned long long pid) {
    long long v = orig_mcInt ? orig_mcInt(self, _cmd, pid) : 0;
    [RYGExpFlags recordMCParamID:pid type:RYGExpMCTypeInt defaultValue:[NSString stringWithFormat:@"%lld", v]];
    return v;
}
...
static long long new_mcInt_def(id self, SEL _cmd, unsigned long long pid, long long def) {
    [RYGExpFlags recordMCParamID:pid type:RYGExpMCTypeInt defaultValue:[NSString stringWithFormat:@"%lld", def]];
    return orig_mcInt_def ? orig_mcInt_def(self, _cmd, pid, def) : def;
}
static double (*orig_mcDouble)(id, SEL, unsigned long long);
static double new_mcDouble(id self, SEL _cmd, unsigned long long pid) {
    double v = orig_mcDouble ? orig_mcDouble(self, _cmd, pid) : 0.0;
    [RYGExpFlags recordMCParamID:pid type:RYGExpMCTypeDouble defaultValue:[NSString stringWithFormat:@"%f", v]];
    return v;
}
...
static double new_mcDouble_def(id self, SEL _cmd, unsigned long long pid, double def) {
    [RYGExpFlags recordMCParamID:pid type:RYGExpMCTypeDouble defaultValue:[NSString stringWithFormat:@"%f", def]];
    return orig_mcDouble_def ? orig_mcDouble_def(self, _cmd, pid, def) : def;
}
static id (*orig_mcString)(id, SEL, unsigned long long);
static id new_mcString(id self, SEL _cmd, unsigned long long pid) {
    id v = orig_mcString ? orig_mcString(self, _cmd, pid) : nil;
    [RYGExpFlags recordMCParamID:pid type:RYGExpMCTypeString defaultValue:[v description] ?: @""];
    return v;
}
...
static id new_mcString_def(id self, SEL _cmd, unsigned long long pid, id def) {
    [RYGExpFlags recordMCParamID:pid type:RYGExpMCTypeString defaultValue:[def description] ?: @""];
    return orig_mcString_def ? orig_mcString_def(self, _cmd, pid, def) : def;
}

static void install(Class cls, NSString *selName, IMP newImp, IMP *origOut) {
    if (!cls || !newImp || !origOut || *origOut) return;
...
%ctor {
    if (![RYGUtils getBoolPref:@"ryg_exp_flags_enabled"]) return;

    if ([RYGExpFlags checkAndHandleCrashLoop]) {
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(5 * NSEC_PER_SEC)),
                       dispatch_get_main_queue(), ^{
            [RYGUtils showToastForDuration:4.0 title:@"Exp flags reset after repeated crashes"];
        });
    }
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(30 * NSEC_PER_SEC)),
                   dispatch_get_main_queue(), ^{ [RYGExpFlags markLaunchStable]; });
    ...
    // Match vanilla: observe actual MC calls, don't enumerate the full mapping in UI.
    Class mc = NSClassFromString(@"IGMobileConfigContextManager");
    if (!mc) mc = NSClassFromString(@"FBMobileConfigContextManager");
