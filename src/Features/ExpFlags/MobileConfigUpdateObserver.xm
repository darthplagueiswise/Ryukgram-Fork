...
static BOOL RGMCUpdateObserverEnabled(void) {
    return [SCIUtils getBoolPref:@"igt_mobileconfig_update_observer"] ||
...
static void RGInstallMCUpdateObserver(const char *phase) {
    if (!RGMCUpdateObserverEnabled()) {
        NSLog(@"[RyukGram][MCUpdate] observer disabled at %s", phase ?: "unknown");
        return;
    }
    ...
    NSLog(@"[RyukGram][MCUpdate] observer fishhook phase=%s rc=%d try=%p force=%p", phase ?: "unknown", rc, orig_RGTryUpdate, orig_RGForceUpdate);
}

%ctor {
    RGInstallMCUpdateObserver("ctor");

    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(2 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        RGInstallMCUpdateObserver("delayed");
    });
}
