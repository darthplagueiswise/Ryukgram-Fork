...
- (BOOL)ryg_importNameMappingData:(NSData *)data
    ...
    // Rebuild the union from scratch. This is essential for Replace: names that
    // disappeared from the new mapping must disappear from the in-memory model,
    // while mapping-only configs/params still need to be materialized.
...
- (BOOL)ryg_importAndApplyOverridesData:(NSData *)data
    ...
    if (![json isKindOfClass:NSDictionary.class]) {
        if (error && !*error) *error = RYGMCJSONError(
            @"mc_overrides.json must be a JSON object keyed by \"<config>:\".");
        return NO;
    }
    ...
    for (id rawKey in (NSDictionary *)json) {
        ...
        char *configEnd = NULL;
        unsigned long long configNumber = strtoull(configKey.UTF8String, &configEnd, 10);
        if (!configKey.length || configEnd == configKey.UTF8String || *configEnd != '\0') continue;
        ...
        for (id rawLine in (NSArray *)lines) {
            ...
            char *indexEnd = NULL;
            unsigned long long index = strtoull(indexText.UTF8String, &indexEnd, 10);
            if (!indexText.length || indexEnd == indexText.UTF8String || *indexEnd != '\0') continue;
        ...
        for (RYGMCParam *param in config.params) {
            ...
            if (value) {
                [lines addObject:[NSString stringWithFormat:@"%u: : %@",
                    param.paramIndex, RYGMCJSONValueString(value, param.type)]];
            }
        }
