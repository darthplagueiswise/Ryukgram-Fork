...
static NSString *RYGMCNormalize(NSString *value) {
    ...
    for (NSUInteger i = 0; i < lower.length; i++) {
        ...
        if (alnum) { [result appendFormat:@"%C", c]; previousSpace = NO; }
        else if (!previousSpace) { [result appendString:@" "]; previousSpace = YES; }
    }
...
static BOOL RYGMCMatches(NSString *text, NSArray<NSString *> *tokens) {
    ...
    for (NSString *token in tokens) {
        if ([normalized rangeOfString:token].location == NSNotFound && [compact rangeOfString:token].location == NSNotFound) return NO;
    }
    return YES;
}
...
static NSArray<RYGMCConfig *> *RYGMCResolvedConfigs(void) {
    ...
    for (NSNumber *number in orderedNumbers) {
        ...
        [mappedParams enumerateKeysAndObjectsUsingBlock:^(NSNumber *index, NSString *name, BOOL *stop) {
            ...
            if (!param) {
                param = [RYGMCParam new]; param.configNumber = number.unsignedIntValue; param.paramIndex = index.unsignedIntValue;
                param.type = RYGMCTypeUnknown; param.runtimeBacked = NO; params[index] = param;
            }
...
- (void)viewDidLoad {
    [super viewDidLoad];
    self.title = @"MobileConfig"; self.scope = RYGMCBrowserScopeAll;
    ...
    self.searchController.searchBar.placeholder = @"Config, ID or parameter"; self.navigationItem.searchController = self.searchController; self.navigationItem.hidesSearchBarWhenScrolling = NO;
    ...
    [NSNotificationCenter.defaultCenter addObserver:self selector:@selector(namesDidChange:) name:kRYGMobileConfigNamesDidChangeNotification object:nil];
    RYGLiquidGlassApplyToViewController(self); [self reloadModel:NO];
}
...
- (UIMenu *)scopeMenu {
    ...
    for (NSInteger index = 0; index < (NSInteger)titles.count; index++) {
        UIAction *action = [UIAction actionWithTitle:titles[(NSUInteger)index] image:nil identifier:nil handler:^(__unused UIAction *item) {
            weakSelf.scope = (RYGMCBrowserScope)index; weakSelf.navigationItem.rightBarButtonItem.menu = [weakSelf scopeMenu]; [weakSelf rebuildVisibleConfigs];
        }];
        action.state = self.scope == index ? UIMenuElementStateOn : UIMenuElementStateOff; [actions addObject:action];
    }
...
- (void)rebuildVisibleConfigs {
    NSArray *tokens = RYGMCTokens(self.searchController.searchBar.text ?: @""); NSMutableArray *visible = [NSMutableArray array]; NSMutableDictionary *matches = [NSMutableDictionary dictionary];
    for (RYGMCConfig *config in self.allConfigs ?: @[]) {
        if (![self configPassesScope:config]) continue;
        BOOL configMatch = RYGMCMatches([NSString stringWithFormat:@"%@ %u", config.name ?: @"", config.number], tokens); NSMutableArray *paramMatches = [NSMutableArray array];
        ...
        if (!tokens.count || configMatch || paramMatches.count) { [visible addObject:config]; if (paramMatches.count) matches[@(config.number)] = paramMatches.copy; }
    }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    RYGMCConfig *config = self.visibleConfigs[(NSUInteger)indexPath.row]; cell.textLabel.text = config.name.length ? config.name : [NSString stringWithFormat:@"Config %u", config.number];
    cell.textLabel.font = [UIFont systemFontOfSize:16.0 weight:UIFontWeightSemibold]; cell.textLabel.lineBreakMode = NSLineBreakByTruncatingMiddle;
    NSArray<RYGMCParam *> *matched = self.searchMatches[@(config.number)];
    if (matched.count) { NSMutableArray *names = [NSMutableArray array]; for (RYGMCParam *param in matched) [names addObject:param.name.length ? param.name : [NSString stringWithFormat:@"#%u", param.paramIndex]]; cell.detailTextLabel.text = [NSString stringWithFormat:@"#%u · %@", config.number, [names componentsJoinedByString:@", "]]; cell.detailTextLabel.numberOfLines = 2; }
    else { cell.detailTextLabel.text = [NSString stringWithFormat:@"#%u", config.number]; cell.detailTextLabel.numberOfLines = 1; }
    ...
    cell.imageView.image = RYGMCConfigOverridden(config) ? [UIImage systemImageNamed:@"circle.fill"] : nil; cell.imageView.tintColor = [RYGUtils RYGColor_Primary]; return cell;
}
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    [tableView deselectRowAtIndexPath:indexPath animated:YES]; RYGMCResolvedConfigDetailViewController *detail = [[RYGMCResolvedConfigDetailViewController alloc] initWithStyle:UITableViewStyleInsetGrouped];
    detail.config = self.visibleConfigs[(NSUInteger)indexPath.row]; [self.navigationController pushViewController:detail animated:YES];
}
@end

@implementation RYGMCResolvedConfigDetailViewController
- (void)viewDidLoad {
    [super viewDidLoad]; NSString *title = self.config.name.length ? self.config.name : [NSString stringWithFormat:@"Config %u", self.config.number];
    self.title = title; self.navigationItem.titleView = RYGLiquidGlassNavigationTitleView(title); self.tableView.backgroundColor = [RYGPopupChrome backgroundColor]; self.view.backgroundColor = [RYGPopupChrome backgroundColor];
    self.tableView.rowHeight = UITableViewAutomaticDimension; RYGLiquidGlassApplyToViewController(self);
}
...
- (NSString *)tableView:(UITableView *)tableView titleForFooterInSection:(NSInteger)section {
    ...
    return @"Seen is telemetry only. Any parameter resolved from the native MobileConfig table can be overridden before Instagram reads it for the first time.";
}
- (void)boolSwitchChanged:(UISwitch *)toggle {
    RYGMCParam *param = objc_getAssociatedObject(toggle, kRYGMCParamKey); if (!param) return;
    // Deliberately no callSite/seen check. runtimeBacked means the parameter ID,
    // type and native table descriptor were resolved and are sufficient to write.
    if (![RYGMobileConfig.shared setOverride:@(toggle.isOn) for:param]) { toggle.on = !toggle.isOn; [RYGUtils showErrorHUDWithDescription:@"MobileConfig rejected this resolved override"]; }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    cell.textLabel.text = param.name.length ? param.name : [NSString stringWithFormat:@"Parameter %u", param.paramIndex]; cell.textLabel.font = [UIFont systemFontOfSize:15.0 weight:UIFontWeightRegular]; cell.textLabel.lineBreakMode = NSLineBreakByTruncatingMiddle;
    ...
    BOOL seen = param.isRuntimeBacked && [RYGMobileConfig.shared callSiteFor:param].length > 0;
    if (!param.isRuntimeBacked) cell.detailTextLabel.text = [NSString stringWithFormat:@"#%u · mapping only · no native PID/type", param.paramIndex];
    else if (forced) cell.detailTextLabel.text = [NSString stringWithFormat:@"#%u · override %@ · %@", param.paramIndex, forced, seen ? @"seen" : @"not seen yet"];
    else if (native) cell.detailTextLabel.text = [NSString stringWithFormat:@"#%u · native %@ · seen", param.paramIndex, native];
    else cell.detailTextLabel.text = [NSString stringWithFormat:@"#%u · not seen yet · overrideable now", param.paramIndex];
    cell.detailTextLabel.textColor = UIColor.secondaryLabelColor; cell.detailTextLabel.font = [UIFont monospacedSystemFontOfSize:11.0 weight:UIFontWeightRegular]; cell.detailTextLabel.numberOfLines = 2;
    if (!param.isRuntimeBacked) { cell.selectionStyle = UITableViewCellSelectionStyleNone; return cell; }
    if (param.type == RYGMCTypeBool) {
        UISwitch *toggle = [UISwitch new]; NSNumber *visibleValue = forced ?: ([native isKindOfClass:NSNumber.class] ? native : @NO);
        toggle.on = visibleValue.boolValue; toggle.onTintColor = [RYGUtils RYGColor_Primary]; objc_setAssociatedObject(toggle, kRYGMCParamKey, param, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
        [toggle addTarget:self action:@selector(boolSwitchChanged:) forControlEvents:UIControlEventValueChanged]; cell.accessoryView = toggle;
    } else cell.accessoryType = UITableViewCellAccessoryDisclosureIndicator;
    return cell;
}
- (void)presentBoolActions:(RYGMCParam *)param source:(UIView *)source {
    BOOL overridden = [RYGMobileConfig.shared overrideStateFor:param] == RYGMCOverrideSet;
    UIAlertController *sheet = [UIAlertController alertControllerWithTitle:param.name.length ? param.name : @"Boolean parameter"
                                                                    message:@"The switch writes immediately even when this ID has not been seen at runtime. Use Native removes the override."
    ...
    [sheet addAction:[UIAlertAction actionWithTitle:@"Force On" style:UIAlertActionStyleDefault handler:^(__unused UIAlertAction *action) {
        if (![RYGMobileConfig.shared setOverride:@YES for:param]) [RYGUtils showErrorHUDWithDescription:@"MobileConfig rejected this override"]; [weakSelf.tableView reloadData];
    }]];
    [sheet addAction:[UIAlertAction actionWithTitle:@"Force Off" style:UIAlertActionStyleDefault handler:^(__unused UIAlertAction *action) {
        if (![RYGMobileConfig.shared setOverride:@NO for:param]) [RYGUtils showErrorHUDWithDescription:@"MobileConfig rejected this override"]; [weakSelf.tableView reloadData];
    }]];
    if (overridden) [sheet addAction:[UIAlertAction actionWithTitle:@"Use Native" style:UIAlertActionStyleDestructive handler:^(__unused UIAlertAction *action) {
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    RYGMCParam *param = self.config.params[(NSUInteger)indexPath.row]; if (!param.isRuntimeBacked) return;
    if (param.type == RYGMCTypeBool) { [self presentBoolActions:param source:cell]; return; }
    ...
    [alert addTextFieldWithConfigurationHandler:^(UITextField *field) {
        id current = [RYGMobileConfig.shared overrideValueFor:param] ?: [RYGMobileConfig.shared liveValueFor:param]; field.text = current ? [current description] : @"";
        if (param.type == RYGMCTypeInt || param.type == RYGMCTypeDouble) field.keyboardType = UIKeyboardTypeNumbersAndPunctuation;
    }];
    __weak typeof(self) weakSelf = self; [alert addAction:[UIAlertAction actionWithTitle:@"Cancel" style:UIAlertActionStyleCancel handler:nil]];
    if ([RYGMobileConfig.shared overrideStateFor:param] == RYGMCOverrideSet) [alert addAction:[UIAlertAction actionWithTitle:@"Use native" style:UIAlertActionStyleDestructive handler:^(__unused UIAlertAction *action) { [RYGMobileConfig.shared clearOverrideFor:param]; [weakSelf.tableView reloadData]; }]];
    [alert addAction:[UIAlertAction actionWithTitle:@"Apply" style:UIAlertActionStyleDefault handler:^(__unused UIAlertAction *action) {
        NSString *text = alert.textFields.firstObject.text ?: @""; id value = nil;
        if (param.type == RYGMCTypeString) value = text;
        else if (param.type == RYGMCTypeInt) { char *end = NULL; long long number = strtoll(text.UTF8String, &end, 10); if (end && *end == '\0') value = @(number); }
        else if (param.type == RYGMCTypeDouble) { char *end = NULL; double number = strtod(text.UTF8String, &end); if (end && *end == '\0') value = @(number); }
        if (!value || ![RYGMobileConfig.shared setOverride:value for:param]) [RYGUtils showErrorHUDWithDescription:@"Invalid or rejected MobileConfig value"]; [weakSelf.tableView reloadData];
    }]];
