...
- (void)rebuildSections {
    ...
    RYGSetting *updateIDs = [RYGSetting buttonCellWithTitle:@"Update IDs"
                                                   subtitle:@"Rebuild runtime ↔ id_name_mapping match once"
                                                       icon:[RYGSymbol symbolWithName:@"arrow.clockwise"]
                                                     action:^{ [weakSelf updateIDs]; }];
    ...
    [self applySettingSections:@[
        ...
        [RYGSettingsViewController sectionWithHeader:@"Names" footer:nil rows:@[updateIDs, importNames, exportNames]],
...
- (void)updateIDs {
    dispatch_async(dispatch_get_global_queue(QOS_CLASS_UTILITY, 0), ^{
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            [RYGUtils showToastForDuration:1.3 title:@"IDs updated" subtitle:[NSString stringWithFormat:@"%lu configs · %lu params", (unsigned long)configs, (unsigned long)params]];
        });
    });
}
...
- (void)documentPicker:(UIDocumentPickerViewController *)controller didPickDocumentsAtURLs:(NSArray<NSURL *> *)urls {
    ...
    if (operation == RYGMCImportOperationNameMapping) {
        if (![mobileConfig ryg_importNameMappingData:data mode:self.pendingNameMappingMode error:&error]) { [RYGUtils showErrorHUDWithDescription:error.localizedDescription ?: @"Mapping import failed"]; return; }
        [mobileConfig ryg_syncPersistedJSONToNativeDataDirectory];
        NSUInteger configs = 0, params = 0;
        for (RYGMCConfig *config in mobileConfig.allConfigs) { if (config.name.length) configs++; for (RYGMCParam *param in config.params) if (param.name.length) params++; }
        [RYGUtils showToastForDuration:1.5 title:self.pendingNameMappingMode == RYGMCNameMappingImportModeReplace ? @"Mapping replaced" : @"Mapping merged" subtitle:[NSString stringWithFormat:@"%lu configs · %lu params", (unsigned long)configs, (unsigned long)params]];
        return;
    }

    if (operation == RYGMCImportOperationOverrides) {
        NSUInteger applied = 0;
        if (![mobileConfig ryg_importAndApplyOverridesData:data appliedCount:&applied error:&error]) { [RYGUtils showErrorHUDWithDescription:error.localizedDescription ?: @"Override import failed"]; return; }
        [mobileConfig ryg_syncPersistedJSONToNativeDataDirectory];
...
- (void)exportNameMapping {
    ...
    if (!data.length) { [RYGUtils showErrorHUDWithDescription:error.localizedDescription ?: @"No mapping available"]; return; }
