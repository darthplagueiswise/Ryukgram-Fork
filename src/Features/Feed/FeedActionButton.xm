...
static inline BOOL RYGFeedActionEnabled(void) {
	return [RYGUtils getBoolPref:@"feed_action_button"];
}

static inline NSString *RYGFeedDefaultAction(void) {
	return [RYGUtils getStringPref:@"feed_action_default"];
}

static BOOL rygFindFeedUFIContext(UIView *view, UICollectionViewCell **outUFICell, UICollectionView **outCollectionView) {
...
static NSInteger rygFeedCarouselPageIndex(UIView *button) {
	UICollectionViewCell *ufiCell = nil;
	UICollectionView *collectionView = nil;

	if (!rygFindFeedUFIContext(button, &ufiCell, &collectionView)) return -1;

	NSIndexPath *ufiPath = [collectionView indexPathForCell:ufiCell];
	if (!ufiPath) return -1;

	Class pageMediaClass = NSClassFromString(@"_TtC18IGFeedItemPageCell15IGPageMediaView") ?: NSClassFromString(@"IGPageMediaView");
...
static IGMedia *rygFeedMediaFromButton(UIView *button) {
	...
	if (!rygFindFeedUFIContext(button, &ufiCell, &collectionView)) return nil;
...
static void rygResetFeedActionButton(RYGChromeButton *button) {
	[button removeTarget:nil action:NULL forControlEvents:UIControlEventAllEvents];
	button.menu = nil;
	button.showsMenuAsPrimaryAction = NO;
}

static void rygConfigureFeedActionButton(RYGChromeButton *button) {
	rygResetFeedActionButton(button);

	[RYGActionButton configureButton:button
							 context:RYGActionContextFeed
							 prefKey:@"feed_action_default"
					   mediaProvider:^id (UIView *sourceView) {
		id parentMedia = rygFeedMediaFromButton(sourceView);
		if (!parentMedia) return nil;

		if ([RYGMediaActions isCarouselMedia:parentMedia]) {
			[RYGMediaActions stashCarouselParentMedia:parentMedia onView:sourceView];

			NSInteger index = rygFeedCarouselPageIndex(sourceView);
...
- (void)updateUFIWithButtonsConfig:(id)config interactionCountProvider:(id)provider {
	...
	RYGChromeButton *button = (RYGChromeButton *)[self viewWithTag:kFeedActionBtnTag];

	if (![button isKindOfClass:RYGChromeButton.class]) {
		button = nil;
	}

	if (!RYGFeedActionEnabled()) {
		[button removeFromSuperview];
		return;
	}

	if (!button) {
		button = [[RYGChromeButton alloc] initWithSymbol:@"" pointSize:21.0 diameter:36.0];
		...
		[RYGActionIcon attachAutoUpdate:button source:RYGActionSourceFeed pointSize:21.0 style:RYGActionIconStylePlain];
	}

	NSString *action = RYGFeedDefaultAction();
	...
	if (!configuredAction || ![configuredAction isEqualToString:action]) {
		rygConfigureFeedActionButton(button);
