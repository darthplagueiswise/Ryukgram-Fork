...
static NSString *rygTrayDiffID(id obj) {
    NSString *diffId = nil;
...
static BOOL rygIsHexUUIDString(NSString *s) {
...
static BOOL rygIsSuggestedTrayItem(id obj) {
    @try {
        ...
        if (!rygIsHexUUIDString(rygTrayDiffID(obj))) return NO;
...
static NSArray *hook_objectsForListAdapter(id self, SEL _cmd, id adapter) {
    ...
    if (![RYGUtils getBoolPref:@"hide_suggested_stories"]) return objects;

    BOOL anySuggested = NO;
    for (id obj in objects) {
        if (rygIsSuggestedTrayItem(obj)) { anySuggested = YES; break; }
    }
    if (!anySuggested) return objects;

    NSMutableArray *filtered = [NSMutableArray arrayWithCapacity:objects.count];
    for (id obj in objects) {
        if (!rygIsSuggestedTrayItem(obj)) [filtered addObject:obj];
    }
...
static id hook_allItemsForTray(id self, SEL _cmd, BOOL cached) {
    ...
    BOOL hideUsers = [RYGUtils getBoolPref:@"no_suggested_users"];
    BOOL hideAds = [RYGUtils getBoolPref:@"hide_ads"] && [RYGUtils getBoolPref:@"hide_ads_stories"];
    BOOL hideHighlights = [RYGUtils getBoolPref:@"hide_story_highlights"];
    ...
    for (id obj in items) {
        ...
        if ([obj isKindOfClass:%c(IGStoryTrayViewModel)]) {
            ...
            if (!hide && hideHighlights) {
                hide = [type isEqual:@(11)] || [rygTrayDiffID(obj) hasPrefix:@"highlightRewind:"];
            }
        }
...
- (void)storyTrayControllerShowSUPOGEducationBump {
    if (![RYGUtils getBoolPref:@"no_suggested_users"]) %orig;
}
%end
