...
+ (instancetype)requestFromJSONDict:(NSDictionary *)d {
	...
	r.isPrivate = [RYGJSONScalar(d[@"isPrivate"]) boolValue];
	r.type = (RYGFollowRequestType)[RYGJSONScalar(d[@"type"]) integerValue];
	r.sentAt = [RYGJSONScalar(d[@"sentAt"]) doubleValue];
	r.resolvedAt = [RYGJSONScalar(d[@"resolvedAt"]) doubleValue];
	return r;
}
