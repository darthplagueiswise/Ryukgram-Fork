...
static NSMutableArray *readArray(NSString *path) {
	...
	for (id entry in RYGJSONArray([NSJSONSerialization JSONObjectWithData:d options:0 error:nil]) ?: @[])
		if (RYGJSONDict(entry)) [out addObject:entry];
	return out;
}
...
+ (BOOL)restoreIncomingToReceivedForTargetPK:(NSString *)targetPK ownerPK:(NSString *)ownerPK {
	...
	ioSync(^{
		...
		for (NSUInteger i = 0; i < arr.count; i++) {
			...
			if (![RYGJSONString(d[@"userPK"]) isEqualToString:targetPK]) continue;
			RYGFollowRequestType t = [RYGJSONScalar(d[@"type"]) integerValue];
			...
			NSTimeInterval ts = MAX([RYGJSONScalar(d[@"resolvedAt"]) doubleValue], [RYGJSONScalar(d[@"sentAt"]) doubleValue]);
...
+ (RYGFollowRequest *)resolveTargetPK:(NSString *)targetPK fromType:(RYGFollowRequestType)fromType toType:(RYGFollowRequestType)toType ownerPK:(NSString *)ownerPK {
	...
	ioSync(^{
		...
		for (NSUInteger i = 0; i < arr.count; i++) {
			NSDictionary *d = arr[i];
			if (![RYGJSONString(d[@"userPK"]) isEqualToString:targetPK]) continue;
			if ([RYGJSONScalar(d[@"type"]) integerValue] != fromType) continue;
			NSTimeInterval ts = [RYGJSONScalar(d[@"sentAt"]) doubleValue];
			if (ts >= bestTs) { bestTs = ts; best = (NSInteger)i; }
		}
		if (best < 0) return;
		NSMutableDictionary *d = [arr[best] mutableCopy];
...
+ (BOOL)incomingSeededForOwnerPK:(NSString *)ownerPK {
	...
	ioSync(^{ seeded = [RYGJSONScalar(readDict(stateFileFor(ownerPK))[@"incomingSeeded"]) boolValue]; });
	return seeded;
}
...
+ (void)deleteRecordID:(NSString *)recordID ownerPK:(NSString *)ownerPK {
	...
	ioSync(^{
		...
		for (NSDictionary *d in readArray(path)) if (![RYGJSONString(d[@"recordID"]) isEqualToString:recordID]) [keep addObject:d];
...
+ (void)mergeImportedStoreAtPath:(NSString *)importedDir {
	ioSync(^{
		...
		for (NSString *f in [NSFileManager.defaultManager contentsOfDirectoryAtPath:importedDir error:nil]) {
			...
			for (NSDictionary *d in mine) { NSString *rid = RYGJSONString(d[@"recordID"]); if (rid) [ids addObject:rid]; }
			for (NSDictionary *d in readArray([importedDir stringByAppendingPathComponent:f])) {
				NSString *rid = RYGJSONString(d[@"recordID"]);
				if (rid && ![ids containsObject:rid]) { [mine addObject:d]; [ids addObject:rid]; }
			}
			[mine sortUsingComparator:^NSComparisonResult(NSDictionary *a, NSDictionary *b) {
				double da = MAX([RYGJSONScalar(a[@"resolvedAt"]) doubleValue], [RYGJSONScalar(a[@"sentAt"]) doubleValue]);
				double db = MAX([RYGJSONScalar(b[@"resolvedAt"]) doubleValue], [RYGJSONScalar(b[@"sentAt"]) doubleValue]);
				return da < db ? NSOrderedDescending : (da > db ? NSOrderedAscending : NSOrderedSame);
			}];
