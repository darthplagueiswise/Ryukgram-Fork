...
NSNotificationName const SCIFollowRequestsDidChangeNotification = @"SCIFollowRequestsDidChangeNotification";
...
@implementation SCIFollowRequestStorage
...
static void post(void) {
	dispatch_async(dispatch_get_main_queue(), ^{
		[NSNotificationCenter.defaultCenter postNotificationName:SCIFollowRequestsDidChangeNotification object:nil];
	});
}
...
+ (NSArray<SCIFollowRequest *> *)allForOwnerPK:(NSString *)ownerPK {
	...
	ioSync(^{
		for (NSDictionary *d in readArray(fileFor(ownerPK))) {
			SCIFollowRequest *r = [SCIFollowRequest requestFromJSONDict:d];
...
+ (NSArray<NSString *> *)pendingTargetPKsForOwnerPK:(NSString *)ownerPK {
	...
	for (SCIFollowRequest *r in [self allForOwnerPK:ownerPK])
		if (r.type == SCIFollowRequestTypeSent && r.userPK.length && ![pks containsObject:r.userPK])
			[pks addObject:r.userPK];
	return pks;
}

+ (SCIFollowRequest *)latestPendingForTargetPK:(NSString *)targetPK ownerPK:(NSString *)ownerPK {
	for (SCIFollowRequest *r in [self allForOwnerPK:ownerPK]) // already newest-first
		if (r.type == SCIFollowRequestTypeSent && [r.userPK isEqualToString:targetPK]) return r;
	return nil;
}
...
+ (SCIFollowRequest *)resolveTargetPK:(NSString *)targetPK fromType:(SCIFollowRequestType)fromType toType:(SCIFollowRequestType)toType ownerPK:(NSString *)ownerPK {
	__block SCIFollowRequest *updated = nil;
	ioSync(^{
		NSString *path = fileFor(ownerPK);
		NSMutableArray *arr = readArray(path);
		...
		arr[best] = d;
		writeArray(arr, path);
		updated = [SCIFollowRequest requestFromJSONDict:d];
	});
	if (updated) post();
	return updated;
}
...
+ (NSUInteger)unreadCountForOwnerPK:(NSString *)ownerPK {
	...
	NSUInteger n = 0;
	for (SCIFollowRequest *r in [self allForOwnerPK:ownerPK]) if (r.sortDate > lastSeen) n++;
	return n;
}
