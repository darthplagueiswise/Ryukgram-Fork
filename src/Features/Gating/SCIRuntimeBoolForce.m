...
static BOOL sciRuntimeBoolForceYES(__unused id self, __unused SEL _cmd) { return YES; }
static BOOL sciRuntimeBoolForceNO(__unused id self, __unused SEL _cmd) { return NO; }
...
+ (BOOL)forceClassNamed:(NSString *)className
    ...
    // Constant C IMP: no Objective-C work in the replacement, so it cannot
    // recurse through MobileConfig. MSHookMessageEx keeps this aligned with the
    // rest of the tweak's runtime hook style.
    IMP imp = (IMP)(value ? sciRuntimeBoolForceYES : sciRuntimeBoolForceNO);
    IMP orig = NULL;
    MSHookMessageEx(target, sel, imp, &orig);
