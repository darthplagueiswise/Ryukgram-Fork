...
// An already-swapped link has no instagram.com host left, so match the embed domain too.
static BOOL rygLooksLikeIGURL(NSString *s) {
    ...
    NSString *custom = [RYGUtils getStringPref:@"embed_link_domain"];
    if (!custom.length) custom = @"kkinstagram.com";
    return [s containsString:custom];
}
...
static NSString *rygRewriteIGURL(NSString *url) {
    ...
    if ([RYGUtils getBoolPref:@"strip_tracking_params"])
        url = [RYGUtils stripTrackingParams:url];
...
static id rygRewriteValue(id v) {
    ...
    if ([v isKindOfClass:[NSURL class]]) {
        ...
        NSURL *u = [NSURL URLWithString:r];
        if (!u) {
            NSString *esc = [r stringByAddingPercentEncodingWithAllowedCharacters:NSCharacterSet.URLQueryAllowedCharacterSet];
            u = esc ? [NSURL URLWithString:esc] : nil;
        }
        return u;
    }
...
#define HOOK_IF(cls, sel, newFn, origPtr) do { \
    SEL _s = (sel); \
    if ((cls) && class_getInstanceMethod((cls), _s)) MSHookMessageEx((cls), _s, (IMP)(newFn), (IMP *)(origPtr)); \
} while (0)
