...
static NSArray<NSString *> *sciIGDomains(void) {
    static NSArray *d;
...
static BOOL sciLooksLikeIGURL(NSString *s) {
    if (s.length < 12 || s.length > 4096) return NO;
    for (NSString *d in sciIGDomains()) {
...
static id sciRewriteValue(id v) {
    if ([v isKindOfClass:[NSString class]]) {
        NSString *s = v;
        if (!sciLooksLikeIGURL(s)) return nil;
        NSString *r = sciRewriteIGURL(s);
...
static NSArray *sciRewriteItemArray(NSArray *items) {
    ...
    for (NSUInteger i = 0; i < items.count; i++) {
        ...
        for (id key in (NSDictionary *)entry) {
            id r = sciRewriteValue(entry[key]);
            if (!r) continue;
...
static NSArray *sciRewriteObjectArray(NSArray *objs) {
    if (!objs.count) return objs;
    NSMutableArray *out = nil;
    for (NSUInteger i = 0; i < objs.count; i++) {
        id r = sciRewriteValue(objs[i]);
        if (!r) continue;
...
static void new_setString(UIPasteboard *self, SEL _cmd, NSString *value) {
    if (sciShouldRewrite() && [value isKindOfClass:[NSString class]] && sciLooksLikeIGURL(value)) {
        value = sciRewriteIGURL(value);
    }
...
static void new_setURL(UIPasteboard *self, SEL _cmd, NSURL *value) {
    if (sciShouldRewrite() && [value isKindOfClass:[NSURL class]]) {
        id r = sciRewriteValue(value);
        if (r) value = r;
    }
...
static void new_setStrings(UIPasteboard *self, SEL _cmd, NSArray *strings) {
    if (sciShouldRewrite()) strings = sciRewriteObjectArray(strings);
...
static void new_setURLs(UIPasteboard *self, SEL _cmd, NSArray *urls) {
    if (sciShouldRewrite()) urls = sciRewriteObjectArray(urls);
...
static void new_setObjects(UIPasteboard *self, SEL _cmd, NSArray *objs) {
    if (sciShouldRewrite()) objs = sciRewriteObjectArray(objs);
...
static void new_setItems(UIPasteboard *self, SEL _cmd, NSArray *items) {
    if (sciShouldRewrite()) items = sciRewriteItemArray(items);
...
static void new_setDataForType(UIPasteboard *self, SEL _cmd, NSData *data, NSString *type) {
    if (sciShouldRewrite() && [data isKindOfClass:[NSData class]] && data.length && data.length < 8192
        ...
        if (s.length && sciLooksLikeIGURL(s)) {
            NSString *r = sciRewriteIGURL(s);
...
static void new_setLoadedURL(id self, SEL _cmd, NSURL *url) {
    if (sciShouldRewrite() && [url isKindOfClass:[NSURL class]]) {
        id r = sciRewriteValue(url);
        if (r) url = r;
    }
...
static NSArray *sciRewriteActivityItems(NSArray *items) {
    if (!sciShouldRewrite() || !items.count) return items;
    NSMutableArray *out = nil;
    for (NSUInteger i = 0; i < items.count; i++) {
        id r = sciRewriteValue(items[i]);
        if (!r) continue;
        if (!out) out = [items mutableCopy];
        out[i] = r;
    }
...
static id new_avcInit(id self, SEL _cmd, NSArray *items, NSArray *apps) {
    return orig_avcInit(self, _cmd, sciRewriteActivityItems(items), apps);
}


static void (*orig_gtvSetText)(id, SEL, NSString *);
static void new_gtvSetText(id self, SEL _cmd, NSString *text) {
    if (sciShouldRewrite() && [text isKindOfClass:[NSString class]] && sciLooksLikeIGURL(text)) {
        NSString *r = sciRewriteIGURL(text);
...
static void new_gtvSetAttrText(id self, SEL _cmd, NSAttributedString *text) {
    if (sciShouldRewrite() && [text isKindOfClass:[NSAttributedString class]]) {
        id r = sciRewriteValue(text);
        if (r) text = r;
    }
