...
static NSString *RYGHighResUserAgentString(NSString *userAgent) {
...
static NSString *RYGHighResHeaderValue(NSString *value, NSString *field) {
	if (![field isKindOfClass:NSString.class] || [field caseInsensitiveCompare:@"User-Agent"] != NSOrderedSame) return value;
	return RYGHighResUserAgentString(value);
}

%group RYGEnhancedMediaResolutionGroup

%hook NSMutableURLRequest

- (void)setValue:(NSString *)value forHTTPHeaderField:(NSString *)field {
	%orig(RYGHighResHeaderValue(value, field), field);
}
...
- (void)setAllHTTPHeaderFields:(NSDictionary *)headerFields {
	...
	for (NSString *key in headerFields) {
		...
		if ([value isKindOfClass:NSString.class]) {
			headers[key] = RYGHighResUserAgentString(value);
		}
		break;
	}
...
%ctor {
	if ([RYGUtils getBoolPref:@"enhanced_media_resolution"]) {
		%init(RYGEnhancedMediaResolutionGroup);
	}
}
