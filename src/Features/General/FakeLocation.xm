...
static BOOL RYGFakeLocationEnabled(void) {
	return [RYGUtils getBoolPref:@"fake_location_enabled"];
}

static CLLocationCoordinate2D RYGFakeCoordinate(void) {
...
static CLLocation *RYGFakeLocation(void) {
	CLLocationCoordinate2D coord = RYGFakeCoordinate();
...
static void RYGFeedFakeLocation(CLLocationManager *manager) {
	if (!manager || !RYGFakeLocationEnabled()) return;

	id<CLLocationManagerDelegate> delegate = manager.delegate;
	if (!delegate) return;

	CLLocation *location = RYGFakeLocation();
...
- (CLLocation *)location {
	return RYGFakeLocationEnabled() ? RYGFakeLocation() : %orig;
}

- (void)startUpdatingLocation {
	%orig;
	if (RYGFakeLocationEnabled()) RYGFeedFakeLocation(self);
}
