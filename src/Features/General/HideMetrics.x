...
static inline BOOL sciHide(void) {
    if (sHideStale) { sHide = [SCIUtils getBoolPref:@"hide_metrics"]; sHideStale = NO; }
    return sHide;
}

static inline BOOL sciIsCountLabel(UIView *label) {
...
static void new_setText(id self, SEL _cmd, id text) {
    if (sciHide() && sciIsCountLabel(self)) text = @"";
...
- (void)setCountString:(id)string showButton:(BOOL)showButton {
    return %orig(sciHide() ? @"" : string, showButton);
}
%end
