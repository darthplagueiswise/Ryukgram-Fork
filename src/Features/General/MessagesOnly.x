...
BOOL RYGMessagesOnlyHooksActive(void) { return sRYGMsgOnlyHooksActive; }
...
// Order lives in _tabBarSurfaces, not in the rebuilt _buttons array. Buttons with
// no shown surface are dropped, not appended back.
static NSArray *rygOrderedButtons(UIViewController *ctrl) {
    NSArray *ordered = [RYGTabBar orderedVisibleButtonsForController:ctrl];
    if (!ordered.count) return [RYGUtils getIvarForObj:ctrl name:"_buttons"];
...
static void rygSelectSurfaceIfStripped(UIViewController *ctrl) {
    ...
    NSString *cur = [RYGTabBar tabKeyForSurface:current];
    ...
    for (id s in surfaces) {
        if (![[RYGTabBar tabKeyForSurface:s] isEqualToString:RYGTabKeyMessages]) continue;
...
BOOL RYGMessagesOnlyApplyLive(void) {
    ...
    UIViewController *ctrl = [RYGTabBar liveController];
