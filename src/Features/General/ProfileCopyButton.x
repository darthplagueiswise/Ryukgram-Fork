...
static void sci_copyAndToast(NSString *value, NSString *label) {
	...
	// Native lightweight success HUD (no third-party dependency, so nothing extra
	// needs to be linked — JGProgressHUD is not bundled in this tweak).
	...
	text.text = [NSString stringWithFormat:@"Copied %@", label];
	text.textColor = UIColor.whiteColor;
	text.font = [UIFont systemFontOfSize:15.0 weight:UIFontWeightSemibold];
	text.textAlignment = NSTextAlignmentCenter;
	text.translatesAutoresizingMaskIntoConstraints = NO;

	[hud addSubview:check];
	[hud addSubview:text];
	[host addSubview:hud];
	...
	[NSLayoutConstraint activateConstraints:@[
		...
		[hud.widthAnchor constraintGreaterThanOrEqualToConstant:140.0],
		[check.topAnchor constraintEqualToAnchor:hud.topAnchor constant:20.0],
		[check.centerXAnchor constraintEqualToAnchor:hud.centerXAnchor],
		[check.widthAnchor constraintEqualToConstant:40.0],
		[check.heightAnchor constraintEqualToConstant:40.0],
		[text.topAnchor constraintEqualToAnchor:check.bottomAnchor constant:12.0],
		[text.leadingAnchor constraintEqualToAnchor:hud.leadingAnchor constant:18.0],
		[text.trailingAnchor constraintEqualToAnchor:hud.trailingAnchor constant:-18.0],
		[text.bottomAnchor constraintEqualToAnchor:hud.bottomAnchor constant:-18.0],
	]];

	[UIView animateWithDuration:0.2 animations:^{ hud.alpha = 1.0; } completion:^(__unused BOOL done) {
		dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(1.5 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
			[UIView animateWithDuration:0.25 animations:^{ hud.alpha = 0.0; } completion:^(__unused BOOL d2) {
