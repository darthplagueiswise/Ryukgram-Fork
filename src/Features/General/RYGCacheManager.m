...
static void rygDeleteContents(NSString *path, BOOL preserveMessages) {
...
+ (void)_clearAuto:(BOOL)isAuto completion:(void(^)(uint64_t))completion {
	...
	dispatch_async(rygQueue(), ^{
		uint64_t reclaimed = atomic_load(&gCachedSize) ?: rygTotalSize();
		...
		for (NSString *dir in rygCacheDirs()) rygDeleteContents(dir, preserve);
