#import "RYGChangelog.h"
#import "../../Utils.h"
#import "../../Tweak.h"

#define kRepo RYGRepoSlug

// Stores the RYGVersionString of the last tweak build whose popup was shown.
// When the tweak updates, this mismatches and triggers a fresh check.
static NSString *const kLastSeenVersionKey = @"ryg_changelog_last_seen_version";
// Debug pref: when YES, the popup fires every launch regardless of version.
static NSString *const kForceShowKey = @"ryg_changelog_force_show";

// MARK: - Cache

static NSString *rygChangelogCacheDir(void) {
	static NSString *dir;
...
static NSString *rygCachedReleasePath(NSString *tag) {
	NSString *safe = [tag stringByReplacingOccurrencesOfString:@"/" withString:@"_"];
	return [rygChangelogCacheDir() stringByAppendingPathComponent:[safe stringByAppendingPathExtension:@"json"]];
}

static NSDictionary *rygLoadCachedRelease(NSString *tag) {
	NSData *data = [NSData dataWithContentsOfFile:rygCachedReleasePath(tag)];
	id json = data ? [NSJSONSerialization JSONObjectWithData:data options:0 error:nil] : nil;
	return [json isKindOfClass:NSDictionary.class] ? json : nil;
}

static void rygSaveCachedRelease(NSDictionary *json) {
	NSString *tag = json[@"tag_name"];
	if (!tag.length) return;

	NSData *data = [NSJSONSerialization dataWithJSONObject:json options:0 error:nil];
	if (data) [data writeToFile:rygCachedReleasePath(tag) atomically:YES];
}

// MARK: - Network

static void rygFetchJSON(NSString *url, void (^completion)(id json)) {
...
static NSRegularExpression *rygRegex(NSString *pattern) {
	return [NSRegularExpression regularExpressionWithPattern:pattern options:0 error:nil];
}

static NSAttributedString *rygRenderInline(NSString *line, NSDictionary *attrs) {
	NSMutableAttributedString *seg = [[NSMutableAttributedString alloc] initWithString:line attributes:attrs];

	for (NSTextCheckingResult *m in [rygRegex(@"\\*\\*(.+?)\\*\\*") matchesInString:seg.string options:0 range:NSMakeRange(0, seg.length)].reverseObjectEnumerator) {
...
static NSAttributedString *rygRenderMarkdown(NSString *md) {
	...
	for (NSString *raw in [md componentsSeparatedByString:@"\n"]) {
		...
		[out appendAttributedString:rygRenderInline(line, attrs)];
		emitted = YES;
	}
...
@interface _RYGChangelogDetailVC : UIViewController
@property (nonatomic, copy) NSDictionary *releaseJSON;
@property (nonatomic, copy) void (^onDismiss)(void);
@end

@implementation _RYGChangelogDetailVC

- (void)viewDidLoad {
	[super viewDidLoad];

	self.title = RYGLocalized(@"What's new in RyukGram");
	...
	tv.attributedText = rygRenderMarkdown([header stringByAppendingString:body]);
...
@interface _RYGReleaseListVC : UITableViewController
@property (nonatomic, copy) NSArray<NSDictionary *> *releases;
@property (nonatomic, strong) UIActivityIndicatorView *spinner;
@end

@implementation _RYGReleaseListVC

- (void)viewDidLoad {
	[super viewDidLoad];

	self.title = RYGLocalized(@"Release notes");
...
- (void)loadReleases {
	rygFetchReleaseList(^(NSArray<NSDictionary *> *arr) {
		self.releases = arr ?: @[];
		...
		if (!self.releases.count) {
			...
			empty.text = RYGLocalized(@"No releases");
...
- (UITableViewCell *)tableView:(UITableView *)tv cellForRowAtIndexPath:(NSIndexPath *)ip {
	...
	if (ip.row == 0) [badges addObject:RYGLocalized(@"latest")];
	if ([tag isEqualToString:RYGVersionString]) [badges addObject:RYGLocalized(@"installed")];
...
- (void)tableView:(UITableView *)tv didSelectRowAtIndexPath:(NSIndexPath *)ip {
	...
	_RYGChangelogDetailVC *vc = [_RYGChangelogDetailVC new];
...
+ (void)presentIfNewFromWindow:(UIWindow *)window {
	...
	if (!force && [[ud stringForKey:kLastSeenVersionKey] isEqualToString:RYGVersionString]) return;

	void (^show)(NSDictionary *) = ^(NSDictionary *json) {
		if (!json) return;

		// Mark seen on show so any dismissal path (Done, swipe) is covered.
		[ud setObject:RYGVersionString forKey:kLastSeenVersionKey];
		[self presentReleaseJSON:json onDismiss:nil fromWindow:window];
	};

	NSDictionary *cached = rygLoadCachedRelease(RYGVersionString);
	if (cached) show(cached);
	else rygFetchRelease(RYGVersionString, show);
}

+ (void)presentAllFromViewController:(UIViewController *)host {
	if (!host) return;

	_RYGReleaseListVC *list = [_RYGReleaseListVC new];
