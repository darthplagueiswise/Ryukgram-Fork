...
- (void)picker:(PHPickerViewController *)picker didFinishPicking:(NSArray<PHPickerResult *> *)results {
	[picker dismissViewControllerAnimated:YES completion:^{
		...
		if (![result.itemProvider canLoadObjectOfClass:UIImage.class]) return;

		[result.itemProvider loadObjectOfClass:UIImage.class completionHandler:^(__kindof id<NSItemProviderReading> obj, NSError *error) {
			UIImage *image = [obj isKindOfClass:UIImage.class] ? (UIImage *)obj : nil;
			if (!image || error) return;

			dispatch_async(dispatch_get_main_queue(), ^{
				[self presentCropForImage:image];
			});
		}];
	}];
}
...
- (void)layoutSubviews {
	...
	UIButton *button = objc_getAssociatedObject(view, kSCIInstantsGalleryButtonKey);
	...
	if (!button) {
		...
		UIImage *image = [UIImage systemImageNamed:@"photo.on.rectangle.angled" withConfiguration:cfg];
		...
		[button addTarget:self action:@selector(sci_instantsGalleryTapped:) forControlEvents:UIControlEventTouchUpInside];
	...
	CGFloat side = 52.0;
	CGFloat x = safe.left + 76.0;
	CGFloat y = size.height - safe.bottom - 122.0;

	UIView *audience = nil;
	for (UIView *subview in view.subviews) {
		if (subview == button || subview.hidden || subview.alpha < 0.05) continue;

		NSString *cls = NSStringFromClass(subview.class);
		if ([cls containsString:@"Audience"] || [cls containsString:@"Friend"]) {
			audience = subview;
			break;
		}
	}
	...
	if (audience) {
		CGRect frame = [view convertRect:audience.bounds fromView:audience];
		x = CGRectGetMinX(frame) - side - 22.0;
		y = CGRectGetMidY(frame) - side * 0.5;
	}

	button.frame = CGRectMake(MAX(safe.left + 16.0, x), y, side, side);
