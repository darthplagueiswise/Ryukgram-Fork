...
- (void)sci_changed {
	...
	if ([btn isKindOfClass:SCIChromeButton.class])
		((SCIChromeButton *)btn).iconTint = sciActive(self) ? UIColor.systemBlueColor : UIColor.labelColor;
...
- (void)sci_installSortButton {
	...
	btn.iconTint = sciActive(self) ? UIColor.systemBlueColor : UIColor.labelColor;
	btn.layer.shadowColor = UIColor.blackColor.CGColor;
	btn.layer.shadowOpacity = 0.18;
	btn.layer.shadowRadius = 8;
	btn.layer.shadowOffset = CGSizeMake(0, 2);
	btn.layer.shadowPath = [UIBezierPath bezierPathWithRoundedRect:CGRectMake(0, 0, 46, 46) cornerRadius:23].CGPath;
