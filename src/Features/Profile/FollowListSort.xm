...
static NSNumber *rygRelFlag(id rel, SEL getter, NSString *dictKey) {
	if (rel && rel != (id)[NSNull null]) {
...
static RYGUserAttr rygAttrForConfig(id cfg, NSString **outName, NSString **outFull) {
	...
	id rel = rygFieldCache(user, @"friendship_status");
	if (!rel || rel == (id)[NSNull null]) { @try { rel = [user valueForKey:@"friendshipStatus"]; } @catch (__unused id e) {} }
	NSNumber *fo = rygRelFlag(rel, @selector(following),  @"following");
	NSNumber *fb = rygRelFlag(rel, @selector(followedBy), @"followed_by");
	if (fo || fb) { a.relKnown = YES; a.following = fo.boolValue; a.followedBy = fb.boolValue; }
