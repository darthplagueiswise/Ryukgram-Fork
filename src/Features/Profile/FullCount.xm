...
static id sciIvarLookup(id obj, const char *name) {
...
static void sciSetText(IGStatButton *button, NSNumber *count) {
	...
	UILabel *label = (UILabel *)sciIvarLookup(button, "_countLabel");
...
- (void)layoutSubviews {
	...
	if (followers) {
		id btn = sciIvarLookup(self, "$__lazy_storage_$_followersButton");
		sciSetText((IGStatButton *)btn, sciProfileCount(user, cache, NO));
	}

	if (posts) {
		id btn = sciIvarLookup(self, "postCountButton");
		sciSetText((IGStatButton *)btn, sciProfileCount(user, cache, YES));
	}
}
%end
