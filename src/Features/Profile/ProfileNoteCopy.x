...
- (void)layoutSubviews {
    %orig;
    if (![RYGUtils getBoolPref:@"profile_note_copy"]) return;
    ...
    UILongPressGestureRecognizer *lp = [[UILongPressGestureRecognizer alloc]
        initWithTarget:self action:@selector(rygCopyNoteLongPress:)];
    lp.minimumPressDuration = 0.5;
    [view addGestureRecognizer:lp];
}

%new - (void)rygCopyNoteLongPress:(UILongPressGestureRecognizer *)gesture {
    ...
    RYGNotifySuccess(RYG_NOTIF_COPY_NOTE, RYGLocalized(@"Note copied"), nil);
}
