...
//
// Posts grid likes/plays are NSNull until tapped — refetch via batched
...
static UIView *sciEnsureRow(UIView *cell, NSInteger tag, NSString *iconName) {
	...
	row.backgroundColor = [UIColor colorWithWhite:0.0 alpha:0.5];
...
static void sciEnsureViewsBackdrop(UIView *target) {
	...
	if (!backdrop) {
		...
		backdrop.tag = kSCIViewsBackdropTag;
		backdrop.userInteractionEnabled = NO;
		backdrop.backgroundColor = [UIColor colorWithWhite:0.0 alpha:0.5];
...
static void new_gridCell_layoutSubviews(id self, SEL _cmd) {
	...
	id media     = sciObjIvar(self, "_mediaAccess");
	...
	if (sciShowLikes()) {
		...
		sciPositionRow(row, sciCount(likes), x, viewsVisible ? yAboveBottom : bottomSlotY);
	} else {
		UIView *r = [cellView viewWithTag:kSCILikesRowTag]; if (r) [r removeFromSuperview];
	}

	if (sciShowDate()) {
		NSDate *d = sciMediaTakenAt(media);
		NSString *txt = sciDateText(d) ?: @"-";
...
static void sciKickCellsForPK(NSString *pk) {
	if (!pk.length) return;
	Class thumbCls = NSClassFromString(@"IGMediaThumbnailCell");
	if (!thumbCls) return;
	for (UIScene *scene in [UIApplication sharedApplication].connectedScenes) {
		...
		for (UIWindow *win in ((UIWindowScene *)scene).windows) {
			...
			while (stack.count) {
				...
				if ([v isKindOfClass:thumbCls]) {
					NSString *cellPK = objc_getAssociatedObject(v, kSCIThumbCellPKKey);
					if ([cellPK isEqualToString:pk]) [v setNeedsLayout];
				}
...
static void sciDropOffscreenPendingPKs(void) {
	...
	for (UIScene *scene in [UIApplication sharedApplication].connectedScenes) {
		...
		for (UIWindow *win in ((UIWindowScene *)scene).windows) {
			...
			while (stack.count) {
				UIView *v = stack.lastObject; [stack removeLastObject];
				if ([v isKindOfClass:thumbCls] && v.window) {
					NSString *pk = objc_getAssociatedObject(v, kSCIThumbCellPKKey);
					if (pk.length) [visiblePKs addObject:pk];
				}
				for (UIView *sv in v.subviews) [stack addObject:sv];
			}
		}
