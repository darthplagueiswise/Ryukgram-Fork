...
// KVC returns NSNull for friendship_status, so the Pando dict is read directly.
static id rygPHFieldCache(id obj, NSString *key) {
	if (!obj) return nil;
	Ivar iv = NULL;
...
static NSNumber *rygPHRelFlag(id rel, SEL getter, NSString *dictKey) {
	if (!rel || rel == (id)[NSNull null]) return nil;
	if ([rel respondsToSelector:getter]) {
		id v = ((id (*)(id, SEL))objc_msgSend)(rel, getter);
		if ([v respondsToSelector:@selector(boolValue)]) return @([v boolValue]);
	}
	if ([rel isKindOfClass:NSDictionary.class]) {
		id v = ((NSDictionary *)rel)[dictKey];
		if ([v respondsToSelector:@selector(boolValue)]) return @([v boolValue]);
	}
	return nil;
}
...
+ (BOOL)relationForUser:(id)user following:(BOOL *)following followedBy:(BOOL *)followedBy {
	if (!user) return NO;
	id rel = rygPHFieldCache(user, @"friendship_status");
	if (!rel) { @try { rel = [user valueForKey:@"friendshipStatus"]; } @catch (__unused id e) {} }
	NSNumber *fo = rygPHRelFlag(rel, @selector(following), @"following");
	NSNumber *fb = rygPHRelFlag(rel, @selector(followedBy), @"followed_by");
	if (!fo && !fb) return NO;
	if (following) *following = fo.boolValue;
	if (followedBy) *followedBy = fb.boolValue;
	return YES;
}
