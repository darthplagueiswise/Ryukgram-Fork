...
+ (void)sharePictureForUser:(id)user {
    ...
    [self resolveHDPictureURLForUser:user completion:^(NSURL *url) {
        ...
        SCIDownloadDelegate *dl = [[SCIDownloadDelegate alloc] initWithAction:share showProgress:YES];
        dl.pendingGallerySaveMetadata = sciProfileGalleryMetadata(user);
...
+ (void)savePictureToGalleryForUser:(id)user {
    ...
    [self resolveHDPictureURLForUser:user completion:^(NSURL *url) {
        ...
        NSString *ext = [[url pathExtension] lowercaseString];
        if (!ext.length) ext = @"jpg";
        SCIDownloadDelegate *dl = [[SCIDownloadDelegate alloc] initWithAction:saveToGallery showProgress:YES];
        dl.pendingGallerySaveMetadata = sciProfileGalleryMetadata(user);
        [dl downloadFileWithURL:url fileExtension:ext hudLabel:nil];
    }];
}
