...
+ (instancetype)userFromAPIDict:(NSDictionary *)d {
	...
	u.isPrivate = [RYGJSONScalar(d[@"is_private"]) boolValue];
	u.isVerified = [RYGJSONScalar(d[@"is_verified"]) boolValue];
	return u;
}
...
+ (instancetype)userFromJSONDict:(NSDictionary *)d {
	NSString *pk = RYGJSONString(d[@"pk"]);
	if (!pk) return nil;
	RYGProfileAnalyzerUser *u = [self new];
	u.pk = pk;
	u.username = RYGJSONString(d[@"username"]) ?: @"";
	u.fullName = RYGJSONString(d[@"full_name"]);
	u.profilePicURL = RYGJSONString(d[@"profile_pic_url"]);
	u.profilePicID = RYGJSONString(d[@"profile_pic_id"]);
	u.isPrivate = [RYGJSONScalar(d[@"is_private"]) boolValue];
	u.isVerified = [RYGJSONScalar(d[@"is_verified"]) boolValue];
	return u;
}
...
+ (instancetype)visitFromJSONDict:(NSDictionary *)d {
	...
	if (!u) return nil;
	double first = [RYGJSONScalar(d[@"first_seen"]) doubleValue];
	double last  = [RYGJSONScalar(d[@"last_seen"]) doubleValue];
	...
	v.visitCount = MAX(1, [RYGJSONScalar(d[@"visit_count"]) integerValue]);
	return v;
}
...
+ (instancetype)snapshotFromJSONDict:(NSDictionary *)d {
	NSString *selfPK = RYGJSONString(d[@"self_pk"]);
	if (!selfPK) return nil;
	RYGProfileAnalyzerSnapshot *s = [self new];
	s.scanDate = [NSDate dateWithTimeIntervalSince1970:[RYGJSONScalar(d[@"scan_date"]) doubleValue]];
	s.selfPK = selfPK;
	s.selfUsername = RYGJSONString(d[@"self_username"]);
	s.selfFullName = RYGJSONString(d[@"self_full_name"]);
	s.selfProfilePicURL = RYGJSONString(d[@"self_profile_pic_url"]);
	s.followerCount = [RYGJSONScalar(d[@"follower_count"]) integerValue];
	s.followingCount = [RYGJSONScalar(d[@"following_count"]) integerValue];
	s.mediaCount = [RYGJSONScalar(d[@"media_count"]) integerValue];
...
// profile_pic_id is stable per upload; the URL is not, IG rotates the CDN host.
...
+ (RYGProfileAnalyzerReport *)reportFromCurrent:(RYGProfileAnalyzerSnapshot *)current
	...
	// Disabled checks aren't computed, so their arrays stay empty.
