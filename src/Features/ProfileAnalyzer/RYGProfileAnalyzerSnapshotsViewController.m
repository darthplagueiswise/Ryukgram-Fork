...
static const unsigned long long kRYGSnapWarnBytes = 50ULL * 1024 * 1024;

@interface RYGProfileAnalyzerSnapshotsViewController () <UITableViewDataSource, UITableViewDelegate>
@property (nonatomic, copy) NSString *userPK;
@property (nonatomic, strong) UITableView *tableView;
@property (nonatomic, copy) NSArray<RYGProfileAnalyzerSnapshotMeta *> *snapshots;
@property (nonatomic, copy, nullable) NSString *selection;
@property (nonatomic, assign) unsigned long long totalBytes;
@end

@implementation RYGProfileAnalyzerSnapshotsViewController
...
- (void)viewDidLoad {
	[super viewDidLoad];
	self.title = RYGLocalized(@"Snapshots");
...
- (void)reloadFromStore {
	self.snapshots = [RYGProfileAnalyzerStorage snapshotHistoryForUserPK:self.userPK];
	self.selection = [RYGProfileAnalyzerStorage compareSelectionForUserPK:self.userPK];
	self.totalBytes = [RYGProfileAnalyzerStorage historyByteSizeForUserPK:self.userPK];
...
- (void)refreshBarButtons {
	if (self.tableView.isEditing) {
		...
		UIBarButtonItem *selectAll = [[UIBarButtonItem alloc] initWithTitle:(allSelected ? RYGLocalized(@"Deselect All") : RYGLocalized(@"Select All"))
		...
		UIBarButtonItem *select = [[UIBarButtonItem alloc] initWithTitle:RYGLocalized(@"Select")
...
- (void)deleteSelected {
	...
	NSString *msg = ids.count == 1
		? RYGLocalized(@"Delete this snapshot? This can't be undone.")
		: [NSString stringWithFormat:RYGLocalized(@"Delete %lu snapshots? This can't be undone."), (unsigned long)ids.count];
	UIAlertController *a = [UIAlertController alertControllerWithTitle:RYGLocalized(@"Delete snapshots") message:msg preferredStyle:UIAlertControllerStyleAlert];
	...
	[a addAction:[UIAlertAction actionWithTitle:RYGLocalized(@"Delete") style:UIAlertActionStyleDestructive handler:^(UIAlertAction *_) {
		[RYGProfileAnalyzerStorage deleteHistorySnapshotIDs:ids forUserPK:self.userPK];
		[self exitEditing];
		[self reloadFromStore];
		[self refreshBarButtons];
	}]];
...
- (NSString *)tableView:(UITableView *)tv titleForHeaderInSection:(NSInteger)section {
	return section == 0 ? RYGLocalized(@"Recording") : RYGLocalized(@"Compare next scan against");
}
...
- (NSString *)tableView:(UITableView *)tv titleForFooterInSection:(NSInteger)section {
	if (section == 0) {
		...
		NSString *base = RYGLocalized(@"Each scan saves a full copy of your followers and following so you can compare against it later. Everything stays on this device.");
		NSString *stat = [NSString stringWithFormat:RYGLocalized(@"Using %@ across %lu snapshots."), usage, (unsigned long)self.snapshots.count];
		NSString *warn = self.totalBytes >= kRYGSnapWarnBytes ? [@"\n\n" stringByAppendingString:RYGLocalized(@"⚠️ This is getting large — lower the limit or delete older snapshots to free space.")] : @"";
		return [NSString stringWithFormat:@"%@\n\n%@%@", base, stat, warn];
	}
	return RYGLocalized(@"“Previous scan” always measures against your last run. Pick a saved snapshot to compare against a fixed point in time instead.");
}
...
- (UITableViewCell *)recordingCellForRow:(NSInteger)row tableView:(UITableView *)tv {
	...
	if (row == 0) {
		cell.textLabel.text = RYGLocalized(@"Record snapshots");
		cell.detailTextLabel.text = RYGLocalized(@"Save a dated entry on every scan");
		...
		sw.on = [RYGUtils getBoolPref:@"profile_analyzer_record_snapshots"];
		...
		cell.textLabel.text = RYGLocalized(@"Keep newest");
		cell.detailTextLabel.text = RYGLocalized(@"Older snapshots beyond the limit are removed automatically");
		...
		val.text = cap > 0 ? [NSString stringWithFormat:@"%ld", (long)cap] : RYGLocalized(@"Unlimited");
		[val sizeToFit];
...
- (UITableViewCell *)compareCellForRow:(NSInteger)row tableView:(UITableView *)tv {
	...
	BOOL selected;
	if (isPrevious) {
		cell.textLabel.text = RYGLocalized(@"Previous scan");
		cell.detailTextLabel.text = RYGLocalized(@"Rolling — always your last run");
		// nil selection (legacy / never chosen) defaults to the rolling diff.
		selected = !self.selection.length || [self.selection isEqualToString:RYGProfileAnalyzerCompareSelectionPrevious];
	} else {
		RYGProfileAnalyzerSnapshotMeta *m = self.snapshots[row - 1];
		cell.textLabel.text = [self dateLabelForMeta:m];
		cell.detailTextLabel.text = [NSString stringWithFormat:RYGLocalized(@"%@ followers · %@ following"),
									 [RYGUtils shortCount:m.followerCount], [RYGUtils shortCount:m.followingCount]];
...
- (NSString *)dateLabelForMeta:(RYGProfileAnalyzerSnapshotMeta *)m {
	...
	return m.scanDate ? [fmt stringFromDate:m.scanDate] : RYGLocalized(@"Snapshot");
}
...
- (UISwipeActionsConfiguration *)tableView:(UITableView *)tv trailingSwipeActionsConfigurationForRowAtIndexPath:(NSIndexPath *)ip {
	...
	UIContextualAction *del = [UIContextualAction contextualActionWithStyle:UIContextualActionStyleDestructive
		...
		[RYGProfileAnalyzerStorage deleteHistorySnapshotIDs:@[sid] forUserPK:self.userPK];
		[self reloadFromStore];
		[self refreshBarButtons];
		done(YES);
	}];
...
- (void)tableView:(UITableView *)tv didSelectRowAtIndexPath:(NSIndexPath *)ip {
	...
	NSString *newSel = (ip.row == 0)
		? RYGProfileAnalyzerCompareSelectionPrevious
		: self.snapshots[ip.row - 1].snapshotID;
	if ([newSel isEqualToString:self.selection]) return;
	[RYGProfileAnalyzerStorage setCompareSelection:newSel forUserPK:self.userPK];
...
- (void)archiveCurrentScanIfNew {
	RYGProfileAnalyzerSnapshot *cur = [RYGProfileAnalyzerStorage currentSnapshotForUserPK:self.userPK];
	if (!cur.scanDate) return;

	RYGProfileAnalyzerSnapshotMeta *newest = [RYGProfileAnalyzerStorage snapshotHistoryForUserPK:self.userPK].firstObject;
	if (newest && fabs([newest.scanDate timeIntervalSinceDate:cur.scanDate]) < 1.0) return;   // already saved

	NSInteger cap = (NSInteger)[RYGUtils getDoublePref:@"profile_analyzer_snapshot_cap"];
	[RYGProfileAnalyzerStorage appendSnapshotToHistory:cur forUserPK:self.userPK capacity:cap];
	[self reloadFromStore];
	[self refreshBarButtons];
	RYGNotifySuccess(RYG_NOTIF_ANALYZER_DONE, RYGLocalized(@"Snapshot saved"),
	                 RYGLocalized(@"Your current scan was archived"));
}

- (void)promptCapacity {
	NSArray<NSNumber *> *opts = @[@10, @20, @50, @100, @0];
	NSInteger cap = (NSInteger)[RYGUtils getDoublePref:@"profile_analyzer_snapshot_cap"];
	UIAlertController *a = [UIAlertController alertControllerWithTitle:RYGLocalized(@"Keep newest snapshots")
															  message:RYGLocalized(@"Older snapshots beyond this limit are deleted on the next scan.")
													   preferredStyle:UIAlertControllerStyleActionSheet];
	for (NSNumber *n in opts) {
		NSInteger v = n.integerValue;
		NSString *title = v > 0 ? [NSString stringWithFormat:@"%ld", (long)v] : RYGLocalized(@"Unlimited");
