...
#import "RYGProfileAnalyzerSnapshotsViewController.h"
#import "../../Utils.h"
...
// RYGProfileAnalyzerChecks; this adds the live count / unread / enabled flags.
@interface RYGPACategoryDescriptor : NSObject
...
@end
@implementation RYGPACategoryDescriptor @end

#pragma mark - Avatar with progress ring

@interface RYGPAAvatarRingView : UIView
...
@implementation RYGPAAvatarRingView
- (instancetype)initWithFrame:(CGRect)frame {
	...
	_progressLayer.strokeColor = ([RYGUtils RYGColor_Primary] ?: [UIColor systemBlueColor]).CGColor;
...
@interface RYGPAHeaderView : UIView
@property (nonatomic, strong) RYGPAAvatarRingView *avatar;
...
@implementation RYGPAHeaderView
- (instancetype)initWithFrame:(CGRect)frame {
	self = [super initWithFrame:frame];
	if (!self) return self;
	...
	_avatar = [[RYGPAAvatarRingView alloc] init];
	...
	_scanButton.backgroundColor = [RYGUtils RYGColor_Primary] ?: [UIColor systemBlueColor];
	...
	[_scanButton setTitle:RYGLocalized(@"Run analysis") forState:UIControlStateNormal];
...
- (void)setStatsLabelsPosts:(NSString *)posts followers:(NSString *)followers following:(NSString *)following {
	...
	[self.statsRow addArrangedSubview:[self statColumn:posts caption:RYGLocalized(@"Posts")]];
	[self.statsRow addArrangedSubview:[self statColumn:followers caption:RYGLocalized(@"Followers")]];
	[self.statsRow addArrangedSubview:[self statColumn:following caption:RYGLocalized(@"Following")]];
}
...
@interface RYGPACategoryCell : UITableViewCell
...
@implementation RYGPACategoryCell
...
@interface RYGProfileAnalyzerViewController () <UITableViewDataSource, UITableViewDelegate>
@property (nonatomic, strong) UITableView *tableView;
@property (nonatomic, strong) UIView *headerContainer;
@property (nonatomic, strong) RYGPAHeaderView *headerView;

@property (nonatomic, strong) RYGProfileAnalyzerReport *report;
@property (nonatomic, strong) NSArray<RYGProfileAnalyzerVisit *> *visits;
@property (nonatomic, strong) NSArray<RYGPACategoryDescriptor *> *categories;
@property (nonatomic, strong) NSArray<RYGPACategoryDescriptor *> *trackingCategories;
...
- (NSArray<NSString *> *)identityIDsForCategory:(RYGPACategory)category;
@end

@implementation RYGProfileAnalyzerViewController

- (void)viewDidLoad {
	[super viewDidLoad];
	self.view.backgroundColor = [RYGPopupChrome backgroundColor];
	self.title = RYGLocalized(@"Profile Analyzer");

	[self setupTable];

	NSNotificationCenter *nc = [NSNotificationCenter defaultCenter];
	[nc addObserver:self selector:@selector(analyzerDataChanged:)
	           name:RYGProfileAnalyzerDataDidChangeNotification object:nil];
	[nc addObserver:self selector:@selector(serviceProgressDidChange:)
	           name:RYGProfileAnalyzerProgressDidChangeNotification object:nil];
	[nc addObserver:self selector:@selector(serviceHeaderInfoDidChange:)
	           name:RYGProfileAnalyzerHeaderInfoDidChangeNotification object:nil];
	[nc addObserver:self selector:@selector(serviceDidFinish:)
	           name:RYGProfileAnalyzerDidFinishNotification object:nil];
}

- (void)dealloc {
	[[NSNotificationCenter defaultCenter] removeObserver:self];
	// Safety net — viewWillDisappear may be skipped on torn-down hierarchies.
	if (_attachedToService) [[RYGProfileAnalyzerService sharedService] detachObserver];
}
...
- (void)analyzerDataChanged:(NSNotification *)note {
	...
	@try {
		...
		RYGProfileAnalyzerSnapshot *cur = self.report.current;
		if (cur) {
...
- (void)viewWillAppear:(BOOL)animated {
	...
	if (!self.attachedToService) {
		...
		[[RYGProfileAnalyzerService sharedService] attachObserver];
	}
...
- (void)viewWillDisappear:(BOOL)animated {
	...
	if (leaving && self.attachedToService) {
		...
		[[RYGProfileAnalyzerService sharedService] detachObserver];
	}
}
...
- (void)attachToRunningServiceIfAny {
	RYGProfileAnalyzerService *svc = [RYGProfileAnalyzerService sharedService];
	if (!svc.isRunning) return;
	[self enterRunningState];
	self.headerView.progressLabel.text = svc.lastStatus.length ? svc.lastStatus : RYGLocalized(@"Starting…");
...
- (void)setupTable {
	...
	[self.tableView registerClass:[RYGPACategoryCell class] forCellReuseIdentifier:@"cat"];
...
- (void)pullToRefreshProfile:(UIRefreshControl *)sender {
	...
	[RYGInstagramAPI sendRequestWithMethod:@"GET"
		...
		if (user.count) {
			[RYGProfileAnalyzerStorage saveHeaderInfo:user forUserPK:pk];
			typeof(self) strongSelf = weakSelf;
			if (strongSelf.isViewLoaded && strongSelf.view.window) {
				[strongSelf paintHeaderFromUserInfo:user];
				[strongSelf applyFollowerLimitGateFor:rygHeaderInteger(user, @"follower_count")];
			}
		}
...
- (void)buildTableHeader {
	...
	self.headerView = [[RYGPAHeaderView alloc] init];
...
- (void)loadHeaderLayered {
	...
	NSMutableDictionary *cached = [[RYGProfileAnalyzerStorage headerInfoForUserPK:pk] mutableCopy]
								?: [NSMutableDictionary dictionary];
	RYGProfileAnalyzerSnapshot *snap = self.report.current;
	...
	if (haveUsername || haveCounts) {
		...
		self.headerView.fullNameLabel.text = RYGLocalized(@"No scan yet");
		self.headerView.usernameLabel.text = @"";
		[self.headerView setStatsLabelsPosts:@"—" followers:@"—" following:@"—"];
	}

	if (haveCounts && haveUsername) {
		[RYGProfileAnalyzerStorage saveHeaderInfo:cached forUserPK:pk];
	}
...
- (void)fetchAndCacheHeader {
	NSString *pk = self.lastHeaderPK ?: [RYGUtils currentUserPK];
	if (!pk.length) {
		...
		dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(1.0 * NSEC_PER_SEC)),
			...
			if ([RYGUtils currentUserPK].length) [weakSelf fetchAndCacheHeader];
		});
		return;
	}
	self.lastHeaderPK = pk;
	...
	[RYGInstagramAPI sendRequestWithMethod:@"GET"
		...
		if (!user.count) {
			static const char kRYGPARetryKey;
			NSNumber *attempt = objc_getAssociatedObject(strongSelf, &kRYGPARetryKey);
			if (attempt.intValue < 1) {
				objc_setAssociatedObject(strongSelf, &kRYGPARetryKey, @(attempt.intValue + 1), OBJC_ASSOCIATION_RETAIN_NONATOMIC);
...
- (void)applyFollowerLimitGateFor:(NSInteger)followers {
	if (followers > RYGProfileAnalyzerMaxFollowerCount) {
		self.headerView.warningLabel.hidden = NO;
		self.headerView.warningLabel.text = [NSString stringWithFormat:
			RYGLocalized(@"Follower count exceeds %ld — analysis disabled to avoid rate limits."),
			(long)RYGProfileAnalyzerMaxFollowerCount];
...
- (NSString *)compactNumber:(NSInteger)n {
	return [RYGUtils shortCount:(long long)n];
}
...
- (RYGProfileAnalyzerSnapshot *)comparisonSnapshotForUserPK:(NSString *)pk
                                           fallbackPrevious:(RYGProfileAnalyzerSnapshot *)prev {
	NSString *sel = [RYGProfileAnalyzerStorage compareSelectionForUserPK:pk];
	// Only a valid, still-present archived pick overrides the rolling last scan.
	if (sel.length && ![sel isEqualToString:RYGProfileAnalyzerCompareSelectionPrevious]) {
		RYGProfileAnalyzerSnapshot *picked = [RYGProfileAnalyzerStorage historySnapshotWithID:sel forUserPK:pk];
...
- (void)loadCachedReport {
	NSString *pk = [RYGUtils currentUserPK];
	RYGProfileAnalyzerSnapshot *cur = [RYGProfileAnalyzerStorage currentSnapshotForUserPK:pk];
	RYGProfileAnalyzerSnapshot *prev = [RYGProfileAnalyzerStorage previousSnapshotForUserPK:pk];
	self.report = [RYGProfileAnalyzerReport reportFromCurrent:cur previous:[self comparisonSnapshotForUserPK:pk fallbackPrevious:prev]];
	self.visits = [RYGProfileAnalyzerStorage visitedProfilesForUserPK:pk];
...
- (void)rebuildCategories {
	NSMutableArray<RYGPACategoryDescriptor *> *cats = [NSMutableArray array];
	for (RYGPACheckDescriptor *chk in [RYGProfileAnalyzerChecks allChecks]) {
		RYGPACategoryDescriptor *d = [RYGPACategoryDescriptor new];
		d.category = chk.category;
		d.title = RYGLocalized(chk.title);
		d.subtitle = RYGLocalized(chk.subtitle);
		d.symbol = chk.symbol;
		d.color = chk.color;
		d.requiresPrevious = chk.requiresPrevious;
		d.enabled = [RYGProfileAnalyzerChecks isCheckEnabledForKey:chk.prefKey];
		d.count = [self identityIDsForCategory:chk.category].count;
		[cats addObject:d];
	}
	self.categories = cats;

	RYGPACategoryDescriptor *visited = [RYGPACategoryDescriptor new];
	visited.category = RYGPACategoryVisitedProfiles;
	visited.title = RYGLocalized(@"Visited profiles");
	visited.subtitle = [RYGUtils getBoolPref:@"profile_analyzer_track_visits"]
		? RYGLocalized(@"Profiles you've opened recently")
		: RYGLocalized(@"Tracking off — enable below to log visits");
...
- (NSArray<NSString *> *)identityIDsForCategory:(RYGPACategory)category {
	RYGProfileAnalyzerReport *r = self.report;
	NSArray<RYGProfileAnalyzerUser *> *users = nil;
	switch (category) {
		case RYGPACategoryMutual:             users = r.mutualFollowers; break;
		case RYGPACategoryNotFollowingBack:   users = r.notFollowingYouBack; break;
		case RYGPACategoryDontFollowBack:     users = r.youDontFollowBack; break;
		case RYGPACategoryNewFollowers:       users = r.recentFollowers; break;
		case RYGPACategoryLostFollowers:      users = r.lostFollowers; break;
		case RYGPACategoryYouStartedFollowing: users = r.youStartedFollowing; break;
		case RYGPACategoryYouUnfollowed:      users = r.youUnfollowed; break;
		case RYGPACategoryProfileUpdates: {
			NSMutableArray *ids = [NSMutableArray array];
			for (RYGProfileAnalyzerProfileChange *c in r.profileUpdates)
				[ids addObject:[NSString stringWithFormat:@"%@|%@|%@|%@",
					c.current.pk ?: @"", c.current.username ?: @"",
					c.current.fullName ?: @"", c.current.profilePicID ?: @""]];
			return ids;
		}
		case RYGPACategoryVisitedProfiles: {
			NSMutableArray *ids = [NSMutableArray array];
			for (RYGProfileAnalyzerVisit *v in self.visits) if (v.user.pk.length) [ids addObject:v.user.pk];
			return ids;
		}
	}
	NSMutableArray *ids = [NSMutableArray array];
	for (RYGProfileAnalyzerUser *u in users) if (u.pk.length) [ids addObject:u.pk];
	return ids;
}

- (void)computeUnreadForDescriptors:(NSArray<RYGPACategoryDescriptor *> *)descs {
	NSString *pk = [RYGUtils currentUserPK];
	for (RYGPACategoryDescriptor *d in descs) {
		if (!d.enabled) { d.unread = 0; d.delta = 0; continue; }
		NSString *key = [NSString stringWithFormat:@"%ld", (long)d.category];
		NSArray<NSString *> *ids = [self identityIDsForCategory:d.category];
		NSArray<NSString *> *seen = [RYGProfileAnalyzerStorage seenIDsForUserPK:pk categoryKey:key];
		if (!seen) {                                  // first sight — seed silently
			[RYGProfileAnalyzerStorage markSeenIDs:ids forUserPK:pk categoryKey:key];
			d.unread = 0;
			d.delta = 0;
			continue;
		}
		NSSet *seenSet = [NSSet setWithArray:seen];
		NSSet *curSet = [NSSet setWithArray:ids];
		NSInteger added = 0, removed = 0;
		for (NSString *i in ids) if (![seenSet containsObject:i]) added++;
		for (NSString *i in seen) if (![curSet containsObject:i]) removed++;
		d.unread = added;
		d.delta = added - removed;
	}
}

- (void)markCategorySeen:(RYGPACategoryDescriptor *)d {
	[RYGProfileAnalyzerStorage markSeenIDs:[self identityIDsForCategory:d.category]
								forUserPK:[RYGUtils currentUserPK]
...
- (void)refreshHeader {
	self.headerView.scanDateLabel.text = self.report.current
		? [self scanDateText]
		: RYGLocalized(@"Run your first analysis");
	[self refreshWarning];
}
...
- (NSString *)scanDateText {
	...
	if (self.report.previous) return [NSString stringWithFormat:RYGLocalized(@"Last scan: %@"), when];
	return [NSString stringWithFormat:RYGLocalized(@"First scan: %@"), when];
}
...
- (void)refreshWarning {
	...
	BOOL overLimit = followers > RYGProfileAnalyzerMaxFollowerCount;
...
- (void)analyzeTapped {
	if (self.running) { [[RYGProfileAnalyzerService sharedService] cancel]; return; }
	[self enterRunningState];
	[[RYGProfileAnalyzerService sharedService] start];
}
...
- (void)enterRunningState {
	...
	self.headerView.progressLabel.text = RYGLocalized(@"Starting…");
	[self.headerView.avatar setShowProgress:YES];
	self.headerView.avatar.progress = 0;
	[self.headerView.scanButton setTitle:RYGLocalized(@"Cancel") forState:UIControlStateNormal];
...
- (void)serviceDidFinish:(NSNotification *)note {
	if (!self.isViewLoaded) return;
	RYGProfileAnalyzerSnapshot *snap = note.userInfo[@"snapshot"];
	NSError *error = note.userInfo[@"error"];
	[self onAnalysisFinished:[snap isKindOfClass:[RYGProfileAnalyzerSnapshot class]] ? snap : nil
	                   error:[error isKindOfClass:[NSError class]] ? error : nil];
}

// IG payloads carry NSNull for absent fields — coerce safely.
static NSString *rygHeaderString(NSDictionary *d, NSString *key) {
	id v = d[key];
	return [v isKindOfClass:[NSString class]] ? (NSString *)v : nil;
}
static NSInteger rygHeaderInteger(NSDictionary *d, NSString *key) {
	id v = d[key];
...
- (void)paintHeaderFromUserInfo:(NSDictionary *)user {
	...
	NSString *username = rygHeaderString(user, @"username");
	NSString *fullName = rygHeaderString(user, @"full_name");
	NSString *picURL   = rygHeaderString(user, @"profile_pic_url");
	NSInteger followers = rygHeaderInteger(user, @"follower_count");
	NSInteger following = rygHeaderInteger(user, @"following_count");
	NSInteger posts	 = rygHeaderInteger(user, @"media_count");

	self.headerView.fullNameLabel.text = fullName.length ? fullName
									   : (username.length ? username : RYGLocalized(@"No scan yet"));
	...
	if (picURL.length) {
		...
		[RYGImageCache loadImageFromURL:picU cacheKey:picKey completion:^(UIImage *img) {
			if (img) iv.image = img;
		}];
	}
...
- (void)onAnalysisFinished:(RYGProfileAnalyzerSnapshot *)snapshot error:(NSError *)error {
	self.running = NO;
	self.headerView.progressLabel.hidden = YES;
	[self.headerView.avatar setShowProgress:NO];
	[self.headerView.scanButton setTitle:RYGLocalized(@"Run analysis") forState:UIControlStateNormal];
	self.headerView.scanButton.backgroundColor = [RYGUtils RYGColor_Primary] ?: [UIColor systemBlueColor];
	[self.view setNeedsLayout];

	if (error && error.code == RYGProfileAnalyzerErrorTooManyFollowers) {
		[self alertTitle:RYGLocalized(@"Too many followers")
				 message:[NSString stringWithFormat:RYGLocalized(@"We refuse to run when the follower count exceeds %ld to avoid Instagram rate limits."),
						  (long)RYGProfileAnalyzerMaxFollowerCount]];
		return;
	}
	if (error && error.code != RYGProfileAnalyzerErrorCancelled) {
		[self alertTitle:RYGLocalized(@"Analysis failed") message:error.localizedDescription ?: @""];
		return;
	}
	[self loadCachedReport];
}
...
- (void)resetTapped {
	NSString *pk = [RYGUtils currentUserPK];
	UIAlertController *a = [UIAlertController alertControllerWithTitle:[NSString stringWithFormat:@"%@?", RYGLocalized(@"Reset analyzer data")]
															  message:RYGLocalized(@"Pick what to remove. Snapshots clears the scan diffs and every recorded snapshot; visited profiles wipes the visit history.")
													   preferredStyle:UIAlertControllerStyleActionSheet];
	[a addAction:[UIAlertAction actionWithTitle:RYGLocalized(@"Reset snapshots") style:UIAlertActionStyleDestructive handler:^(UIAlertAction *_) {
		[RYGProfileAnalyzerStorage resetForUserPK:pk];
		[self loadCachedReport];
	}]];
	[a addAction:[UIAlertAction actionWithTitle:RYGLocalized(@"Clear visited profiles") style:UIAlertActionStyleDestructive handler:^(UIAlertAction *_) {
		[RYGProfileAnalyzerStorage clearVisitsForUserPK:pk];
		[self loadCachedReport];
	}]];
	[a addAction:[UIAlertAction actionWithTitle:RYGLocalized(@"Reset everything") style:UIAlertActionStyleDestructive handler:^(UIAlertAction *_) {
		[RYGProfileAnalyzerStorage resetForUserPK:pk];
		[RYGProfileAnalyzerStorage clearVisitsForUserPK:pk];
		[self loadCachedReport];
	}]];
...
- (void)infoTapped {
	NSString *body = [@[
		RYGLocalized(@"First scan: we collect your followers and following lists and save them locally."),
		RYGLocalized(@"Second scan onward: each scan compares against the last, so we can show gained/lost followers, your own follow/unfollow moves, and profile updates."),
		RYGLocalized(@"Nothing is uploaded — everything stays on this device and can be wiped from the trash icon."),
		RYGLocalized(@"Large accounts are blocked: analysis is disabled above 13,000 followers to avoid Instagram rate-limiting the whole app."),
		RYGLocalized(@"Heads up: this feature hits Instagram's private API. Running it back-to-back or right after heavy follow/unfollow activity can trigger a short rate-limit. Use it sparingly and at your own risk."),
	] componentsJoinedByString:@"\n\n"];
	UIAlertController *a = [UIAlertController alertControllerWithTitle:RYGLocalized(@"About Profile Analyzer") message:body preferredStyle:UIAlertControllerStyleAlert];
...
- (NSString *)tableView:(UITableView *)tv titleForHeaderInSection:(NSInteger)section {
	if (section == 0) return RYGLocalized(@"Categories");
	if (section == 1) return RYGLocalized(@"Tracking");
	if (section == 2) return RYGLocalized(@"Preferences");
	return @"";
}
- (NSString *)tableView:(UITableView *)tv titleForFooterInSection:(NSInteger)section {
	if (section == 2) return RYGLocalized(@"Snapshots let you archive each scan and pick which one new scans compare against. Track visits records every profile you open so you can review them here.");
	return nil;
}
...
- (UITableViewCell *)tableView:(UITableView *)tv cellForRowAtIndexPath:(NSIndexPath *)indexPath {
	...
	RYGPACategoryCell *cell = [tv dequeueReusableCellWithIdentifier:@"cat" forIndexPath:indexPath];
	RYGPACategoryDescriptor *d = (indexPath.section == 1)
	...
	if (turnedOff) {
		cell.subtitleLabel.text = RYGLocalized(@"Disabled");
	} else if (d.standalone) {
		cell.subtitleLabel.text = d.subtitle;
	} else if (waitingForPrev) {
		cell.subtitleLabel.text = RYGLocalized(@"Available after your next scan");
	} else {
		cell.subtitleLabel.text = d.subtitle;
	}
	BOOL showDash = !d.standalone && (waitingForPrev || !hasReport);
	cell.countLabel.text = turnedOff ? RYGLocalized(@"Off")
	...
	BOOL showBadge = !disabled && d.delta != 0;
	cell.badgeView.hidden = !showBadge;
	cell.badgeCollapse.active = !showBadge;
	if (showBadge) {
		BOOL grew = d.delta > 0;
		NSInteger mag = ABS(d.delta);
		cell.badgeView.backgroundColor = grew ? [UIColor systemGreenColor] : [UIColor systemRedColor];
		cell.badgeLabel.text = [NSString stringWithFormat:@"%@%@",
			grew ? @"+" : @"−", mag > 99 ? @"99" : [NSString stringWithFormat:@"%ld", (long)mag]];
	}
...
- (void)tableView:(UITableView *)tv didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
	...
	if (indexPath.section == 2) {
		if (indexPath.row == 0) {
			[self.navigationController pushViewController:[RYGProfileAnalyzerChecksViewController new] animated:YES];
		} else if (indexPath.row == 1) {
			RYGProfileAnalyzerSnapshotsViewController *vc = [[RYGProfileAnalyzerSnapshotsViewController alloc] initWithUserPK:[RYGUtils currentUserPK]];
	...
	if ([vc isKindOfClass:[RYGProfileAnalyzerListViewController class]])
		((RYGProfileAnalyzerListViewController *)vc).unseenEntryIDs = newIDs;
	[self.navigationController pushViewController:vc animated:YES];
}

- (NSSet<NSString *> *)unseenIDSetForCategory:(RYGPACategoryDescriptor *)d {
	NSString *pk = [RYGUtils currentUserPK];
	NSString *key = [NSString stringWithFormat:@"%ld", (long)d.category];
	NSArray<NSString *> *seen = [RYGProfileAnalyzerStorage seenIDsForUserPK:pk categoryKey:key];
...
- (UITableViewCell *)preferencesCellForRow:(NSInteger)row tableView:(UITableView *)tv {
	if (row == 0) {
		...
		cell.textLabel.text = RYGLocalized(@"Checks");
	...
	if (row == 1) {
		...
		cell.selectionStyle = UITableViewCellSelectionStyleDefault;
		cell.accessoryType = UITableViewCellAccessoryDisclosureIndicator;
		cell.textLabel.text = RYGLocalized(@"Snapshots");
	...
	cell.textLabel.text = RYGLocalized(@"Track visited profiles");
	cell.detailTextLabel.text = RYGLocalized(@"Logs every profile you open. Stays on-device.");
	...
	sw.on = [RYGUtils getBoolPref:@"profile_analyzer_track_visits"];
...
- (NSString *)checksSubtitle {
	NSArray<RYGPACheckDescriptor *> *all = [RYGProfileAnalyzerChecks allChecks];
	NSUInteger on = 0;
	for (RYGPACheckDescriptor *c in all)
		if ([RYGProfileAnalyzerChecks isCheckEnabledForKey:c.prefKey]) on++;
	return [NSString stringWithFormat:RYGLocalized(@"%lu of %lu checks on"), (unsigned long)on, (unsigned long)all.count];
}

- (NSString *)snapshotsSubtitle {
	NSString *pk = [RYGUtils currentUserPK];
	NSUInteger count = [RYGProfileAnalyzerStorage snapshotHistoryForUserPK:pk].count;
	NSString *sel = [RYGProfileAnalyzerStorage compareSelectionForUserPK:pk];
	NSString *anchor;
	if (sel.length && ![sel isEqualToString:RYGProfileAnalyzerCompareSelectionPrevious]
		&& [RYGProfileAnalyzerStorage historySnapshotWithID:sel forUserPK:pk]) {
		anchor = RYGLocalized(@"a saved snapshot");
	} else {
		anchor = RYGLocalized(@"your previous scan");
	}
	if (count == 0) return [NSString stringWithFormat:RYGLocalized(@"Comparing against %@"), anchor];
	return [NSString stringWithFormat:RYGLocalized(@"%lu saved · comparing against %@"), (unsigned long)count, anchor];
}
...
- (UITableViewCell *)actionCellForRow:(NSInteger)row tableView:(UITableView *)tv {
	...
	if (row == 0) {
		cell.textLabel.text = RYGLocalized(@"About Profile Analyzer");
		cell.textLabel.textColor = [RYGUtils RYGColor_Primary] ?: [UIColor systemBlueColor];
		cell.imageView.image = [UIImage systemImageNamed:@"info.circle"];
		cell.imageView.tintColor = cell.textLabel.textColor;
	} else {
		cell.textLabel.text = RYGLocalized(@"Reset analyzer data");
...
- (UIViewController *)listVCForCategory:(RYGPACategoryDescriptor *)d {
	RYGProfileAnalyzerReport *r = self.report;
	switch (d.category) {
		case RYGPACategoryMutual:
			return [[RYGProfileAnalyzerListViewController alloc] initWithTitle:d.title users:r.mutualFollowers kind:RYGPAListKindMutual];
		case RYGPACategoryVisitedProfiles:
			return [[RYGProfileAnalyzerListViewController alloc] initVisitedListWithTitle:d.title visits:self.visits];
		case RYGPACategoryNotFollowingBack:
			return [[RYGProfileAnalyzerListViewController alloc] initWithTitle:d.title users:r.notFollowingYouBack kind:RYGPAListKindUnfollow];
		case RYGPACategoryDontFollowBack:
			return [[RYGProfileAnalyzerListViewController alloc] initWithTitle:d.title users:r.youDontFollowBack kind:RYGPAListKindFollow];
		case RYGPACategoryNewFollowers:
			return [[RYGProfileAnalyzerListViewController alloc] initWithTitle:d.title users:r.recentFollowers kind:RYGPAListKindPlain];
		case RYGPACategoryLostFollowers:
			return [[RYGProfileAnalyzerListViewController alloc] initWithTitle:d.title users:r.lostFollowers kind:RYGPAListKindPlain];
		case RYGPACategoryYouStartedFollowing:
			return [[RYGProfileAnalyzerListViewController alloc] initWithTitle:d.title users:r.youStartedFollowing kind:RYGPAListKindUnfollow];
		case RYGPACategoryYouUnfollowed:
			return [[RYGProfileAnalyzerListViewController alloc] initWithTitle:d.title users:r.youUnfollowed kind:RYGPAListKindRefollow];
		case RYGPACategoryProfileUpdates:
			return [[RYGProfileAnalyzerListViewController alloc] initWithTitle:d.title profileUpdates:r.profileUpdates];
	}
}
