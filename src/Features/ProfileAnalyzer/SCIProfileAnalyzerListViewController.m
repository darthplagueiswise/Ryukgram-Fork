...
static NSString *sciPicCacheKey(SCIProfileAnalyzerUser *user, NSURL *url) {
...
- (BOOL)supportsBatchAction {
	return self.kind == SCIPAListKindUnfollow
		...
		|| self.kind == SCIPAListKindRefollow
...
- (UITableViewCell *)tableView:(UITableView *)tv cellForRowAtIndexPath:(NSIndexPath *)indexPath {
	...
	if (self.kind == SCIPAListKindProfileUpdate) {
		...
		NSString *dateLine = (visit.visitCount > 1)
			? [NSString stringWithFormat:SCILocalized(@"%@ · %ld"), when, (long)visit.visitCount]
			: when;
	...
	if (user.profilePicURL.length) {
		...
		[SCIImageCache loadImageFromURL:url cacheKey:sciPicCacheKey(user, url) completion:^(UIImage *image) {
...
- (void)refreshBatchBar {
	...
	BOOL follow = (self.kind == SCIPAListKindFollow || self.kind == SCIPAListKindRefollow);
	NSString *t = follow
...
- (void)batchActionTapped {
	...
	if (self.kind == SCIPAListKindRefollow) {
		[self confirmBatchOp:SCIPABatchOpFollow];
		return;
	}
