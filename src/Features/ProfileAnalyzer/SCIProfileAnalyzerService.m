...
- (void)runForSelfWithHeaderInfo:(SCIPAHeaderInfo)headerInfo
    ...
    if (!selfPK.length) {
        [self finishWithSnapshot:nil
                           error:[self errorWithCode:SCIProfileAnalyzerErrorNoSession message:@"No active session"]
    ...
    [SCIInstagramAPI sendRequestWithMethod:@"GET"
        ...
        if (strongSelf.cancelled) {
            [strongSelf finishWithSnapshot:nil error:[strongSelf errorWithCode:SCIProfileAnalyzerErrorCancelled message:@"Cancelled"]
                                completion:completion];
            return;
        }
        ...
        if (!user) {
            [strongSelf finishWithSnapshot:nil
                                     error:[strongSelf errorWithCode:SCIProfileAnalyzerErrorNetwork message:@"Profile fetch failed"]
                                completion:completion];
            return;
        }
...
- (void)pagePath:(NSString *)basePath
    ...
    [SCIInstagramAPI sendRequestWithMethod:@"GET" path:path body:nil completion:^(NSDictionary *resp, NSError *error) {
        ...
        NSString *label = [NSString stringWithFormat:@"Fetching %@ (%lu/%ld)…", stage, (unsigned long)acc.count, (long)total];
