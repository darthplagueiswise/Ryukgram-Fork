...
NSNotificationName const RYGReadReceiptsDidChangeNotification = @"RYGReadReceiptsDidChangeNotification";
...
@implementation RYGReadReceiptStorage
...
// Read several times per thread update, so it stays in memory and writes through on the serial ioQ.
...
static void post(void) {
    dispatch_async(dispatch_get_main_queue(), ^{
        [NSNotificationCenter.defaultCenter postNotificationName:RYGReadReceiptsDidChangeNotification object:nil];
    });
}
...
+ (NSArray<RYGReadReceipt *> *)allReceiptsForOwnerPK:(NSString *)ownerPK {
    __block NSMutableArray *out = [NSMutableArray array];
    ioSync(^{
        for (NSDictionary *d in readArray(receiptsFile(ownerPK))) {
            RYGReadReceipt *r = [RYGReadReceipt receiptFromJSONDict:d];
            if (r.messageId) [out addObject:r];
        }
    });
    [out sortUsingComparator:^NSComparisonResult(RYGReadReceipt *a, RYGReadReceipt *b) {
        return [b.readAt compare:a.readAt]; // newest-first
    }];
    return out;
}

+ (NSArray<RYGReadReceipt *> *)receiptsForReaderPK:(NSString *)readerPK ownerPK:(NSString *)ownerPK {
    NSMutableArray *out = [NSMutableArray array];
    for (RYGReadReceipt *r in [self allReceiptsForOwnerPK:ownerPK])
        if ([r.readerPk isEqualToString:readerPK]) [out addObject:r];
    return out;
}

+ (NSArray<RYGReadReceipt *> *)receiptsForThreadId:(NSString *)threadId ownerPK:(NSString *)ownerPK {
    NSMutableArray *out = [NSMutableArray array];
    for (RYGReadReceipt *r in [self allReceiptsForOwnerPK:ownerPK])
...
+ (NSArray<RYGReadReceiptGroup *> *)groupedByThreadForOwnerPK:(NSString *)ownerPK {
    ...
    [byThread enumerateKeysAndObjectsUsingBlock:^(NSString *key, NSMutableArray *list, BOOL *stop) {
        RYGReadReceipt *latest = list.firstObject; // newest-first
        RYGReadReceiptGroup *g = [RYGReadReceiptGroup new];
    ...
    [groups sortUsingComparator:^NSComparisonResult(RYGReadReceiptGroup *a, RYGReadReceiptGroup *b) {
...
+ (void)addReceipt:(RYGReadReceipt *)receipt forOwnerPK:(NSString *)ownerPK {
...
+ (void)deleteReceiptWithMessageId:(NSString *)messageId readerPk:(NSString *)readerPk threadId:(NSString *)threadId ownerPK:(NSString *)ownerPK {
    if (!messageId.length || !readerPk.length) return;
    ioSync(^{
        NSString *path = receiptsFile(ownerPK);
        NSMutableArray *keep = [NSMutableArray array];
        for (NSDictionary *d in readArray(path)) {
            BOOL match = [d[@"messageId"] isEqualToString:messageId] && [d[@"readerPk"] isEqualToString:readerPk]
                && (!threadId.length || [d[@"threadId"] isEqualToString:threadId]);
            if (!match) [keep addObject:d];
        }
        writeJSON(keep, path);
    });
    post();
        ...
        for (NSString *name in names) {
            ...
            if ([name hasSuffix:@".state.json"]) {
                ...
                [arr sortUsingComparator:^NSComparisonResult(NSDictionary *a, NSDictionary *b) {
                    double ta = [RYGJSONScalar(a[@"readAt"]) doubleValue], tb = [RYGJSONScalar(b[@"readAt"]) doubleValue];
