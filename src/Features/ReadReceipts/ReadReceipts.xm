...
static void rrInspectThread(id thread, NSString *ownerUsername) {
    @try {
        ...
        if (isGroup && ![RYGUtils getBoolPref:@"read_receipts_log_groups"]) return;
        if ([RYGReadReceiptStorage isThreadExcluded:tid ownerPK:viewer]) return;

        NSString *threadTitle = isGroup ? [RYGDirectThreadInfo groupNameFromMetadata:meta viewerPK:viewer] : nil;
        NSString *threadAvatar = isGroup ? [RYGDirectThreadInfo groupImageURLFromMetadata:meta] : nil;
        ...
        if (myIds.count) [RYGReadReceiptStorage recordMyMessages:myIds forThread:tid ownerPK:viewer];

        NSDictionary<NSString *, NSNumber *> *myMsgs = [RYGReadReceiptStorage myMessagesForThread:tid ownerPK:viewer];
        ...
        [(NSDictionary *)seen enumerateKeysAndObjectsUsingBlock:^(id pkKey, id info, BOOL *stop) {
            ...
            if ([RYGReadReceiptStorage isReaderExcluded:readerPk ownerPK:viewer]) return;

            NSString *lastSeen = rrStr(rrCall(info, @selector(messageId)));
            if (!lastSeen.length) return;
            NSString *prevSeen = [RYGReadReceiptStorage lastSeenMessageIdForThread:tid reader:readerPk ownerPK:viewer];
            if ([lastSeen isEqualToString:prevSeen]) return; // position unchanged
            [RYGReadReceiptStorage setLastSeenMessageId:lastSeen forThread:tid reader:readerPk ownerPK:viewer];
            ...
            double prevMineTs = [RYGReadReceiptStorage lastReadMineTimeForThread:tid reader:readerPk ownerPK:viewer];
            ...
            if (!prevSeen) { // first sighting -> baseline, don't fire for already-read history
                if (candidate) [RYGReadReceiptStorage setLastReadMineTime:candidateTs forThread:tid reader:readerPk ownerPK:viewer];
                return;
            }
            if (!candidate || candidateTs <= prevMineTs) return; // nothing newly read of ours
            [RYGReadReceiptStorage setLastReadMineTime:candidateTs forThread:tid reader:readerPk ownerPK:viewer];
            ...
            if ([RYGActivityConfig shouldLogType:RYGActivityTypeRead forPK:readerPk]) {
                ...
                [RYGReadReceiptStorage addReceipt:r forOwnerPK:viewer];
                [RYGHomeShortcutBadges bumpActionID:@"read_receipts"];
            }

            // UIKit / notification / session work must be on the main thread (this can run off it).
            NSString *fOwner = ownerUsername, *fViewer = viewer, *fTid = tid, *fUser = username, *fPreview = preview, *fTitle = threadTitle, *fReader = readerPk;
            BOOL fGroup = isGroup;
            dispatch_async(dispatch_get_main_queue(), ^{
                if (![RYGActivityConfig shouldNotifyType:RYGActivityTypeRead forPK:fReader]) return;
                BOOL inThatChat = UIApplication.sharedApplication.applicationState == UIApplicationStateActive
                                  && [[RYGExcludedThreads activeThreadId] isEqualToString:fTid];
                if (inThatChat) return;
                NSString *handle = fUser.length ? [@"@" stringByAppendingString:fUser] : RYGLocalized(@"Someone");
                NSString *sub = fGroup && fTitle.length
                    ? [NSString stringWithFormat:RYGLocalized(@"read your message in %@"), fTitle]
                    : RYGLocalized(@"read your message");
                NSString *active = [RYGUtils currentUserPK];
                if (fOwner.length && active.length && ![active isEqualToString:fViewer])
                    sub = [NSString stringWithFormat:RYGLocalized(@"%@ · on @%@"), sub, fOwner];
                if (fPreview.length) sub = [NSString stringWithFormat:@"%@ · %@", sub, fPreview];
                RYGNotify(RYG_NOTIF_READ_RECEIPT, handle, sub, @"eye.fill", RYGNotificationToneInfo);
            });
        }];
    } @catch (__unused id e) {}
}
...
- (void)_applyThreadUpdates:(id)updates completion:(id)completion userAccess:(id)access {
    %orig;
    @try {
        BOOL rrOn = [RYGActivityConfig readReceiptsActive];
        // deleted-messages group images use the same shared cache; register it for them too.
        BOOL dmOn = [RYGUtils getBoolPref:@"keep_deleted_message"] || [RYGUtils getBoolPref:@"deleted_messages_log_enabled"];
        ...
        [RYGDirectThreadInfo registerCache:cache forOwnerPK:ownerPk];
