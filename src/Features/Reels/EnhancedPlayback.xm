...
// Reels playback enhancements, active when tap control is set to pause/play.
...
// ============ FIND PLAYBACK VIEW ============
// Two IG A/B variants: an IGSundialPlaybackToggleView wrapper, or a bare IGDSMediaIconButton.
...
// The toggle sits outside the container IG animates, so it mirrors the download button's opacity.
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context {
	...
	// Zoom callbacks drive the play button directly.
...
static void rygSetupKVO(UIView *ufi) {
	...
	// Strong: a layer that deallocates while still observed crashes the app.
	if (!rygObservedLayers) rygObservedLayers = [NSHashTable hashTableWithOptions:NSPointerFunctionsStrongMemory];

	if ([rygObservedLayers containsObject:dlBtn.layer]) return;

	for (CALayer *layer in rygObservedLayers.allObjects) {
		UIView *owner = [layer.delegate isKindOfClass:UIView.class] ? (UIView *)layer.delegate : nil;
		if (owner.window) continue;
		[layer removeObserver:rygObserver forKeyPath:@"opacity"];
		[rygObservedLayers removeObject:layer];
	}
...
// hidden for play/unpause (IG unhides on next pause), layer.opacity for zoom/KVO (we restore).
...
// layer.opacity, restored on zoom end.
...
// Photo cells skip IG's tap delegate and mute via the hardware-switch notification.
...
// Carousels route the tap through the outer cell; video pages keep %orig.
...
// Pause/play mode hides the mute sub-toggle and keeps only the play/pause button.
...
// swift__currentAudioCell is the Swift form, kept in case the bridged selector stops registering.
