...
static BOOL sciIsPausePlayMode(void) {
...
static BOOL sciBool(id obj, SEL sel) {
...
static void sciSetupKVO(UIView *ufi) {
	...
	if (!sciObserver) sciObserver = [SCIOpacityObserver new];
	...
	[dlBtn.layer addObserver:sciObserver forKeyPath:@"opacity" options:NSKeyValueObservingOptionNew context:NULL];
...
static void sciHidePlayView(id cell) {
	...
	UIView *indicator = sciIvar(cell, "_playPauseMediaIndicator");
...
static void sciForceUnmuteCell(id cell) {
	if (!cell) return;
	...
	if (mediaPk && [sciNoAudioMediaIds containsObject:mediaPk]) return;

	if ([cell respondsToSelector:@selector(hasAudio)] && !sciBool(cell, @selector(hasAudio))) {
		if (mediaPk) {
			if (!sciNoAudioMediaIds) sciNoAudioMediaIds = [NSMutableSet new];
			[sciNoAudioMediaIds addObject:mediaPk];
		}
		return;
	}

	if ([cell respondsToSelector:@selector(isAudioEnabled)] && sciBool(cell, @selector(isAudioEnabled))) return;
...
- (void)sundialVideoPlaybackView:(id)pbView didEndZoomInteractionForView:(id)view withLogging:(id)logging minScale:(CGFloat)minScale {
	...
	dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.3 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
		...
		if (strongSelf && sciBool(strongSelf, @selector(isPlaying))) sciHidePlayView(strongSelf);
	});
}
...
- (void)gestureController:(id)gc didObserveSingleTap:(id)tap {
	...
	UICollectionView *cv = sciIvar(self, "_collectionView");
...
static id sciCurrentAudioCell(id feedVC) {
...
static void sciForceCurrentFeedAudio(id feedVC) {
	if (!sciIsPausePlayMode() || !sciIsInReelsTab) return;

	id cell = sciCurrentAudioCell(feedVC);
...
- (void)viewDidAppear:(BOOL)animated {
	...
	for (NSUInteger i = 0; i < sizeof(delays) / sizeof(delays[0]); i++) {
		...
		dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(delay * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
			sciForceCurrentFeedAudio(weakSelf);
		});
	}
}
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView {
	...
	sciForceCurrentFeedAudio(self);
}

- (void)scrollViewDidEndDragging:(UIScrollView *)scrollView willDecelerate:(BOOL)decelerate {
	%orig;
	if (!decelerate) sciForceCurrentFeedAudio(self);
}
