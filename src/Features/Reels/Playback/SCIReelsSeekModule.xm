...
static inline BOOL sciSeekEnabled(void) {
	return [SCIUtils getBoolPref:@"reels_playback_seek"];
}

static double sciSeekStep(void) {
	double v = [SCIUtils getDoublePref:@"reels_playback_seek_step"];
	if (v < 1.0 || v > 600.0) v = 10.0;
	return v;
}

static void sciStoreSeekStep(double step) {
	if (step < 1.0) step = 1.0;
	if (step > 600.0) step = 600.0;
	[[NSUserDefaults standardUserDefaults] setDouble:step forKey:@"reels_playback_seek_step"];
}

static SEL sciCellSeekSel(void) {
...
static id sciActiveReelCell(void) {
	id c = sCurrentReelCell;
	if (c && [c respondsToSelector:sciCellSeekSel()]
		&& [c isKindOfClass:[UIView class]] && ((UIView *)c).window) return c;
	return [SCIReelsPlaybackMenu capturedReelCell];
}
...
static void sciSeekBy(double delta) {
	id cell = sciActiveReelCell();
	if (!cell || ![cell respondsToSelector:sciCellSeekSel()]) return;
	...
	@try {
		((void(*)(id, SEL, double, BOOL, NSInteger, BOOL, id))objc_msgSend)
			(cell, sciCellSeekSel(), target, YES, 0, NO, nil);
	} @catch (__unused id e) {}
...
@interface SCIReelsSeekView : UIView
@end

@implementation SCIReelsSeekView {
...
- (instancetype)init {
	if ((self = [super initWithFrame:CGRectZero])) {
		...
		_customPill = [self _pillWithTitle:SCILocalized(@"Custom") action:@selector(_onCustomTap)];
...
- (void)_refresh {
	...
	[_customPill setTitle:isPreset ? SCILocalized(@"Custom")
...
- (void)_onPresetTap:(UIButton *)sender {
	sciStoreSeekStep((double)sender.tag / 10.0);
	[self _refresh];
...
- (void)_onCustomTap {
	...
	UIAlertController *ac = [UIAlertController
		alertControllerWithTitle:SCILocalized(@"Custom seek step")
						 message:SCILocalized(@"Enter the number of seconds to skip")
				  preferredStyle:UIAlertControllerStyleAlert];
	[ac addTextFieldWithConfigurationHandler:^(UITextField *tf) {
		tf.keyboardType = UIKeyboardTypeDecimalPad;
		tf.text = [NSString stringWithFormat:@"%g", sciSeekStep()];
		tf.clearButtonMode = UITextFieldViewModeAlways;
	}];
	__weak SCIReelsSeekView *ws = self;
...
- (void)_onBack { sciSeekBy(-sciSeekStep()); }
- (void)_onForward { sciSeekBy(sciSeekStep()); }
...
%ctor {
	[SCIReelsPlaybackMenu registerModuleWithID:@"seek"
		isOn:^BOOL { return sciSeekEnabled(); }
		buildSection:^UIView *{
			return [[SCIReelsPlaybackSection alloc] initWithTitle:SCILocalized(@"Seek")
														 content:[SCIReelsSeekView new]];
		}];
}
