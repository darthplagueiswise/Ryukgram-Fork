...
static char kRYGReelsSwipePanKey;

static const CGFloat kRYGReelsSwipeBottomIgnore = 140.0;
static const NSTimeInterval kRYGReelsSwipeMaxDuration = 0.40;

static BOOL rygReelsSwipeEnabled(void) {
	return [RYGUtils getBoolPref:@"reels_swipe_to_profile"];
}

static UIView *rygDeepFindByClass(UIView *root, Class cls) {
	if (!root || !cls) return nil;
	if ([root isKindOfClass:cls]) return root;

	for (UIView *sub in root.subviews) {
		UIView *hit = rygDeepFindByClass(sub, cls);
		if (hit) return hit;
	}
...
static UIView *rygCurrentReelCell(UIViewController *vc) {
	if (!vc) return nil;
...
static NSAttributedString *rygAttributedStringFromCoreTextView(id core) {
	if (!core) return nil;
...
static BOOL rygFirstURL(NSAttributedString *as, NSURL **outURL, NSString **outString) {
...
static BOOL rygTryCoauthorTap(UIView *cell) {
	...
	UIView *attrib = rygDeepFindByClass(cell, textCls);
	if (!attrib) return NO;
	...
	if (!rygFirstURL(rygAttributedStringFromCoreTextView(core), &url, &text)) return NO;
...
static BOOL rygTryHeaderButtonTap(UIView *cell) {
	Class buttonCls = %c(IGUnifiedVideoUserButton);
	UIView *button = rygDeepFindByClass(cell, buttonCls);
	if (!button) return NO;
...
static BOOL rygTriggerHeaderUsernameTap(UIViewController *vc) {
	UIView *cell = rygCurrentReelCell(vc);
	if (!cell) return NO;

	return rygTryHeaderButtonTap(cell) || rygTryCoauthorTap(cell);
}

// Carousel reels use horizontal paging, so don't hijack left swipes there.
static BOOL rygCurrentReelIsCarousel(UIViewController *vc) {
	UIView *cell = rygCurrentReelCell(vc);
	if (!cell) return NO;
...
static void rygCoupleParentPanRecognizers(UIView *view, UIPanGestureRecognizer *ourPan) {
...
@interface RYGReelsSwipeToProfileDelegate : NSObject <UIGestureRecognizerDelegate>
...
@implementation RYGReelsSwipeToProfileDelegate

- (BOOL)gestureRecognizerShouldBegin:(UIPanGestureRecognizer *)gr {
	if (!rygReelsSwipeEnabled()) return NO;
	...
	if (loc.y > gr.view.bounds.size.height - kRYGReelsSwipeBottomIgnore) return NO;

	if (rygCurrentReelIsCarousel(self.owner)) return NO;
...
- (void)handle:(UIPanGestureRecognizer *)gr {
	...
	if (CACurrentMediaTime() - self.startTime > kRYGReelsSwipeMaxDuration) return;
	...
	rygTriggerHeaderUsernameTap(self.owner);
}
...
- (void)viewDidLoad {
	...
	if (!rygReelsSwipeEnabled()) return;
	if (objc_getAssociatedObject(self, &kRYGReelsSwipePanKey)) return;

	RYGReelsSwipeToProfileDelegate *delegate = [RYGReelsSwipeToProfileDelegate new];
	delegate.owner = self;
	...
	objc_setAssociatedObject(self, &kRYGReelsSwipePanKey, delegate, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}

- (void)viewDidAppear:(BOOL)animated {
	%orig;

	RYGReelsSwipeToProfileDelegate *delegate = objc_getAssociatedObject(self, &kRYGReelsSwipePanKey);
	if (!delegate.pan || delegate.coupled) return;

	rygCoupleParentPanRecognizers(self.view, delegate.pan);
