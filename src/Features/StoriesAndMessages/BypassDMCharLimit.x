...
static inline BOOL rygBypassEnabled(void) {
    return [RYGUtils getBoolPref:@"bypass_dm_char_limit"];
}

static void rygBumpCharacterLimit(id composer) {
    if (!composer) return;
...
- (void)didMoveToWindow {
    %orig;
    if (rygBypassEnabled() && self.window) rygBumpCharacterLimit(self);
}

- (BOOL)textView:(UITextView *)textView shouldChangeTextInRange:(NSRange)range replacementText:(NSString *)text {
    if (rygBypassEnabled()) rygBumpCharacterLimit(self);
    return %orig;
}
