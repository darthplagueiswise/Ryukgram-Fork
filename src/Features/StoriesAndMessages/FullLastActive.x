...
static NSDateFormatter *rygDMDateFormatter(void) {
...
static NSDate *rygLastActiveDateForTitleView(UIView *titleView) {
...
static void rygRewriteSubtitle(UIView *titleView) {
    if (![RYGUtils getBoolPref:@"dm_full_last_active"]) return;
    ...
    NSDate *date = rygLastActiveDateForTitleView(titleView);
    if (!date) return;
    ...
    @try {
        ...
        NSString *formatted = [rygDMDateFormatter() stringFromDate:date];
