...
+ (void)recordThreadId:(NSString *)threadId coveredTs:(double)ts pk:(NSString *)pk {
	...
	if ([existing isKindOfClass:NSDictionary.class] && [existing[@"ts"] doubleValue] >= ts) return;
	...
	for (NSString *key in stored) {
		...
		if ([e isKindOfClass:NSDictionary.class] && now - [e[@"at"] doubleValue] < kRYGDMLocalSeenTTL) entries[key] = e;
	}
...
+ (double)localSeenTsForThreadId:(NSString *)threadId pk:(NSString *)pk {
	...
	return [e isKindOfClass:NSDictionary.class] ? [e[@"ts"] doubleValue] : 0;
}
...
+ (void)recordServerSeenThreadId:(NSString *)threadId ts:(double)ts pk:(NSString *)pk {
	...
	NSDictionary *existing = stored[threadId];
	if ([existing isKindOfClass:NSDictionary.class] && [existing[@"ts"] doubleValue] + 2.0 >= ts) return;
