...
static id sciCurrentStorySectionController(UIViewController *vc) {
	return sciSafeCall0(vc, @selector(currentlyDisplayedSectionController));
}
...
static void sciAdvance(NSString *prefKey, NSTimeInterval delay) {
	...
	id ctrl = sciCurrentStorySectionController(vc);
	if (!ctrl) return;
	...
	dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(delay * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
		...
		dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.2 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
			id c2 = vc ? sciCurrentStorySectionController(vc) : nil;
