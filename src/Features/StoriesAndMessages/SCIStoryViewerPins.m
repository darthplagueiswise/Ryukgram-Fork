...
#define SCI_STORY_PINS_KEY @"story_pinned_viewers"
...
@implementation SCIStoryViewerPins

+ (void)primeCache {
    NSString *scoped = [SCIAccountScopedDefaults scopedKey:SCI_STORY_PINS_KEY];
    if (gCacheArr && [gCacheKey isEqualToString:scoped]) return;
    NSArray *a = [SCIAccountScopedDefaults arrayForKey:SCI_STORY_PINS_KEY];
...
+ (void)saveAll:(NSArray *)entries {
    [SCIAccountScopedDefaults setObject:(entries ?: @[]) forKey:SCI_STORY_PINS_KEY];
