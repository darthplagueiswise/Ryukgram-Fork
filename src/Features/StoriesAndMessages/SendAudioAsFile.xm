...
static void sciPrepareAndShowTrim(NSURL *url, UIViewController *threadVC) {
	...
	dispatch_async(dispatch_get_global_queue(QOS_CLASS_USER_INITIATED, 0), ^{
		...
		if (!exporter) {
			...
			[SCITempFiles releaseURL:outURL];
			dispatch_async(dispatch_get_main_queue(), ^{
				if ([SCIFFmpeg isAvailable]) sciFFmpegPreConvertForTrim(url, threadVC);
				else sciShowUnsupportedAlert(url, SCILocalized(@"Format not supported without FFmpegKit"), threadVC);
			});
			return;
		}
		exporter.outputURL = outURL;
