...
static NSArray<NSString *> *sciSSPinList(void) {
    NSArray *a = [SCIAccountScopedDefaults arrayForKey:SSPIN_KEY];
...
static void sciSSPinSave(NSArray<NSString *> *list) {
    [SCIAccountScopedDefaults setObject:(list ?: @[]) forKey:SSPIN_KEY];
}
...
- (void)viewDidLoad {
    %orig;
    if (![SCIUtils getBoolPref:@"share_sheet_pin_threads"]) return;
