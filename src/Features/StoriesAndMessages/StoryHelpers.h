...
typedef id (*RYGMsgSend0)(id, SEL);
typedef id (*RYGMsgSend1)(id, SEL, id);

static inline id rygCall(id obj, SEL sel) {
	if (!obj || !sel || ![obj respondsToSelector:sel]) return nil;

	@try {
		return ((RYGMsgSend0)objc_msgSend)(obj, sel);
	} @catch (__unused id e) {
		return nil;
	}
}

static inline id rygCall1(id obj, SEL sel, id arg) {
	if (!obj || !sel || ![obj respondsToSelector:sel]) return nil;

	@try {
		return ((RYGMsgSend1)objc_msgSend)(obj, sel, arg);
	} @catch (__unused id e) {
		return nil;
	}
}

static inline UIViewController *rygFindVC(UIResponder *start, NSString *className) {
...
static inline IGMedia *rygExtractMediaFromItem(id item) {
	if (!item) return nil;
	...
	for (NSString *name in @[@"media", @"mediaItem", @"storyItem", @"item", @"feedItem", @"igMedia", @"model", @"backingModel", @"storyMedia", @"mediaModel"]) {
		id value = rygCall(item, NSSelectorFromString(name));
