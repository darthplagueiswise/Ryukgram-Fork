...
static void sciPauseStoryPlayback(UIView *sourceView) {
	UIViewController *storyVC = sciStoryVCForView(sourceView);
	if (!storyVC) return;
	SEL withCtx = @selector(pauseWithReason:pauseReasonDebugString:callsiteContext:);
	if ([storyVC respondsToSelector:withCtx]) {
		((void (*)(id, SEL, NSInteger, id, id))objc_msgSend)(storyVC, withCtx, 10, @"scinsta", nil);
		return;
	}
	SEL ctx = @selector(pauseWithReason:callsiteContext:);
	if ([storyVC respondsToSelector:ctx]) {
		((void (*)(id, SEL, NSInteger, id))objc_msgSend)(storyVC, ctx, 10, nil);
		return;
	}
...
static void sciResumeStoryPlayback(UIView *sourceView) {
	UIViewController *storyVC = sciStoryVCForView(sourceView);
	if (!storyVC) return;
	SEL resumeReason = @selector(tryResumePlaybackWithReason:);
	if ([storyVC respondsToSelector:resumeReason]) {
		((void (*)(id, SEL, NSInteger))objc_msgSend)(storyVC, resumeReason, 10);
		return;
	}
...
static BOOL sciAnyContextMenuVisible(void) {
	for (UIScene *scene in [UIApplication sharedApplication].connectedScenes) {
		...
		for (UIWindow *win in ((UIWindowScene *)scene).windows) {
			...
			while (q.count) {
				UIView *cur = q.firstObject; [q removeObjectAtIndex:0];
				NSString *cls = NSStringFromClass([cur class]);
				if ([cls containsString:@"ContextMenuContainerView"]
				    || [cls containsString:@"_UIContextMenuView"]
				    || [cls containsString:@"ContextMenuPlatterView"])
					return YES;
...
static void SCIConfigureStoryActionButton(SCIChromeButton *button) {
	...
	SCIActionMediaProvider provider = ^id (UIView *sourceView) {
		...
		// Coalesce sciCurrentStoryMedia's kCFNull sentinel — menu resolver doesn't guard against it.
		id media = sciCurrentStoryMedia(sourceView);
...
// Music attribution (highlights) is an IGTapButton in IG's sticker layer that
// overlaps our column. Returns how far up to shift to clear it (0 = no collision).
static CGFloat sciStoryMusicClearance(UIView *overlay) {
	...
	static Class tapCls; static dispatch_once_t once;
	dispatch_once(&once, ^{ tapCls = NSClassFromString(@"IGTapButton"); });
	if (!tapCls) return 0;
	...
	while (stack.count) {
		...
		if (v.bounds.size.width > 120.0 || v.bounds.size.height > 120.0) continue; // sticker-sized only
...
- (void)sciApplyMusicButtonClearance {
	...
	if (!bottoms) return; // no buttons — skip the tree walk
...
- (void)sciRefreshStoryActionButton {
	...
	NSLayoutConstraint *actionBottom = [button.bottomAnchor constraintEqualToAnchor:self.safeAreaLayoutGuide.bottomAnchor constant:kStoryBottomBaseOffset];
	actionBottom.identifier = kStoryBottomConstraintID;
	[NSLayoutConstraint activateConstraints:@[
		actionBottom,
...
- (void)sciRefreshStoryAudioButton {
	...
	NSLayoutConstraint *audioBottom = [button.bottomAnchor constraintEqualToAnchor:self.safeAreaLayoutGuide.bottomAnchor constant:kStoryBottomBaseOffset];
	audioBottom.identifier = kStoryBottomConstraintID;
	[NSLayoutConstraint activateConstraints:@[
		audioBottom,
...
- (void)sciRefreshSeenButton {
	...
	if (![button isKindOfClass:SCIChromeButton.class]) {
		...
		if (anchor) {
			...
			NSLayoutConstraint *eyeBottom = [button.bottomAnchor constraintEqualToAnchor:self.safeAreaLayoutGuide.bottomAnchor constant:kStoryBottomBaseOffset];
			eyeBottom.identifier = kStoryBottomConstraintID;
			[NSLayoutConstraint activateConstraints:@[
				eyeBottom,
				[button.trailingAnchor constraintEqualToAnchor:self.trailingAnchor constant:-12.0],
				[button.widthAnchor constraintEqualToConstant:36.0],
...
- (void)sciRefreshStoryMentionsButton {
	...
	NSLayoutConstraint *mentionsBottom = [button.bottomAnchor constraintEqualToAnchor:self.safeAreaLayoutGuide.bottomAnchor constant:kStoryBottomBaseOffset];
	mentionsBottom.identifier = kStoryBottomConstraintID;
	[NSLayoutConstraint activateConstraints:@[
		mentionsBottom,
...
static void sciRefreshMentionsInVisibleOverlays(id storyVC) {
	BOOL mentions = [SCIUtils getBoolPref:@"story_mentions_button"];

	for (UIView *overlay in sciLiveStoryOverlays().allObjects) {
		if (!overlay.window || sciOverlayIsInDMContext(overlay)) continue;

		// Music button lives in a sibling subtree that never fires our layoutSubviews.
