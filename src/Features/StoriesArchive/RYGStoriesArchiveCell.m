...
@property (nonatomic, strong) UIView *reactionPill;
@property (nonatomic, strong) UILabel *reactionLabel;
@property (nonatomic, strong) UIImageView *reactionHeart;
...
- (instancetype)initWithFrame:(CGRect)frame {
	...
	_reactionPill = [self pill];
	_reactionPill.hidden = YES;
	_reactionLabel = [UILabel new];
	_reactionLabel.font = [UIFont systemFontOfSize:12];
	_reactionLabel.textAlignment = NSTextAlignmentCenter;
	_reactionLabel.translatesAutoresizingMaskIntoConstraints = NO;
	[_reactionPill addSubview:_reactionLabel];
	_reactionHeart = [self statIcon:@"ig_icon_heart_filled_24" fallback:@"heart.fill" color:UIColor.systemRedColor box:12];
	_reactionHeart.translatesAutoresizingMaskIntoConstraints = NO;
	[_reactionPill addSubview:_reactionHeart];
	[self.contentView addSubview:_reactionPill];

	[NSLayoutConstraint activateConstraints:@[
		[_reactionPill.bottomAnchor constraintEqualToAnchor:self.contentView.bottomAnchor constant:-6],
		[_reactionPill.trailingAnchor constraintEqualToAnchor:self.contentView.trailingAnchor constant:-6],
		[_reactionPill.widthAnchor constraintEqualToConstant:22],
		[_reactionPill.heightAnchor constraintEqualToConstant:20],
		[_reactionLabel.centerXAnchor constraintEqualToAnchor:_reactionPill.centerXAnchor],
		[_reactionLabel.centerYAnchor constraintEqualToAnchor:_reactionPill.centerYAnchor],
		[_reactionHeart.centerXAnchor constraintEqualToAnchor:_reactionPill.centerXAnchor],
		[_reactionHeart.centerYAnchor constraintEqualToAnchor:_reactionPill.centerYAnchor],
	]];
...
- (void)configureReaction:(NSString *)emoji liked:(BOOL)liked {
	BOOL hasEmoji = emoji.length > 0;
	self.reactionPill.hidden = !(hasEmoji || liked);
	self.reactionLabel.text = hasEmoji ? emoji : nil;
	self.reactionLabel.hidden = !hasEmoji;
	self.reactionHeart.hidden = hasEmoji;
}
