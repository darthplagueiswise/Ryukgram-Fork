...
- (NSManagedObjectModel *)buildModel {
	...
	[storyProps addObject:[self attr:@"viewersFinal" type:NSBooleanAttributeType optional:NO default:@NO]];
...
- (instancetype)initWithPK:(NSString *)pk {
	...
	[self reconcileStoredCounts];
	return self;
}

// Viewer rows are the record and only ever grow; the counts cache them.
- (void)reconcileStoredCounts {
	[self performBackground:^(NSManagedObjectContext *ctx) {
		NSFetchRequest *r = [NSFetchRequest fetchRequestWithEntityName:kStoryEntity];
		r.relationshipKeyPathsForPrefetching = @[@"viewers"];
		r.includesSubentities = NO;
		BOOL repaired = NO;
		for (RYGArchivedStory *story in [ctx executeFetchRequest:r error:nil])
			if ([self applyCountsFromViewers:story.viewers toStory:story]) repaired = YES;
		if (!repaired) return;
...
- (BOOL)applyCountsFromViewers:(id<NSFastEnumeration>)viewers toStory:(RYGArchivedStory *)story {
	int64_t count = 0, likes = 0, reactions = 0;
	for (RYGArchivedStoryViewer *v in viewers) {
		count++;
		if (v.liked) likes++;
		if (v.reactionEmoji.length) reactions++;
	}
	if (story.viewersCount == count && story.likesCount == likes && story.reactionsCount == reactions) return NO;
	story.viewersCount = count;
...
- (void)notifyPinnedEvents:(NSArray<NSDictionary *> *)events {
	...
	if (events.count == 1) {
		...
		NSString *username = RYGJSONString(e[@"username"]);
		NSString *u = username.length ? [@"@" stringByAppendingString:username] : RYGLocalized(@"A pinned viewer");
		NSString *title, *subtitle = u;
		if ([RYGJSONScalar(e[@"reacted"]) boolValue]) {
			title = RYGLocalized(@"Pinned viewer reacted to your story");
			NSString *emoji = RYGJSONString(e[@"emoji"]);
			if (emoji.length) subtitle = [NSString stringWithFormat:@"%@ %@", u, emoji];
		} else if ([RYGJSONScalar(e[@"liked"]) boolValue]) {
...
- (void)markViewersFinalForMediaID:(NSString *)mediaID {
	...
	[self performBackground:^(NSManagedObjectContext *ctx) {
		...
		if (!story || story.viewersFinal) return;
		story.viewersFinal = YES;
		[ctx save:nil];
	}];
}
...
- (NSArray<NSString *> *)mediaIDsNeedingViewerRefreshBefore:(NSDate *)cutoff {
	...
	[ctx performBlockAndWait:^{
		...
		NSFetchRequest *r = [NSFetchRequest fetchRequestWithEntityName:kStoryEntity];
		r.predicate = [NSPredicate predicateWithFormat:@"takenAt >= %@ AND viewersFinal == NO AND (lastViewersFetch == nil OR lastViewersFetch < %@)", lo, cutoff];
...
// A 0-viewer snapshot still records lastViewersFetch so the window doesn't re-poll it.
- (void)saveViewers:(NSArray<NSDictionary *> *)viewers
	...
	[self performBackground:^(NSManagedObjectContext *ctx) {
		RYGArchivedStory *story = [self storyWithMediaID:mediaID inContext:ctx];
		if (!story) return;
		// An aged-out story returns an empty list rather than an error. Keep the
		// snapshot we already have and stop polling for it.
		if (!viewers.count && story.viewers.count) {
			if (!story.viewersFinal) { story.viewersFinal = YES; [self saveContext:ctx]; }
			return;
		}
		...
		for (NSDictionary *d in viewers) {
			...
			if (notifyPinned && hadPriorFetch && [RYGStoryViewerPins isPinned:pk]) {
				...
				BOOL nowLiked = [RYGJSONScalar(d[@"liked"]) boolValue];
				BOOL newlyLiked = nowLiked && ![wasLiked[pk] boolValue];
				NSString *emoji = RYGJSONString(d[@"reaction_emoji"]);
				if (!emoji.length) emoji = nil;
				BOOL newlyReacted = emoji && ![emoji isEqualToString:(wasReaction[pk] ?: @"")];
				if (!existedBefore || newlyLiked || newlyReacted)
					[pinnedEvents addObject:@{ @"username": RYGJSONString(d[@"username"]) ?: @"", @"liked": @(newlyLiked), @"reacted": @(newlyReacted), @"emoji": emoji ?: @"" }];
			}
			...
			v.username = RYGJSONString(d[@"username"]);
			v.fullName = RYGJSONString(d[@"full_name"]);
			v.profilePicURL = RYGJSONString(d[@"profile_pic_url"]);
			v.isVerified = [RYGJSONScalar(d[@"is_verified"]) boolValue];
			v.liked = [RYGJSONScalar(d[@"liked"]) boolValue];
			v.following = [RYGJSONScalar(d[@"following"]) boolValue];
			v.followedBy = [RYGJSONScalar(d[@"followed_by"]) boolValue];
			NSString *reaction = RYGJSONString(d[@"reaction_emoji"]);
			v.reactionEmoji = reaction.length ? reaction : nil;
		}

		[self applyCountsFromViewers:existing.allValues toStory:story];
...
- (void)applyViewerIdentities:(NSDictionary<NSString *, NSDictionary *> *)byPK {
	if (!byPK.count) return;
	[self performBackground:^(NSManagedObjectContext *ctx) {
		NSFetchRequest *r = [NSFetchRequest fetchRequestWithEntityName:kViewerEntity];
		r.predicate = [NSPredicate predicateWithFormat:@"pk IN %@", byPK.allKeys];
		BOOL changed = NO;
		for (RYGArchivedStoryViewer *v in [ctx executeFetchRequest:r error:nil]) {
			NSDictionary *info = byPK[v.pk];
			if (![info isKindOfClass:NSDictionary.class]) continue;
			NSString *username = RYGJSONString(info[@"username"]);
			NSString *fullName = RYGJSONString(info[@"full_name"]);
			NSString *pic = RYGJSONString(info[@"profile_pic_url"]);
			if (username.length && ![username isEqualToString:v.username ?: @""]) { v.username = username; changed = YES; }
			if (fullName && ![fullName isEqualToString:v.fullName ?: @""]) { v.fullName = fullName; changed = YES; }
			if (pic.length && ![pic isEqualToString:v.profilePicURL ?: @""]) { v.profilePicURL = pic; changed = YES; }
			BOOL verified = [RYGJSONScalar(info[@"is_verified"]) boolValue];
			if (verified != v.isVerified) { v.isVerified = verified; changed = YES; }
		}
		if (!changed) return;
		[self saveContext:ctx];
		[self postChange];
...
- (NSArray<RYGArchivedStory *> *)allStoriesSortedByDateDescending {
	return [self storiesSortedByDateDescendingInContext:self.viewContext];
}
...
- (NSArray<RYGArchivedStory *> *)storiesSortedByDateDescendingInContext:(NSManagedObjectContext *)ctx {
	...
	r.sortDescriptors = @[[NSSortDescriptor sortDescriptorWithKey:@"takenAt" ascending:NO]];
	r.includesSubentities = NO;
	return [ctx executeFetchRequest:r error:nil] ?: @[];
}

- (NSArray<RYGArchivedStoryViewer *> *)viewerRowsForUserPK:(NSString *)pk {
	if (!pk.length) return @[];
	NSFetchRequest *r = [NSFetchRequest fetchRequestWithEntityName:kViewerEntity];
	r.predicate = [NSPredicate predicateWithFormat:@"pk == %@", pk];
	r.sortDescriptors = @[[NSSortDescriptor sortDescriptorWithKey:@"story.takenAt" ascending:NO]];
	r.relationshipKeyPathsForPrefetching = @[@"story"];
	return [self.viewContext executeFetchRequest:r error:nil] ?: @[];
}
...
// Raw sqlite survives model drift. Local rows win; the import only adds what's missing.
