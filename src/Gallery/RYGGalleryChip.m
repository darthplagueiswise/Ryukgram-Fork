...
static UIImage *RYGGalleryChipGlyph(NSString *name, CGFloat pointSize) {
	if (name.length == 0) return nil;
	return [RYGAssetUtils instagramIconNamed:name pointSize:pointSize];
}

@implementation RYGGalleryChip
...
+ (instancetype)chipWithTitle:(NSString *)title
	...
	RYGGalleryChip *chip = [RYGGalleryChip buttonWithType:UIButtonTypeSystem];
	...
	if (@available(iOS 15.0, *)) {
		...
		cfg.title = title ?: @"";
		if (sfSymbol.length) cfg.image = RYGGalleryChipGlyph(sfSymbol, ceil(fontSize * 1.7));
		...
		if (sfSymbol.length) [chip setImage:RYGGalleryChipGlyph(sfSymbol, ceil(fontSize * 1.7)) forState:UIControlStateNormal];
...
- (void)applyAppearance {
	if (self.isOnState) {
		UIColor *primary = [RYGUtils RYGColor_Primary];
