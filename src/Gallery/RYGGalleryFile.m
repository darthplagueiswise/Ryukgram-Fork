...
static NSSet<NSString *> *RYGGalleryAudioExts(void) {
...
BOOL RYGGalleryExtensionIsAudio(NSString *ext) {
	...
	return [RYGGalleryAudioExts() containsObject:ext.lowercaseString];
}
...
RYGGalleryMediaType RYGGalleryMediaTypeForExtension(NSString *ext) {
	...
	if ([RYGGalleryAudioExts() containsObject:e]) return RYGGalleryMediaTypeAudio;
	static NSSet<NSString *> *videoExts;
	static dispatch_once_t once;
	dispatch_once(&once, ^{ videoExts = [NSSet setWithArray:@[ @"mp4", @"mov", @"m4v", @"webm" ]]; });
	if ([videoExts containsObject:e]) return RYGGalleryMediaTypeVideo;
	return RYGGalleryMediaTypeImage;
}

static NSString *RYGGalleryNormalizedExtension(NSString * _Nullable origExt, RYGGalleryMediaType mediaType) {
	...
	NSSet<NSString *> *audioExts = RYGGalleryAudioExts();
...
static NSString *RYGGallerySourceSlug(RYGGallerySource source) {
	switch (source) {
		...
		case RYGGallerySourceDMs:	 return @"dms";
...
static NSString *RYGSanitizedGalleryUsername(NSString *raw) {
	if (!raw.length) {
		return @"";
	}
...
static BOOL RYGStringLooksLikeUUIDFilename(NSString *baseName) {
...
static BOOL RYGStringLooksLikeScratchFilename(NSString *baseName) {
	return [baseName hasPrefix:@"ryuk_tmp_"] || [baseName hasPrefix:@"ryg_tmp_"];
}

NSString *RYGFileNameForMedia(NSURL *fileURL,
							  RYGGalleryMediaType mediaType,
							  RYGGallerySaveMetadata * _Nullable metadata) {
	NSString *orig = fileURL.lastPathComponent ?: @"";
	NSString *origExt = orig.pathExtension;
	NSString *ext = RYGGalleryNormalizedExtension(origExt, mediaType);
	...
	RYGGallerySource src = metadata ? (RYGGallerySource)metadata.source : RYGGallerySourceOther;
	NSString *slug = RYGGallerySourceSlug(src);

	if (metadata.sourceUsername.length > 0) {
		NSString *user = RYGSanitizedGalleryUsername(metadata.sourceUsername);
		return [NSString stringWithFormat:@"%@_%@_%@.%@", user, slug, dateCompact, ext];
	}

	NSString *base = [orig stringByDeletingPathExtension];
	if (RYGStringLooksLikeUUIDFilename(base) || RYGStringLooksLikeScratchFilename(base) || base.length == 0) {
...
+ (RYGGalleryFile *)saveFileToGallery:(NSURL *)fileURL
    ...
    void (^body)(void) = ^{
        ...
        NSString *fileName = RYGFileNameForMedia(fileURL, mediaType, metadata);
        NSString *destPath = [[RYGGalleryPaths galleryMediaDirectory] stringByAppendingPathComponent:fileName];
        ...
        if ([fm fileExistsAtPath:destPath]) {
            ...
            for (int n = 1; n < 100; n++) {
                ...
                NSString *candidatePath = [[RYGGalleryPaths galleryMediaDirectory] stringByAppendingPathComponent:candidate];
