...
@interface RYGFolderThumbCollage : UIView
...
@implementation RYGFolderThumbCollage
...
- (instancetype)initWithFrame:(CGRect)frame {
	if ((self = [super initWithFrame:frame])) {
		...
		_placeholder = [[UIImageView alloc] initWithImage:[RYGAssetUtils instagramIconNamed:@"folder" pointSize:22.0]];
...
- (void)setPlaceholderSymbol:(NSString *)symbolName {
	_placeholder.image = [RYGAssetUtils instagramIconNamed:symbolName pointSize:22.0];
}
...
#pragma mark - RYGGalleryFolderCell

@interface RYGGalleryFolderCell ()

@property (nonatomic, assign) RYGGalleryFolderCellLayout currentLayout;
@property (nonatomic, strong) UIView *listSeparator;

// List-mode views
@property (nonatomic, strong) UIView *listContainer;
@property (nonatomic, strong) RYGFolderThumbCollage *listCollage;
...
@property (nonatomic, strong) RYGFolderThumbCollage *gridCollage;
...
@implementation RYGGalleryFolderCell
...
- (instancetype)initWithFrame:(CGRect)frame {
	if ((self = [super initWithFrame:frame])) {
		...
		[self buildListViews];
		[self buildGridViews];
		_currentLayout = RYGGalleryFolderCellLayoutList;
...
- (void)buildListViews {
	...
	_listCollage = [RYGFolderThumbCollage new];
	...
	_listChevron = [[UIImageView alloc] initWithImage:[RYGAssetUtils instagramIconNamed:@"chevron_right" pointSize:13.0]];
...
- (void)buildGridViews {
	...
	_gridCollage = [RYGFolderThumbCollage new];
	...
	UIImageView *badgeIcon = [[UIImageView alloc] initWithImage:[RYGAssetUtils instagramIconNamed:@"folder" pointSize:12.0]];
...
- (void)configureWithFolderName:(NSString *)name
	...
	BOOL isGrid = (layoutMode == RYGGalleryFolderCellLayoutGrid);

	self.currentLayout = layoutMode;
	self.listContainer.hidden = isGrid;
	self.gridContainer.hidden = !isGrid;

	NSString *placeholderSymbol = isUserFolder ? @"profile" : @"folder";
