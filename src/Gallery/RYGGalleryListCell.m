#import "RYGGalleryListCell.h"
#import "RYGGalleryFile.h"
#import "RYGAssetUtils.h"
#import "../Utils.h"
#import "RYGGalleryShim.h"

@interface RYGGalleryListCell ()

@property (nonatomic, strong, readwrite) RYGGalleryFile *file;
...
@implementation RYGGalleryListCell
...
- (void)setupViews {
	...
	self.typeIcon.image = [RYGAssetUtils instagramIconNamed:@"video_filled" pointSize:10.0];
	...
	UIImage *favImg = [RYGAssetUtils instagramIconNamed:@"heart_filled" pointSize:14.0];
...
- (void)configureWithGalleryFile:(RYGGalleryFile *)file {
	...
	BOOL isVideo = (file.mediaType == RYGGalleryMediaTypeVideo);
	BOOL isAudio = (file.mediaType == RYGGalleryMediaTypeAudio);
	BOOL isGIF = (file.mediaType == RYGGalleryMediaTypeGIF);
	if (isAudio) {
		self.typeIcon.hidden = NO;
		self.typeIcon.image = [RYGAssetUtils instagramIconNamed:@"audio" pointSize:10.0];
	} else if (isGIF) {
		self.typeIcon.hidden = NO;
		self.typeIcon.image = [RYGAssetUtils instagramIconNamed:@"gif" pointSize:10.0];
	} else {
		self.typeIcon.hidden = !isVideo;
		if (isVideo) self.typeIcon.image = [RYGAssetUtils instagramIconNamed:@"video" pointSize:10.0];
	}
	self.favoriteIcon.hidden = !file.isFavorite;

	UIImage *thumb = [RYGGalleryFile loadThumbnailForFile:file];
	if (thumb) {
		...
		[RYGGalleryFile generateThumbnailForFile:file completion:^(BOOL ok) {
			...
			UIImage *img = [RYGGalleryFile loadThumbnailForFile:file];
			if (img) weakSelf.thumbnailView.image = img;
		}];
	}
}

- (NSString *)formattedDetailForFile:(RYGGalleryFile *)file {
