...
static CGFloat const kRYGGalleryListRowHeight = 88.0;
static CGFloat const kRYGGalleryThumbSize = 56.0;
static CGFloat const kRYGGalleryThumbLeadingNormal = 8.0;
static CGFloat const kRYGGalleryThumbLeadingSelection = 40.0;

@interface RYGGalleryListCollectionCell ()
...
@implementation RYGGalleryListCollectionCell
...
- (void)setupViews {
	...
	self.favoriteIcon = [[UIImageView alloc] initWithImage:[RYGAssetUtils instagramIconNamed:@"heart_filled" pointSize:14.0]];
	...
	self.favoriteIcon.tintColor = [RYGUtils RYGColor_InstagramFavorite];
	...
	[self.moreButton setImage:[RYGAssetUtils instagramIconNamed:@"more" pointSize:22.0] forState:UIControlStateNormal];
	self.moreButton.tintColor = UIColor.secondaryLabelColor;
	self.moreButton.accessibilityLabel = RYGLocalized(@"More");
...
- (void)setupConstraints {
	...
	self.thumbnailLeadingConstraint = [self.thumbnailView.leadingAnchor constraintEqualToAnchor:margin.leadingAnchor constant:kRYGGalleryThumbLeadingNormal];

	NSLayoutConstraint *height = [self.contentView.heightAnchor constraintEqualToConstant:kRYGGalleryListRowHeight];
	...
	[NSLayoutConstraint activateConstraints:@[
		...
		[self.thumbnailView.widthAnchor constraintEqualToConstant:kRYGGalleryThumbSize],
		[self.thumbnailView.heightAnchor constraintEqualToConstant:kRYGGalleryThumbSize],
...
- (void)prepareForReuse {
	...
	self.thumbnailLeadingConstraint.constant = kRYGGalleryThumbLeadingNormal;
}
...
- (void)updateThumbnailForFile:(RYGGalleryFile *)file token:(NSString *)token {
	...
	[RYGGalleryFile generateThumbnailForFile:file completion:^(BOOL ok) {
		if (!ok) return;
		...
		dispatch_async(dispatch_get_main_queue(), ^{
			...
			UIImage *image = [RYGGalleryFile loadThumbnailForFile:file];
			if (image) self.thumbnailView.image = image;
		});
	}];
}

- (UIImage *)iconForMediaType:(RYGGalleryMediaType)type {
	switch (type) {
		case RYGGalleryMediaTypeVideo:
			return [RYGAssetUtils instagramIconNamed:@"video_filled" pointSize:12.0];

		case RYGGalleryMediaTypeAudio:
			return [RYGAssetUtils instagramIconNamed:@"audio" pointSize:12.0];

		case RYGGalleryMediaTypeGIF:
			return [RYGAssetUtils instagramIconNamed:@"gif" pointSize:12.0];

		case RYGGalleryMediaTypeImage:
		default:
			return [RYGAssetUtils instagramIconNamed:@"photo_filled" pointSize:12.0];
	}
}
...
- (void)setSelectionMode:(BOOL)selectionMode selected:(BOOL)selected animated:(BOOL)animated {
	...
	self.thumbnailLeadingConstraint.constant = selectionMode ? kRYGGalleryThumbLeadingSelection : kRYGGalleryThumbLeadingNormal;
