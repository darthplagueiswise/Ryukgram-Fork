...
static NSDate *RYGDateFromValue(id value) {
	if ([value isKindOfClass:NSDate.class]) return value;

	double seconds = 0.0;
	if ([value isKindOfClass:NSNumber.class]) seconds = [value doubleValue];
	else if ([value isKindOfClass:NSString.class]) seconds = [(NSString *)value doubleValue];
	if (seconds <= 0.0) return nil;

	// IG hands these out in seconds, milliseconds or microseconds depending on the field.
	if (seconds > 1e15) seconds /= 1e6;
	else if (seconds > 1e12) seconds /= 1e3;
	if (seconds < 1e8) return nil;

	return [NSDate dateWithTimeIntervalSince1970:seconds];
}

// Some of these getters return a primitive, which would crash when messaged as `id`.
static id RYGObjectReturningValue(id target, NSString *key) {
	SEL sel = NSSelectorFromString(key);
	if ([target respondsToSelector:sel]) {
		NSMethodSignature *sig = nil;
		@try { sig = [target methodSignatureForSelector:sel]; } @catch (__unused id e) {}
		if (!sig || strcmp(sig.methodReturnType, "@") != 0) return nil;
		return RYGObjectForSelector(target, key);
	}
	return RYGFieldValue(target, key);
}

static NSDate *RYGMediaDateFromMedia(id media, NSInteger depth) {
	if (!media || depth > 3) return nil;

	for (NSString *key in @[@"takenAtDate", @"taken_at", @"device_timestamp",
							@"created_at", @"upload_time", @"published_time"]) {
		NSDate *date = RYGDateFromValue(RYGObjectReturningValue(media, key));
		if (date) return date;
	}
	...
	for (NSString *key in RYGNestedKeys()) {
		...
		NSDate *date = RYGMediaDateFromMedia(nested, depth + 1);
		if (date) return date;
	}
...
+ (void)populateMetadata:(RYGGallerySaveMetadata *)metadata fromMedia:(id)media {
	...
	if (!metadata.mediaDate) metadata.mediaDate = RYGMediaDateFromMedia(media, 0);
