...
@interface RYGGallerySheetViewController () <UIGestureRecognizerDelegate, UIScrollViewDelegate>
...
@property (nonatomic, assign) CGFloat heightBeforeKeyboard;
@property (nonatomic, assign) BOOL keyboardIsUp;
@end

@implementation RYGGallerySheetViewController
...
// Card is pinned to the bottom, so lift it above the keyboard while editing.
- (void)keyboardWillShow:(NSNotification *)note {
	if (!self.didAnimateIn) return;

	CGRect end = [note.userInfo[UIKeyboardFrameEndUserInfoKey] CGRectValue];
	CGFloat kbHeight = CGRectGetHeight([self.view convertRect:end fromView:nil]);
	CGFloat duration = [note.userInfo[UIKeyboardAnimationDurationUserInfoKey] doubleValue];

	if (!self.keyboardIsUp) self.heightBeforeKeyboard = self.cardHeightConstraint.constant;
	self.keyboardIsUp = YES;

	CGFloat available = self.view.bounds.size.height - self.view.safeAreaInsets.top - 12.0 - kbHeight;

	self.cardHeightConstraint.constant = MIN(self.maxHeight, MAX(260.0, available));
	self.cardBottomConstraint.constant = -kbHeight;

	[UIView animateWithDuration:MAX(duration, 0.2) animations:^{ [self.view layoutIfNeeded]; }];
}

- (void)keyboardWillHide:(NSNotification *)note {
	if (!self.didAnimateIn || !self.keyboardIsUp) return;

	self.keyboardIsUp = NO;
	CGFloat duration = [note.userInfo[UIKeyboardAnimationDurationUserInfoKey] doubleValue];

	self.cardBottomConstraint.constant = 0.0;
	self.cardHeightConstraint.constant = [self clampedHeight:(self.heightBeforeKeyboard > 0.0 ? self.heightBeforeKeyboard : self.compactHeight)];
