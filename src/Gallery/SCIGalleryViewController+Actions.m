...
- (NSURL *)sciCopyToTemp:(NSURL *)src {
	...
	NSURL *dst = [SCITempFiles claimWithExt:ext ttl:600 tag:@"gal"];
...
- (UIContextMenuConfiguration *)contextMenuForUserFolder:(NSString *)username {
	...
	return [UIContextMenuConfiguration configurationWithIdentifier:nil
		...
		UIAction *open = [UIAction actionWithTitle:SCILocalized(@"Open")
											 image:SCIGalleryActionIcon(@"folder")
										identifier:nil
										   handler:^(UIAction *a) {
			(void)a;
			__strong typeof(weakSelf) self = weakSelf;
			...
			SCIGalleryViewController *child = [[SCIGalleryViewController alloc] initWithUsernameScope:username];
			[self.navigationController pushViewController:child animated:YES];
		}];

		UIAction *delete = [UIAction actionWithTitle:SCILocalized(@"Delete all files")
...
- (void)confirmDeleteUserFolder:(NSString *)username {
	...
	NSString *message = [NSString stringWithFormat:SCILocalized(@"This will permanently remove %ld file(s)."), (long)count];

	UIAlertController *alert = [UIAlertController alertControllerWithTitle:[NSString stringWithFormat:SCILocalized(@"Delete %@?"), [@"@" stringByAppendingString:username]]
																  message:message
														   preferredStyle:UIAlertControllerStyleAlert];
	...
	[alert addAction:[UIAlertAction actionWithTitle:SCILocalized(@"Delete")
		...
		for (SCIGalleryFile *file in files) [file removeWithError:nil];
