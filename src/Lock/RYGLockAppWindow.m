...
@interface RYGLockAppWindow ()
@property (nonatomic, strong, nullable) UIWindow *window;
@property (nonatomic, strong, nullable) UIViewController *shroudVC;
@property (nonatomic) BOOL inPasscodeMode;
@end

@implementation RYGLockAppWindow

+ (instancetype)shared {
    static RYGLockAppWindow *s; static dispatch_once_t once;
...
- (BOOL)appLockEnabled {
    return [[RYGLockManager shared] isMasterEnabled]
        && [RYGUtils getBoolPref:RYGLockPrefEnabled(RYGLockGroupApp)];
}

- (BOOL)masterEnabled { return [[RYGLockManager shared] isMasterEnabled]; }
...
- (void)presentIfNeeded {
    ...
    if (![[RYGLockManager shared] isGroupLocked:RYGLockGroupApp]) return;
    [self showPasscode];
}
...
- (void)showPasscode {
    ...
    RYGLockPasscodeViewController *pad = [[RYGLockPasscodeViewController alloc]
        initWithTitle:RYGLocalized(@"Unlock Instagram")
    ...
    pad.completion = ^(BOOL ok) {
        if (!ok) return;
        [[RYGLockManager shared] markGroupUnlocked:RYGLockGroupApp];
        [[RYGLockAppWindow shared] dismiss];
    };
...
- (void)resolveOnForeground {
    ...
    if ([[RYGLockManager shared] isGroupLocked:RYGLockGroupApp]) {
        [self showPasscode];
    } else {
        [self dismiss];
    }
