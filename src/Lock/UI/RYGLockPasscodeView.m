...
static const CGFloat kRYGMinButtonSize = 60.0;
static const CGFloat kRYGMaxButtonSize = 74.0;

static const CGFloat kRYGDotSize = 13.0;
static const CGFloat kRYGDotGap = 18.0;

@implementation RYGLockPasscodeView
...
- (void)buildView {
	...
	self.dotsStack.spacing = kRYGDotGap;
...
- (void)updateResponsiveMetrics {
	...
	CGFloat buttonSize = floor(MIN(kRYGMaxButtonSize, MAX(kRYGMinButtonSize, (available - (minGap * 2.0)) / 3.0)));
...
- (void)rebuildDots {
	...
	for (NSInteger i = 0; i < self.codeLength; i++) {
		...
		dot.layer.cornerRadius = kRYGDotSize / 2.0;
		dot.layer.cornerCurve = kCACornerCurveContinuous;
		dot.layer.borderWidth = 1.5;
		dot.layer.borderColor = UIColor.tertiaryLabelColor.CGColor;

		[NSLayoutConstraint activateConstraints:@[
			[dot.widthAnchor constraintEqualToConstant:kRYGDotSize],
			[dot.heightAnchor constraintEqualToConstant:kRYGDotSize],
		]];
...
- (void)refreshDotsAnimated:(BOOL)animated {
	UIColor *filledColor = [RYGUtils RYGColor_Primary] ?: UIColor.labelColor;
...
- (void)refreshBiometricImage {
	NSString *symbolName = [RYGLockManager biometricSymbolName];
	...
	self.biometricButton.tintColor = [RYGUtils RYGColor_Primary] ?: UIColor.labelColor;
}
...
- (void)flashError {
	...
	[self.dotsStack.layer addAnimation:shake forKey:@"ryg.passcode.shake"];
