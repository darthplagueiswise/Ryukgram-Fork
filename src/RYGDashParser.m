...
@implementation RYGDashRepresentation
@end
...
static id rygDashFieldCache(id obj, NSString *key) {
...
@implementation RYGDashParser

static BOOL rygLooksLikeManifest(id val) {
...
static NSString *rygScanDictForManifest(NSDictionary *dict, NSString *path, int depth) {
	if (depth > 3 || ![dict isKindOfClass:[NSDictionary class]]) return nil;
	for (NSString *k in dict) {
		id v = dict[k];
		NSString *lk = k.lowercaseString;
		if (([lk containsString:@"dash"] || [lk containsString:@"manifest"]) && rygLooksLikeManifest(v)) {
			return v;
		}
		if ([v isKindOfClass:[NSDictionary class]]) {
			NSString *found = rygScanDictForManifest(v, [NSString stringWithFormat:@"%@/%@", path, k], depth + 1);
			if (found) return found;
		} else if ([v isKindOfClass:[NSArray class]]) {
			for (id item in (NSArray *)v) {
				if ([item isKindOfClass:[NSDictionary class]]) {
					NSString *found = rygScanDictForManifest(item, [NSString stringWithFormat:@"%@/%@[]", path, k], depth + 1);
					if (found) return found;
				}
			}
		}
...
static NSDictionary *rygFieldCacheDict(id obj) {
	if (!obj) return nil;
	Ivar fcIvar = NULL;
	@try {
		for (Class c = [obj class]; c && !fcIvar; c = class_getSuperclass(c)) {
...
static NSString *rygToManifestString(id val) {
...
+ (NSString *)dashManifestForMedia:(id)media {
	...
	for (NSString *key in keys) {
		id val = rygDashFieldCache(media, key);
		if (rygLooksLikeManifest(val)) return val;
	}
	...
	@try {
		if ([media respondsToSelector:@selector(videoDashManifest)]) {
			...
			NSString *str = rygToManifestString(val);
			if (rygLooksLikeManifest(str)) return str;
		}
	} @catch (__unused id e) {}
	...
	if (video) {
		for (NSString *key in keys) {
			id val = rygDashFieldCache(video, key);
			if (rygLooksLikeManifest(val)) return val;
		}
		@try {
	...
	NSDictionary *fc = rygFieldCacheDict(media);
	if (fc) {
		NSString *found = rygScanDictForManifest(fc, @"fieldCache", 0);
		if (found) return found;
		...
		// Repost wrappers — manifest lives on the nested media. Keys mirror
		// rygUnwrapRepostInnerMedia (Utils.m).
		for (NSString *key in @[@"repost_media", @"reshared_media", @"reposted_post", @"repost", @"repost_info", @"media_or_ad"]) {
			id inner = fc[key];
			if ([inner isKindOfClass:[NSDictionary class]]) {
				NSDictionary *d = (NSDictionary *)inner;
				id nested = d[@"media"] ?: d[@"reposted_post"] ?: d[@"repost_media"] ?: d[@"reshared_media"];
				if (nested && ![nested isKindOfClass:[NSNull class]]) inner = nested;
			}
			if (!inner || [inner isKindOfClass:[NSNull class]] || inner == media) continue;
			NSString *innerManifest = [self dashManifestForMedia:inner];
			if (innerManifest.length) return innerManifest;
		}
	}
...
+ (NSArray<RYGDashRepresentation *> *)parseManifest:(NSString *)xmlString {
	...
	NSMutableArray<RYGDashRepresentation *> *results = [NSMutableArray array];
...
+ (RYGDashRepresentation *)bestAudioFromRepresentations:(NSArray<RYGDashRepresentation *> *)reps {
	RYGDashRepresentation *best = nil;
	for (RYGDashRepresentation *r in reps) {
...
+ (NSArray<RYGDashRepresentation *> *)videoRepresentations:(NSArray<RYGDashRepresentation *> *)reps {
	...
	return [videos sortedArrayUsingComparator:^NSComparisonResult(RYGDashRepresentation *a, RYGDashRepresentation *b) {
...
+ (NSArray<RYGDashRepresentation *> *)audioRepresentations:(NSArray<RYGDashRepresentation *> *)reps {
	...
	for (RYGDashRepresentation *r in reps) {
		if ([r.contentType isEqualToString:@"audio"]) [audios addObject:r];
	}
	return [audios sortedArrayUsingComparator:^NSComparisonResult(RYGDashRepresentation *a, RYGDashRepresentation *b) {
		return [@(b.bandwidth) compare:@(a.bandwidth)];
	}];
}
...
+ (RYGDashRepresentation *)representationForQuality:(RYGVideoQuality)quality
	...
	switch (quality) {
		case RYGVideoQualityHighest: return sorted.firstObject;
		case RYGVideoQualityLowest: return sorted.lastObject;
		case RYGVideoQualityMedium: return sorted[sorted.count / 2];
		case RYGVideoQualityAsk: return sorted.firstObject; // caller handles the picker
	}
