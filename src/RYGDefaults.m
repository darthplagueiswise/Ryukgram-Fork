...
// Split the old per-type on/off (which drove notify and log together) into an
// independent mode per type, so either channel can be muted alone.
void RYGMigrateActivityModes(void) {
	...
	if (!touched) return; // fresh install — the registered defaults are already right
