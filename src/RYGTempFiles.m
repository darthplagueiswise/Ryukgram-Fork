...
static NSString *const kRYGTempPrefix = @"ryuk_tmp_";
static NSString *const kRYGLegacyTempPrefix = @"ryg_tmp_";
static const NSTimeInterval kRYGDefaultTTL = 300.0;

@implementation RYGTempFiles
...
+ (NSURL *)claimWithExt:(NSString *)ext {
	return [self claimWithExt:ext ttl:kRYGDefaultTTL tag:nil];
}
...
+ (NSURL *)claimWithExt:(NSString *)ext ttl:(NSTimeInterval)ttl tag:(NSString *)tag {
	...
	NSString *name = [NSString stringWithFormat:@"%@%@_%@.%@", kRYGTempPrefix, cleanTag, NSUUID.UUID.UUIDString, cleanExt];
...
+ (NSURL *)claimNamedFile:(NSString *)filename ttl:(NSTimeInterval)ttl tag:(NSString *)tag {
	NSString *cleanTag = tag.length ? [tag stringByReplacingOccurrencesOfString:@"/" withString:@"_"] : @"anon";
	NSString *dirName = [NSString stringWithFormat:@"%@%@_%@", kRYGTempPrefix, cleanTag, NSUUID.UUID.UUIDString];
...
+ (void)releaseURL:(NSURL *)url {
	...
	BOOL parentTracked = [parent.lastPathComponent hasPrefix:kRYGTempPrefix];
...
+ (void)sweepLeftovers {
	dispatch_async(dispatch_get_global_queue(QOS_CLASS_UTILITY, 0), ^{
		...
		for (NSString *name in names) {
			if (![name hasPrefix:kRYGTempPrefix] && ![name hasPrefix:kRYGLegacyTempPrefix]) continue;
