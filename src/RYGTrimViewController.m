...
@interface RYGTrimViewController ()
...
@implementation RYGTrimViewController
...
- (void)buildUI {
	...
	self.durationLabel.text = [NSString stringWithFormat:RYGLocalized(@"Total: %@"), [self timeText:self.totalDuration]];
	...
	[self.sendButton setTitle:self.sendButtonTitle ?: RYGLocalized(@"Send") forState:UIControlStateNormal];
...
- (void)sendTapped {
	...
	if (duration < kMinTrim) {
		[RYGUtils showErrorHUDWithDescription:RYGLocalized(@"Selection too short (min 0.5s)")];
		return;
	}
...
- (void)deletePreConvertedTempIfNeeded {
	...
	if ([name hasPrefix:@"ryuk_tmp_"] || [name hasPrefix:@"ryg_tmp_"]) [RYGTempFiles releaseURL:self.mediaURL];
}
@end