...
- (void)reloadIcon {
	...
	// SF first so user-picked SF symbols aren't hijacked by the FB friendly-map.
	// Fall through to FB only for explicit IG asset names (ig_icon_*, bcn_*).
	UIImage *img = [SCIIcon sfImageNamed:_symbolName
							  pointSize:_symbolPointSize
								 weight:UIImageSymbolWeightSemibold];
	if (!img) img = [SCIIcon fbImageNamed:_symbolName pointSize:_symbolPointSize];
	_iconView.image = img;
}
