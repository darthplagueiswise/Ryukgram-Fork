...
+ (NSString *)dashManifestForMedia:(id)media {
	...
	if (fc) {
		...
		// Repost wrappers — manifest lives on the nested media.
		for (NSString *key in @[@"repost_media", @"reshared_media", @"reposted_post", @"media_or_ad"]) {
			id inner = fc[key];
			if ([inner isKindOfClass:[NSDictionary class]]) {
				id nested = ((NSDictionary *)inner)[@"media"];
