...
@interface RYGFakeLocationSearchResultsVC : UITableViewController <MKLocalSearchCompleterDelegate>
...
@implementation RYGFakeLocationSearchResultsVC
...
@interface RYGFakeLocationPickerVC () <MKMapViewDelegate,UISearchResultsUpdating,CLLocationManagerDelegate>
@property (nonatomic,strong) MKMapView *mapView;
@property (nonatomic,strong) MKPointAnnotation *pin;
@property (nonatomic,strong) UISearchController *searchController;
@property (nonatomic,strong) RYGFakeLocationSearchResultsVC *resultsVC;
...
@implementation RYGFakeLocationPickerVC

- (void)viewDidLoad {
	[super viewDidLoad];

	self.view.backgroundColor = UIColor.systemBackgroundColor;
	self.title = self.titleText.length ? self.titleText : RYGLocalized(@"Pick location");
...
- (void)setupSearch {
	self.resultsVC = [RYGFakeLocationSearchResultsVC new];
	...
	self.searchController.searchBar.placeholder = RYGLocalized(@"Search address or place");
...
- (void)setupCard {
	self.cardView = RYGLiquidGlassView(YES, NO, nil);
	...
	[self.useButton setTitle:RYGLocalized(@"Use this location") forState:UIControlStateNormal];
...
- (void)onLocateTap {
	if (!CLLocationManager.locationServicesEnabled) {
		[self showAlert:RYGLocalized(@"Location Services off") message:RYGLocalized(@"Turn Location Services on in Settings → Privacy to use your current location.") settings:NO];
		return;
	}
	...
	if (status == kCLAuthorizationStatusDenied || status == kCLAuthorizationStatusRestricted) {
		[self showAlert:RYGLocalized(@"Location access denied") message:RYGLocalized(@"Enable Location Services for Instagram in Settings to use your current location.") settings:YES];
		return;
	}

	CLLocation *loc = self.mapView.userLocation.location ?: self.locationManager.location;
	if (loc) [self useCoordinate:loc.coordinate name:RYGLocalized(@"Current location") distance:800.0 resolve:YES];
...
- (void)showAlert:(NSString *)title message:(NSString *)message settings:(BOOL)settings {
	...
	if (settings) {
		[alert addAction:[UIAlertAction actionWithTitle:RYGLocalized(@"Open Settings") style:UIAlertActionStyleDefault handler:^(__unused UIAlertAction *action) {
...
- (void)updateCard {
	...
	self.titleLabel.text = self.resolvedName.length ? self.resolvedName : RYGLocalized(@"Dropped pin");
...
- (MKAnnotationView *)mapView:(MKMapView *)mapView viewForAnnotation:(id<MKAnnotation>)annotation {
	...
	static NSString *identifier = @"RYGFakeLocationPin";
