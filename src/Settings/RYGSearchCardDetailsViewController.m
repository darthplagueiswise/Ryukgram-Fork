...
static NSString *const kOrderKey = @"search_card_order";
...
static NSString *rygMetricPrefKey(NSString *m) {
	if ([m isEqualToString:@"views"])    return @"search_card_full_views";
	if ([m isEqualToString:@"likes"])    return @"search_card_show_likes";
	if ([m isEqualToString:@"comments"]) return @"search_card_show_comments";
	if ([m isEqualToString:@"shares"])   return @"search_card_show_shares";
	if ([m isEqualToString:@"reposts"])  return @"search_card_show_reposts";
	return @"search_card_show_date";
}
...
typedef NS_ENUM(NSInteger, RYGSearchCardSection) {
	RYGSearchCardSectionBehavior = 0,
	RYGSearchCardSectionMetrics,
	RYGSearchCardSectionApply,
};

typedef NS_ENUM(NSInteger, RYGSearchBehaviorRow) {
	RYGSearchBehaviorRowMaster = 0,
	RYGSearchBehaviorRowShortNumbers,
	RYGSearchBehaviorRowFetchMissing,
};

@interface RYGSearchCardDetailsViewController ()
@property (nonatomic, copy) NSArray<NSString *> *order;
@end

@implementation RYGSearchCardDetailsViewController
...
- (NSInteger)tableView:(UITableView *)tv numberOfRowsInSection:(NSInteger)s {
	switch (s) {
		case RYGSearchCardSectionBehavior: return 3;
		case RYGSearchCardSectionMetrics:  return self.order.count;
		default:                           return 2;
	}
}
...
- (UITableViewCell *)tableView:(UITableView *)tv cellForRowAtIndexPath:(NSIndexPath *)ip {
	if (ip.section == RYGSearchCardSectionBehavior) return [self behaviorCellForRow:ip.row];
	if (ip.section == RYGSearchCardSectionMetrics)  return [self metricCellForRow:ip.row];
...
- (UITableViewCell *)behaviorCellForRow:(NSInteger)row {
	...
	switch (row) {
		case RYGSearchBehaviorRowMaster:       title = RYGLocalized(@"Show card details"); key = @"search_card_enabled"; sf = @"rectangle.grid.1x2"; break;
		case RYGSearchBehaviorRowShortNumbers: title = RYGLocalized(@"Short numbers"); key = @"search_card_shortened_numbers"; sf = @"number"; break;
		default:                               title = RYGLocalized(@"Fetch missing counts"); key = @"search_card_fetch_missing"; sf = @"arrow.down.circle"; break;
	}
...
- (void)tableView:(UITableView *)tv didSelectRowAtIndexPath:(NSIndexPath *)ip {
	...
	if (ip.section != RYGSearchCardSectionApply) return;
...
- (void)confirmReset {
	UIAlertController *alert = [UIAlertController alertControllerWithTitle:RYGLocalized(@"Reset to defaults")
																   message:RYGLocalized(@"Restores the default stats, order, and options for search & explore card details.")
	...
	[alert addAction:[UIAlertAction actionWithTitle:RYGLocalized(@"Reset") style:UIAlertActionStyleDestructive handler:^(__unused UIAlertAction *a) {
		...
		if (!self) return;
		for (NSString *k in @[@"search_card_enabled", @"search_card_full_views", @"search_card_show_likes",
		                      @"search_card_show_comments", @"search_card_show_shares", @"search_card_show_reposts",
		                      @"search_card_show_date", @"search_card_shortened_numbers", @"search_card_fetch_missing"])
...
- (BOOL)isReorderableSection:(NSInteger)section { return section == RYGSearchCardSectionMetrics; }
