...
#import "../Utils.h"
#import "../Features/General/RYGTabBar.h"
#import <objc/message.h>

static NSArray<NSDictionary *> *rygTabCatalog(void) {
	return @[
		@{@"key": RYGTabKeyFeed,     @"title": @"Feed",     @"icon": @"ig_icon_home_pano_prism_outline_24",        @"sfFallback": @"house"},
		@{@"key": RYGTabKeyReels,    @"title": @"Reels",    @"icon": @"ig_icon_reels_pano_prism_outline_24",       @"sfFallback": @"play.rectangle"},
		@{@"key": RYGTabKeyMessages, @"title": @"Messages", @"icon": @"ig_icon_direct_prism_outline_24",           @"sfFallback": @"paperplane"},
		@{@"key": RYGTabKeyExplore,  @"title": @"Explore",  @"icon": @"ig_icon_search_pano_outline_24",            @"sfFallback": @"magnifyingglass"},
		@{@"key": RYGTabKeyProfile,  @"title": @"Profile",  @"icon": @"ig_icon_user_circle_pano_prism_outline_24", @"sfFallback": @"person.crop.circle"},
		@{@"key": RYGTabKeyCreate,   @"title": @"Create",   @"icon": @"ig_icon_add_pano_outline_24",               @"sfFallback": @"plus.app"},
	];
}
...
static UIVisualEffectView *rygMakeGlass(void) {
	...
	if (glassCls) {
		@try {
			id g = [[glassCls alloc] init];
			SEL sel = NSSelectorFromString(@"setInteractive:");
			if ([g respondsToSelector:sel]) {
				void (*setInteractive)(id, SEL, BOOL) = (void (*)(id, SEL, BOOL))objc_msgSend;
				setInteractive(g, sel, YES);
			}
			fx = g;
		} @catch (__unused id e) { fx = nil; }
	}
	if (!fx) fx = [UIBlurEffect effectWithStyle:UIBlurEffectStyleSystemThinMaterial];
	return [[UIVisualEffectView alloc] initWithEffect:fx];
}
...
- (void)loadOrder {
	...
	for (NSString *key in all) {
		BOOL hidden = pristine ? [key isEqualToString:RYGTabKeyCreate] : [RYGUtils getBoolPref:[RYGTabBar hidePrefForTabKey:key]];
...
- (void)commit {
	...
	for (NSString *key in self.barKeys)
		[RYGUtils setPref:@(NO) forKey:[RYGTabBar hidePrefForTabKey:key]];
	for (NSString *key in self.hiddenKeys)
		[RYGUtils setPref:@(YES) forKey:[RYGTabBar hidePrefForTabKey:key]];
}
...
- (void)resetTapped {
	...
	for (NSString *key in [RYGTabBar catalogTabKeys])
		[RYGUtils setPref:@(NO) forKey:[RYGTabBar hidePrefForTabKey:key]];
	[self loadOrder];
	[self.preview setKeys:self.barKeys animatingInKey:nil];
	[self rebuildHidden];
