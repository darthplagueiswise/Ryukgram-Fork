...
static UIImage *SCIRoundedAvatar(UIImage *src) {
	...
	return [[[UIGraphicsImageRenderer alloc] initWithSize:size format:fmt] imageWithActions:^(__unused UIGraphicsImageRendererContext *ctx) {
		[[UIBezierPath bezierPathWithOvalInRect:(CGRect){ .origin = CGPointZero, .size = size }] addClip];
		[src drawInRect:(CGRect){ .origin = CGPointZero, .size = size }];
	}];
}
...
static NSString *SCIAvatarURLForEntry(NSDictionary *e) {
	...
	return SCIText(u[@"profilePicURL"]);
}

static UIImage *SCIAvatarForEntry(NSDictionary *e) {
	BOOL group = [e[@"isGroup"] boolValue];
	NSString *urlString = SCIAvatarURLForEntry(e);
	...
	[[NSURLSession.sharedSession dataTaskWithURL:url completionHandler:^(NSData *data, __unused NSURLResponse *res, __unused NSError *err) {
		UIImage *img = data.length ? SCIRoundedAvatar([UIImage imageWithData:data]) : nil;
		if (img) [SCIAvatarCache() setObject:img forKey:urlString];
...
static NSDictionary *SCINormalizedEntry(NSDictionary *e) {
	...
	NSString *avatarURL = SCIAvatarURLForEntry(e);
