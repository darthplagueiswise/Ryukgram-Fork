...
static NSString *const kStoryAvatarLoadedNotif = @"SCIExcludedStoryUserAvatarLoaded";
...
static NSCache<NSString *, UIImage *> *SCIStoryAvatarCache(void) {
...
static NSMutableSet<NSString *> *SCIStoryAvatarLoading(void) {
...
static UIImage *SCIStoryAvatarPlaceholder(void) {
	return [UIImage systemImageNamed:@"person.circle.fill"];
}

static UIImage *SCIRoundedStoryAvatar(UIImage *src) {
	if (!src) return nil;
...
static NSString *SCIStoryAvatarURL(NSDictionary *e) {
	NSString *url = SCIText(e[@"avatarURL"]);
	if (!url.length) url = SCIText(e[@"profilePicURL"]);
	return url;
}

static UIImage *SCIStoryAvatarForEntry(NSDictionary *e) {
	NSString *urlString = SCIStoryAvatarURL(e);
	if (!urlString.length) return SCIStoryAvatarPlaceholder();

	UIImage *cached = [SCIStoryAvatarCache() objectForKey:urlString];
	if (cached) return cached;

	@synchronized (SCIStoryAvatarLoading()) {
		if ([SCIStoryAvatarLoading() containsObject:urlString]) return SCIStoryAvatarPlaceholder();
		[SCIStoryAvatarLoading() addObject:urlString];
	}

	NSURL *url = [NSURL URLWithString:urlString];
	if (!url) return SCIStoryAvatarPlaceholder();
	...
	NSURLSessionDataTask *task = [NSURLSession.sharedSession dataTaskWithURL:url completionHandler:^(NSData *data, __unused NSURLResponse *res, __unused NSError *err) {
		UIImage *img = data.length ? SCIRoundedStoryAvatar([UIImage imageWithData:data]) : nil;
		if (img) [SCIStoryAvatarCache() setObject:img forKey:urlString];

		@synchronized (SCIStoryAvatarLoading()) {
			[SCIStoryAvatarLoading() removeObject:urlString];
		}

		if (img) {
			dispatch_async(dispatch_get_main_queue(), ^{
				[NSNotificationCenter.defaultCenter postNotificationName:kStoryAvatarLoadedNotif object:nil];
			});
		}
	}];

	[task resume];
	return SCIStoryAvatarPlaceholder();
}
...
static NSDictionary *SCINormalizedStoryUser(NSDictionary *e) {
	...
	NSString *avatarURL = SCIStoryAvatarURL(e);
	...
	if (avatarURL.length) out[@"avatarURL"] = avatarURL;
...
- (instancetype)init {
	...
	cfg.iconProvider = ^UIImage *(NSDictionary *e) {
		return SCIStoryAvatarForEntry(e);
	};
...
- (void)viewDidLoad {
	...
	[NSNotificationCenter.defaultCenter addObserver:self selector:@selector(reload) name:kStoryAvatarLoadedNotif object:nil];
}
