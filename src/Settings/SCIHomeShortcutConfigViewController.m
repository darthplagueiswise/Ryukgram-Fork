...
static void sciApplyIcon(UIListContentConfiguration *config, NSString *symbol, UIColor *tint) {
	...
	UIImage *image = [SCIIcon imageNamed:symbol pointSize:22 weight:UIImageSymbolWeightRegular];
	if (!image) return;
...
static UIImageView *sciIconView(NSString *symbol) {
	UIImageView *view = [[UIImageView alloc] initWithImage:(symbol.length ? [SCIIcon imageNamed:symbol pointSize:22 weight:UIImageSymbolWeightRegular] : nil)];
...
- (void)configureWithSymbol:(NSString *)symbol selected:(BOOL)selected {
	...
	self.iconView.image = isAuto
		? [UIImage systemImageNamed:@"wand.and.stars" withConfiguration:cfg]
		: [SCIIcon imageNamed:symbol pointSize:24 weight:UIImageSymbolWeightSemibold];
...
@interface SCIHomeShortcutConfigViewController ()
