...
#import "../Features/ExpFlags/SCINumericMCCatalog.h"
#import "../Features/ExpFlags/SCIMobileConfigBrokerStore.h"
#import "../Utils.h"

@interface SCINumericCatalogCell : UITableViewCell
@property (nonatomic, strong) UILabel *title;
@property (nonatomic, strong) UILabel *detail;
@property (nonatomic, strong) UISegmentedControl *state;
@property (nonatomic, copy) void (^changed)(NSInteger idx);
@end
...
- (void)stateChanged {
    if (self.changed) self.changed(self.state.selectedSegmentIndex);
}
@end

@interface SCINumericCatalogViewController () <UISearchResultsUpdating>
@property (nonatomic, copy) NSArray<NSString *> *groups;
@property (nonatomic, copy) NSDictionary<NSString *, NSArray<SCINumericMCEntry *> *> *rowsByGroup;
...
- (void)importFromPasteboard {
    ...
    NSString *msg = ok
        ? [NSString stringWithFormat:@"Imported %lu entries.\n%@", (unsigned long)[SCINumericMCCatalog entryCount], [SCINumericMCCatalog catalogPath]]
        : (err.localizedDescription ?: @"Pasteboard does not contain a valid sci_numeric_mc_overrides.json");

    UIAlertController *a = [UIAlertController alertControllerWithTitle:ok ? @"Catalog imported" : @"Import failed"
                                                                message:msg
...
- (NSString *)tableView:(UITableView *)tableView titleForHeaderInSection:(NSInteger)section {
    return section == 0 ? @"Catalog" : self.groups[section - 1];
}

- (NSString *)tableView:(UITableView *)tableView titleForFooterInSection:(NSInteger)section {
    if (section == 0) return @"Paste the generated sci_numeric_mc_overrides.json and tap Import. Rows write mcbr:ig:<hex> through SCIMobileConfigBrokerStore only when changed manually.";
    return nil;
}
...
- (NSInteger)segmentForEntry:(SCINumericMCEntry *)e {
    NSNumber *forced = [SCIMobileConfigBrokerStore overrideValueForKey:e.overrideKey];
    if (!forced) return 0;
    return forced.boolValue ? 2 : 1;
}

- (void)setSegment:(NSInteger)idx entry:(SCINumericMCEntry *)e {
    if (!e.overrideKey.length) return;

    if (idx == 0) {
        [SCIMobileConfigBrokerStore setOverrideValue:nil forKey:e.overrideKey];
    } else {
        [SCIMobileConfigBrokerStore setBrokerHookEnabled:YES brokerID:@"ig"];
        [SCIMobileConfigBrokerStore setOverrideValue:@(idx == 2) forKey:e.overrideKey];
    }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    c.changed = ^(NSInteger idx) {
        [ws setSegment:idx entry:e];
    };
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    NSString *msg = [NSString stringWithFormat:@"%@\n\n%@\n\nsource=%@\nclassification=%@\nvmaddr=%@\nfileoff=%@\nkey=%@\n\n%@",
                     ...
                     e.fileoff ?: @"",
                     e.overrideKey ?: @"",
                     [e.evidence componentsJoinedByString:@"\n"] ?: @""];

    UIAlertController *a = [UIAlertController alertControllerWithTitle:e.displayTitle
                                                                message:msg
                                                         preferredStyle:UIAlertControllerStyleActionSheet];

    [a addAction:[UIAlertAction actionWithTitle:@"Copy ID" style:UIAlertActionStyleDefault handler:^(__unused UIAlertAction *x) {
        UIPasteboard.generalPasteboard.string = e.specifierHex;
    }]];
    [a addAction:[UIAlertAction actionWithTitle:@"Copy key" style:UIAlertActionStyleDefault handler:^(__unused UIAlertAction *x) {
        UIPasteboard.generalPasteboard.string = e.overrideKey;
    }]];
    [a addAction:[UIAlertAction actionWithTitle:@"Force ON" style:UIAlertActionStyleDefault handler:^(__unused UIAlertAction *x) {
        [self setSegment:2 entry:e];
    }]];
    [a addAction:[UIAlertAction actionWithTitle:@"Force OFF" style:UIAlertActionStyleDefault handler:^(__unused UIAlertAction *x) {
        [self setSegment:1 entry:e];
    }]];
    [a addAction:[UIAlertAction actionWithTitle:@"System" style:UIAlertActionStyleDefault handler:^(__unused UIAlertAction *x) {
        [self setSegment:0 entry:e];
    }]];
