...
+ (RYGSetting *)deviceIDNavCell {
	...
	RYGSetting *deviceIDCell = [RYGSetting navigationCellWithTitle:RYGLocalized(@"Device ID")
		...
		@"footer": RYGLocalized(@"Forgets every saved login, cookie and the stored device identity, then relaunches so Instagram starts as a brand-new device. You'll sign in again afterwards."),
		@"rows": @[
