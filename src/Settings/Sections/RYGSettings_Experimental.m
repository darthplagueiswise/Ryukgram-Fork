...
+ (void)openMobileConfigBrowser {
	...
	if ([[NSUserDefaults standardUserDefaults] boolForKey:@"ryg_metaconfig_warning_seen"]) {
		[RYGPopupChrome presentVC:[RYGFastMobileConfigBrowserViewController new] from:nil];
		return;
	}
	UIAlertController *a = [UIAlertController
...
+ (NSArray *)experimentalNavSections {
	return @[
		...
		@{
			...
			@"footer": RYGLocalized(@"Read Instagram's current in-memory MobileConfig table in real time. Names come only from Instagram's mappings on this device; no table of parameters is bundled or cached. Only explicit overrides and notes persist. If Instagram crashes on launch three times in a row, your changes are cleared for you."),
			@"rows": @[
