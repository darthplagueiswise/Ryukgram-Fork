...
+ (RYGSetting *)messagesOnlyNavCell {
	RYGSetting *enable = [RYGSetting switchCellWithTitle:RYGLocalized(@"Automatic schedule")
		...
		RYGMessagesOnlySchedule *sched = [RYGMessagesOnlySchedule shared];
		[sched refreshFromPrefs];
		if (on && !RYGMessagesOnlyHooksActive() && ![sched isWithinWindowNow])
			[RYGUtils showRestartConfirmation];
	}];
...
+ (RYGSetting *)interfaceNavCell {
	return [RYGSetting navigationCellWithTitle:RYGLocalized(@"Interface")
										...
										@{
											@"footer": RYGLocalized(@"Brings back the previous feed header wordmark."),
											@"rows": @[
												[RYGSetting switchCellWithTitle:RYGLocalized(@"Old Instagram logo")
																	   subtitle:@""
																	defaultsKey:@"old_feed_logo" requiresRestart:YES],
											]
										},
										@{
