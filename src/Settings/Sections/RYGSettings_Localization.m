...
+ (void)refreshFakeCountSubtitlesInVC:(UIViewController *)vc {
	...
	NSDictionary *map = @{
		...
		RYGLocalized(@"Username"):		  @"fake_username_value",
		RYGLocalized(@"Display name"):	  @"fake_full_name_value",
	};
...
+ (void)promptFakeCountForKey:(NSString *)key title:(NSString *)title {
	[self promptFakeValueForKey:key title:title placeholder:RYGLocalized(@"e.g. 1000000") numeric:YES];
}

+ (void)promptFakeTextForKey:(NSString *)key title:(NSString *)title {
	[self promptFakeValueForKey:key title:title placeholder:title numeric:NO];
}
...
+ (void)promptFakeValueForKey:(NSString *)key title:(NSString *)title placeholder:(NSString *)placeholder numeric:(BOOL)numeric {
	...
	[alert addTextFieldWithConfigurationHandler:^(UITextField *tf) {
		...
		tf.text = current ?: @"";
		tf.keyboardType = numeric ? UIKeyboardTypeNumberPad : UIKeyboardTypeDefault;
