	...
	return [RYGSetting navigationCellWithTitle:RYGLocalized(@"Media saving")
										...
										navSections:@[@{
											...
											@"rows": @[
												...
												[RYGSetting switchCellWithTitle:RYGLocalized(@"Add prefix to file names") subtitle:RYGLocalized(@"Start every saved file name with ryuk_ so downloads are easy to spot") defaultsKey:@"dl_name_prefix"],
											]
										},
										@{
										...
										@{
											@"header": RYGLocalized(@"Duplicates"),
											@"footer": RYGLocalized(@"Keeps a list of what you already grabbed and asks first when the same media comes up again. Only IDs are stored, never files, and carousels ask once for the whole post."),
											@"rows": @[
												[RYGSetting switchCellWithTitle:RYGLocalized(@"Check for duplicates") subtitle:RYGLocalized(@"Ask before downloading media you already saved") defaultsKey:@"dl_duplicate_check"],
												[RYGSetting buttonCellWithTitle:RYGLocalized(@"Clear duplicate list") subtitle:RYGLocalized(@"Forget every download recorded so far") icon:[RYGSymbol symbolWithName:@"trash"] action:^{ [RYGDownloadManagerViewController presentClearDuplicatesConfirmation]; }],
											]
										},
										@{
