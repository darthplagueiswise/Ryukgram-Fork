...
+ (SCISetting *)backupNavCell {
	return [SCISetting navigationCellWithTitle:SCILocalized(@"Backup & Restore")
										...
										navSections:@[@{
											@"header": @"",
											@"footer": SCILocalized(@"Export or import RyukGram data — settings, per-account filters, hidden & locked chats, Profile Analyzer, gallery, chat backgrounds, deleted messages and the read receipts log. Pick any combination on each page. Settings stay a plain JSON file; bundles with media export as a compressed .ryukbak."),
											@"rows": @[
