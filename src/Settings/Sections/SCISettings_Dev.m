...
static void SCIDevPromptFOASandboxHostname(void) {
	...
	[a addAction:[UIAlertAction actionWithTitle:SCILocalized(@"Set")
		style:UIAlertActionStyleDefault handler:^(__unused UIAlertAction *action) {
			NSString *hostname = a.textFields.firstObject.text ?: @"";
			NSString *result =
				[SCIGraphQLDogfoodDiagnostics setFOASandboxHostname:hostname];
			// The prompt is still dismissing inside its action handler.
...
+ (SCISetting *)devNavCell {
	...
	return [SCISetting navigationCellWithTitle:SCILocalized(@"Dev")
		...
		@{
			...
			@"footer": SCILocalized(@"Validated in Instagram(3): FBCCIGExperimentManager/FBCustomExperimentManager use BOOL isFeatureEnabled:(uint64_t). ExperimentConfig and helper sweeps enumerate only supported BOOL methods with zero or one ObjC/integer argument."),
			@"rows": @[
				[SCISetting switchCellWithTitle:SCILocalized(@"Force unified experiment managers")
					...
					value:^BOOL {
						return [SCIUtils getBoolPref:@"sci_force_unified_experiment_managers"];
					}
					action:^(BOOL on) {
				...
				SCIExperimentSwitch(
					...
					^NSUInteger(NSNumber *v) {
						return [SCISymbolBrowserEngine setExperimentConfigsForced:v];
					}
				),
				SCIExperimentSwitch(
					...
					^NSUInteger(NSNumber *v) {
						return [SCISymbolBrowserEngine setExperimentHelpersForced:v];
					}
				),
					...
					action:^{
						dispatch_async(dispatch_get_global_queue(QOS_CLASS_USER_INITIATED, 0), ^{
							...
							if ([SCIUtils getBoolPref:@"sci_force_experiment_configs"])
								n += [SCISymbolBrowserEngine setExperimentConfigsForced:@YES];
							if ([SCIUtils getBoolPref:@"sci_force_experiment_helpers"])
								n += [SCISymbolBrowserEngine setExperimentHelpersForced:@YES];
							dispatch_async(dispatch_get_main_queue(), ^{
								[SCIUtils showToastForDuration:2.5
									title:[NSString stringWithFormat:
										@"Experiment rescan: %lu methods",
										(unsigned long)n]
									subtitle:nil];
							});
						});
		...
		@{
			...
			@"rows": @[
				...
				[SCISetting switchCellWithTitle:SCILocalized(@"Force E2E bypass")
					...
					value:^BOOL {
						return [SCIUtils getBoolPref:@"sci_force_e2e_bypass"];
					}
					action:^(BOOL on) {
				...
				[SCISetting buttonCellWithTitle:SCILocalized(@"Current FOA Sandbox Override")
					...
					action:^{
						SCIDevShow(SCILocalized(@"FOA Sandbox"),
				...
				[SCISetting buttonCellWithTitle:SCILocalized(@"Set FOA Sandbox Hostname")
					...
					action:^{
						SCIDevPromptFOASandboxHostname();
					}],
				...
				[SCISetting buttonCellWithTitle:SCILocalized(@"GraphQL Debug capabilities")
					...
					action:^{
						SCIDevShow(SCILocalized(@"GraphQL Debug"),
				...
				[SCISetting buttonCellWithTitle:SCILocalized(@"Warm up GraphQL Debug provider")
					...
					action:^{
						[SCIGraphQLDogfoodDiagnostics
							warmupGraphQLDebugWithCompletion:^(NSString *result) {
				...
				[SCISetting buttonCellWithTitle:SCILocalized(@"Retrieve GraphQL ACS status")
					...
					action:^{
						[SCIGraphQLDogfoodDiagnostics
							retrieveGraphQLDebugACSTokenStatusWithCompletion:^(NSString *result) {
								SCIDevShow(SCILocalized(@"GraphQL ACS"), result);
							}];
					}],
				[SCISetting buttonCellWithTitle:SCILocalized(@"Retrieve ACS + OHAI status")
					subtitle:SCILocalized(@"Runs the validated native callback; contents remain redacted")
					icon:[SCISymbol symbolWithName:@"shield.lefthalf.filled"]
					action:^{
						[SCIGraphQLDogfoodDiagnostics
							retrieveGraphQLDebugACSAndOHAIStatusWithCompletion:^(NSString *result) {
				...
				[SCISetting switchCellWithTitle:SCILocalized(@"React Native Dev Menu")
					...
					value:^BOOL {
						return [SCIUtils getBoolPref:@"sci_force_rct_dev_menu"];
					}
					action:^(BOOL on) {
				...
				[SCISetting buttonCellWithTitle:SCILocalized(@"Apply internal/debug now")
					...
					icon:[SCISymbol symbolWithIGName:@"bcn_wrench_outline_24"
						fallback:@"wrench.and.screwdriver"]
					action:^{
						SCIDevShow(SCILocalized(@"Internal/debug"),
		...
		@{
			...
			@"footer": SCILocalized(@"The removed IGMobileConfigBooleanValueForInternalUse reader is not exposed. Remaining rows map to imports/exports confirmed in Instagram(3) and FBSharedFramework(3); complex readers call the original first, then force YES."),
			@"rows": @[
				[SCISetting switchCellWithTitle:SCILocalized(@"Instagram internal apps installed")
					subtitle:@"IGAppIsInstagramInternalAppsInstalledAndNotHiddenAfteriOS18"
					defaultsKey:@"sci_force_ig_internal_apps_installed_after_ios18"
					requiresRestart:YES],
				[SCISetting switchCellWithTitle:SCILocalized(@"Minos dogfood MEK")
					subtitle:@"MEBIsMinosDogfoodMekEncryptionVersionEnabled"
					defaultsKey:@"sci_force_minos_dogfood_mek_encryption"
					requiresRestart:YES],
				[SCISetting switchCellWithTitle:SCILocalized(@"Force all EasyGating BOOL readers")
					subtitle:@"" defaultsKey:@"sci_force_easy_gating_all" requiresRestart:YES],
				[SCISetting switchCellWithTitle:@"EasyGating Internal"
					subtitle:@"EasyGatingGetBoolean_Internal_DoNotUseOrMock"
					defaultsKey:@"sci_force_easy_gating_internal" requiresRestart:YES],
				[SCISetting switchCellWithTitle:@"EasyGating Platform"
					subtitle:@"EasyGatingPlatformGetBoolean"
					defaultsKey:@"sci_force_easy_gating_platform" requiresRestart:YES],
				[SCISetting switchCellWithTitle:@"EasyGating AuthDataContext"
					subtitle:@"" defaultsKey:@"sci_force_easy_gating_auth" requiresRestart:YES],
				[SCISetting switchCellWithTitle:@"MCQ EasyGating"
					subtitle:@"" defaultsKey:@"sci_force_easy_gating_mcq" requiresRestart:YES],
				[SCISetting switchCellWithTitle:SCILocalized(@"Force Sessioned/MCI BOOL readers")
					subtitle:SCILocalized(@"MSGC + MCIExperiment + MCIExtension")
					defaultsKey:@"sci_force_sessioned_mc_all" requiresRestart:YES],
				[SCISetting switchCellWithTitle:@"MSGC Sessioned BOOL"
					subtitle:@"MSGCSessionedMobileConfigGetBoolean"
					defaultsKey:@"sci_force_msgc_sessioned_boolean" requiresRestart:YES],
				[SCISetting switchCellWithTitle:@"MCI Experiment BOOL"
					subtitle:@"MCIExperimentCacheGetMobileConfigBoolean"
					defaultsKey:@"sci_force_mci_experiment_boolean" requiresRestart:YES],
				[SCISetting switchCellWithTitle:@"MCI Extension BOOL"
					subtitle:@"MCIExtensionExperimentCacheGetMobileConfigBoolean"
					defaultsKey:@"sci_force_mci_extension_boolean" requiresRestart:YES],
				[SCISetting switchCellWithTitle:@"META Extensions experiments"
					subtitle:@"GetBoolean + WithoutExposure"
					defaultsKey:@"sci_force_meta_ext_experiment" requiresRestart:YES],
			]
		},
		@{
			@"header": SCILocalized(@"Open native internal menus"),
			@"footer": SCILocalized(@"The Debug Menu opener dismisses RyukGram, restores IGWindow and invokes the validated entryPoint 0 thunk path on the next main-loop turn."),
			@"rows": @[
				[SCISetting buttonCellWithTitle:SCILocalized(@"Open Instagram Debug Menu")
					subtitle:SCILocalized(@"Calls -[IGWindow showDebugMenuWithEntryPoint:0] after dismissing this sheet")
					...
					action:^{
						[SCIInternalMenusLauncher
							openInstagramDebugMenuWithCompletion:^(NSString *result) {
								if (!SCIDevResultWasHandled(result)) {
									SCIDevShow(@"Instagram Debug Menu", result);
								}
							}];
					}],
				[SCISetting buttonCellWithTitle:SCILocalized(@"Open Dogfooding/Notes settings")
					...
					action:^{
						...
						if (!SCIDevResultWasHandled(r)) SCIDevShow(@"Internal menu", r);
					}],
				...
				[SCISetting buttonCellWithTitle:SCILocalized(@"Open internal URL")
					...
					action:^{
						NSString *r = [SCIInternalMenusLauncher
							openInternalURLString:@"instagram://internal_settings"];
						if (!SCIDevResultWasHandled(r)) SCIDevShow(@"Internal URL", r);
					}],
			]
		},
		@{
			...
			@"rows": @[
				[SCISetting navigationCellWithTitle:SCILocalized(@"Unified Runtime Browser")
					...
					viewController:[[SCISymbolsBrowserViewController alloc]
						initWithMode:SCICSymbolsBrowserModeObjCMethods]],
		...
		@{
			...
			@"rows": @[
				...
				({
					SCISetting *s = [SCISetting switchCellWithTitle:
						SCILocalized(@"Status Bar Old School")
						subtitle:@"" defaultsKey:@"sci_statusbar_oldschool"
						requiresRestart:NO];
					s.icon = [SCISymbol symbolWithName:@"statusbar_oldschool"
						color:UIColor.labelColor];
					s;
				}),
				({
					SCISetting *s = [SCISetting switchCellWithTitle:
						SCILocalized(@"Stories Tray")
						subtitle:@"" defaultsKey:@"sci_story_tray"
						requiresRestart:NO];
					s.icon = [SCISymbol symbolWithName:@"story_tray"
						color:UIColor.labelColor];
					s;
				}),
				({
					SCISetting *s = [SCISetting menuCellWithTitle:
						SCILocalized(@"Custom Feed Header")
						subtitle:@""
						menu:[self menus][@"ig_wordmark_variant"]];
					s.icon = [SCISymbol symbolWithName:@"custom_feed_header"
						color:UIColor.labelColor];
					s;
				}),
			]
