	...
	return [SCISetting navigationCellWithTitle:SCILocalized(@"Messages")
										...
										navSections:@[@{
											...
											@"rows": @[
												[SCISetting navigationCellWithTitle:SCILocalized(@"Read receipts")
																		...
																		navSections:@[@{
																			...
																			@"rows": @[
																				...
																				[SCISetting switchCellWithTitle:SCILocalized(@"Auto mark seen on interact") subtitle:SCILocalized(@"Marks messages as seen when you reply, react or send media") defaultsKey:@"seen_auto_on_interact"],
												...
												[SCISetting navigationCellWithTitle:SCILocalized(@"Read receipts log")
																		   subtitle:SCILocalized(@"Notifies and records when someone reads a message you sent")
																			   icon:nil
																		navSections:@[@{
																			@"header": @"",
																			@"footer": SCILocalized(@"When enabled, a notification fires when someone reads your message. Turn off \"Save to log\" for toast-only mode. Long-press a person in the log to stop logging them."),
																			@"rows": @[
																				[SCISetting switchCellWithTitle:SCILocalized(@"Enable read receipts") subtitle:SCILocalized(@"Detects when others read your messages and notifies you") defaultsKey:@"read_receipts_enabled"],
																				[SCISetting switchCellWithTitle:SCILocalized(@"Save to log") subtitle:SCILocalized(@"Record reads on this device. Off = notifications only, nothing stored") defaultsKey:@"read_receipts_save_log"],
																				[SCISetting switchCellWithTitle:SCILocalized(@"Log group chats") subtitle:SCILocalized(@"Also track reads in group chats. Off by default — groups can be noisy") defaultsKey:@"read_receipts_log_groups"],
																				[SCISetting buttonCellWithTitle:SCILocalized(@"Open log")
																								   subtitle:SCILocalized(@"Browse who read your messages, grouped by person")
																									   icon:[SCISymbol symbolWithName:@"eye.fill"]
																									 action:^(void) {
																				[SCIReadReceiptLogViewController presentFromViewController:nil];
																			}],
																			]
																		}]
												],
