...
static SCISetting *MCBRow(void) {
    return [SCISetting navigationCellWithTitle:@"MC Brokers v2"
                                      subtitle:@"C broker router for FBSharedFramework MobileConfig/EasyGating: mcbr:<id>:<hex> overrides, mcob:<id>:<hex> observed."
    ...
    for (NSUInteger s = 0; s < out.count; s++) {
        ...
        for (NSUInteger r = 0; r < rows.count; r++) {
            ...
            [nav addObject:@{
                ...
                @"footer": @"Separated from DexKit ObjC getter scanner. Installs real-body C hooks only for saved per-value overrides or explicit pass-through observer toggles.",
                @"rows": @[MCBRow()]
            }];
            row.navSections = nav;
            rows[r] = row;
