...
@interface RYGDragLayoutChip : UIView
@property (nonatomic, strong) RYGDragLayoutItem *item;
...
@implementation RYGDragLayoutChip

- (BOOL)isRect { return self.item.cornerRadius > 0.0 && self.item.width > 0.0; }
- (BOOL)isPill { return !self.isRect && self.item.width > self.item.diameter; }

- (instancetype)initWithItem:(RYGDragLayoutItem *)item {
...
- (CGSize)chipSize {
	...
	if (self.isRect) return CGSizeMake(self.item.width * s, h);
...
- (void)layoutChrome {
	...
	if (self.isRect) {
		...
		self.bubble.layer.cornerRadius = self.item.cornerRadius * s;
		self.bubble.layer.borderWidth = 1.5;
		self.bubble.layer.borderColor = [UIColor colorWithWhite:1.0 alpha:0.9].CGColor;
		CGFloat iconD = MIN(self.bounds.size.width, self.bounds.size.height) * 0.42;
		self.iconView.frame = CGRectMake((self.bounds.size.width - iconD) / 2.0, (self.bounds.size.height - iconD) / 2.0, iconD, iconD);
		return;
	}
...
@interface RYGDragLayoutEditorViewController ()
@property (nonatomic, strong) NSMutableArray<RYGDragLayoutItem *> *mutableItems;
@property (nonatomic, strong) UIView *canvas;
@property (nonatomic, strong) NSMutableArray<RYGDragLayoutChip *> *chips;
...
@property (nonatomic, weak) RYGDragLayoutChip *draggingChip;
@property (nonatomic, assign) CGFloat canvasScale;
@end

@implementation RYGDragLayoutEditorViewController
...
- (instancetype)initWithItems:(NSArray<RYGDragLayoutItem *> *)items {
	...
	_snapMask = RYGDragLayoutSnapAll;
	_gridDivisions = 6;
...
- (NSArray<RYGDragLayoutItem *> *)items { return self.mutableItems.copy; }
...
- (void)viewDidLoad {
	...
	if (self.slots.count && self.showsSlots) {
		...
		self.slotHighlight.fillColor = [[RYGUtils RYGColor_Primary] colorWithAlphaComponent:0.25].CGColor;
		self.slotHighlight.strokeColor = [RYGUtils RYGColor_Primary].CGColor;
	...
	if (self.accessoryView) [self.view addSubview:self.accessoryView];
	...
	if (self.onReset) {
		self.navigationItem.rightBarButtonItem = [[UIBarButtonItem alloc] initWithTitle:RYGLocalized(@"Reset")
...
- (UIView *)makeGuide {
	UIView *g = [UIView new];
	g.backgroundColor = [RYGUtils RYGColor_Primary] ?: UIColor.systemYellowColor;
	g.alpha = 0.0;
...
- (void)rebuildChips {
	for (RYGDragLayoutChip *c in self.chips) [c removeFromSuperview];
	[self.chips removeAllObjects];

	for (RYGDragLayoutItem *item in self.mutableItems) {
		RYGDragLayoutChip *chip = [[RYGDragLayoutChip alloc] initWithItem:item];
...
- (void)viewDidLayoutSubviews {
	...
	if (self.accessoryView) {
		accH = [self.accessoryView sizeThatFits:CGSizeMake(availW, CGFLOAT_MAX)].height + 16.0;
	}
	CGFloat availH = safe.size.height - pad * 2.0 - hintH - accH;
	...
	if (self.accessoryView) {
		self.accessoryView.frame = CGRectMake(safe.origin.x + pad, CGRectGetMaxY(self.canvas.frame) + 16.0, availW, accH - 16.0);
	}

	// Scale against the real target surface so preview sizing matches; else the screen's short side.
	CGFloat refW = self.referenceWidth > 0 ? self.referenceWidth
	...
	for (RYGDragLayoutChip *chip in self.chips) chip.displayScale = self.canvasScale;
	...
	if (self.slotDots) {
		...
		for (RYGDragLayoutItem *item in self.mutableItems) {
			...
			CGPoint hp = [self pointForNormalized:item.homePosition size:rygVisualSize(item)];
...
// Normalized positions map into this, not the whole canvas.
- (CGRect)mapRect {
	CGRect b = self.canvas.bounds;
	UIEdgeInsets o = self.canvasOutsetFractions;
	return CGRectMake(b.origin.x + o.left * b.size.width,
					  b.origin.y + o.top * b.size.height,
					  b.size.width * (1.0 - o.left - o.right),
					  b.size.height * (1.0 - o.top - o.bottom));
}

- (CGPoint)canvasPointForNormalized:(CGPoint)norm {
	CGRect b = [self mapRect];
...
static CGSize rygVisualSize(RYGDragLayoutItem *item) {
	CGFloat h = item.diameter;
	if (item.cornerRadius > 0.0 && item.width > 0.0) return CGSizeMake(item.width, h);
	return CGSizeMake(item.width > h ? item.width : h, h);
}

- (void)positionChipsFromModel {
	for (RYGDragLayoutChip *chip in self.chips) {
...
- (CGPoint)pointForNormalized:(CGPoint)norm size:(CGSize)size {
	return [self clampCenter:[self canvasPointForNormalized:norm] size:size];
}
...
- (CGPoint)normalizedForPoint:(CGPoint)center {
	...
	return CGPointMake((center.x - b.origin.x) / b.size.width, (center.y - b.origin.y) / b.size.height);
}

#pragma mark - Drag

- (void)handlePan:(UIPanGestureRecognizer *)pan {
	RYGDragLayoutChip *chip = (RYGDragLayoutChip *)pan.view;
...
- (void)handleSlotPan:(RYGDragLayoutChip *)chip translation:(CGPoint)t ending:(BOOL)ending {
	if (hypot(t.x, t.y) < 6.0) {
		chip.center = chip.dragStart;
		self.slotHighlight.opacity = 0.0;
	...
	RYGDragLayoutChip *occupant = [self chipAtSlotNorm:targetNorm excluding:chip];
...
- (void)resetTransform:(RYGDragLayoutChip *)chip {
...
- (RYGDragLayoutChip *)chipAtSlotNorm:(CGPoint)norm excluding:(RYGDragLayoutChip *)excluding {
	for (RYGDragLayoutChip *c in self.chips) {
...
- (CGPoint)homePointForChip:(RYGDragLayoutChip *)chip {
...
- (void)endDragOfChip:(RYGDragLayoutChip *)chip storeNormalized:(CGPoint)norm {
	self.draggingChip = nil;
...
- (CGPoint)resolveCollisions:(CGPoint)c chip:(RYGDragLayoutChip *)chip diameter:(CGFloat)d {
	if (self.minimumSpacing <= 0.0) return c;

	for (NSInteger pass = 0; pass < 3; pass++) {
		BOOL moved = NO;

		for (RYGDragLayoutChip *other in self.chips) {
...
- (void)snapPoint:(CGPoint)p diameter:(CGFloat)d outX:(CGFloat *)outX outY:(CGFloat *)outY result:(CGPoint *)result {
	...
	if (self.snapMask & RYGDragLayoutSnapEdges) {
	...
	if (self.snapMask & RYGDragLayoutSnapCenter) {
		[xs addObject:@(CGRectGetMidX(b))];
		[ys addObject:@(CGRectGetMidY(b))];
	}
	if ((self.snapMask & RYGDragLayoutSnapGrid) && self.gridDivisions > 1) {
