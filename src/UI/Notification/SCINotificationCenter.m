...
- (void)notifyAction:(NSString *)actionID
    ...
    if (!tapCopy) tapCopy = [self sciResolveDefaultTapForAction:actionID];

    if (surface == SCINotifSurfaceIGNative) {
        NSTimeInterval igEffective = MAX(0.6, duration * [self sciDurationMultiplier]);
        [self sciOnMain:^{ [SCIUtils showIGNativeToastForDuration:igEffective title:title subtitle:subtitle onTap:tapCopy]; }];
        return;
    }

    [self sciOnMain:^{
        [self sciPresentToastForAction:actionID title:title subtitle:subtitle icon:iconSymbol tone:tone duration:duration onTap:tapCopy];
    }];
}
...
- (SCINotificationHandle *)sciBeginProgressForAction:(NSString *)actionID
    ...
    void (^ (^provider)(void))(void) = nil;
    @synchronized (_tapProviders) { provider = _tapProviders[actionID]; }
    void (^tap)(void) = provider ? provider() : nil;
...
- (SCINotifSlot *)sciCreateSlotForAction:(NSString *)actionID title:(NSString *)title subtitle:(NSString *)subtitle icon:(NSString *)icon tone:(SCINotificationTone)tone isProgress:(BOOL)isProgress onTap:(void (^)(void))onTap {
    ...
    pill.onTap = ^(__unused SCINotificationPillView *p) {
        ...
        if (!s) return;
        if (tapCopy) tapCopy();
        if (!s.isProgress) [weakSelf sciDismissSlot:s animated:YES];
    };
...
- (void)sciDrainQueueIfPossible {
    while (_visible.count < [self sciMaxVisible] && _queue.count > 0) {
        ...
        void (^queuedTap)(void) = next[@"onTap"];
...
- (void)setDefaultTapProvider:(void (^ (^)(void))(void))provider forAction:(NSString *)actionID {
    ...
    @synchronized (_tapProviders) {
        if (provider) _tapProviders[actionID] = [provider copy];
        else          [_tapProviders removeObjectForKey:actionID];
    }
}

- (void (^)(void))sciResolveDefaultTapForAction:(NSString *)actionID {
    if (!actionID.length) return nil;
    id provider;
    @synchronized (_tapProviders) { provider = _tapProviders[actionID]; }
    if (!provider) return nil;
    void (^(^p)(void))(void) = provider;
    void (^resolved)(void) = p();
...
- (void)sciAppBackgrounded {
    ...
    [self sciOnMain:^{
        for (SCINotifSlot *s in self->_visible.copy) {
            if (s.isProgress && !s.terminal) continue;
            [self sciDismissSlot:s animated:NO];
        }
        [self->_queue removeAllObjects];
    }];
