...
static BOOL SCINotifStyleIsIsland(SCINotificationStyle style) {
	return style == SCINotificationStyleIsland;
}
...
- (void)sciBuild {
	...
	[self sciBuildBackground];
	[self sciBuildIcon];
	[self sciBuildCloseButton];
	[self sciBuildTextAndProgress];
	[self sciBuildSizeConstraints];
	[self sciBuildProgressRing];
	[self sciBuildGestures];
}

- (void)sciPinView:(UIView *)view toView:(UIView *)target {
...
- (void)sciBuildBackground {
	...
	[self sciPinView:self.blurView toView:self];
...
- (CGFloat)sciCurrentCornerRadius {
	return SCINotifStyleIsIsland(_style) ? MAX(CGRectGetHeight(self.bounds) / 2.0, 1.0) : kPillCorner;
}
...
- (void)layoutSubviews {
	...
	[self sciApplyCornerRadius:[self sciCurrentCornerRadius]];
}

	// Island uses a true capsule (height/2); others use kPillCorner.
- (void)sciApplyCornerRadius:(CGFloat)radius {
...
- (void)sciApplyVisualStyleAnimated:(BOOL)animated {
	...
	void (^apply)(void) = ^{
		...
		CGFloat radius = [self sciCurrentCornerRadius];
		...
		BOOL minimal = self->_style == SCINotificationStyleMinimal;
...
- (void)setIconSymbolName:(NSString *)iconSymbolName {
	...
	self.iconView.image = SCINotifIcon(iconSymbolName ?: SCINotifFallbackIconForTone(_tone), 16.0, UIFontWeightSemibold);
}
...
- (void)sciApplyProgressVisibility {
	...
	self.progressRowHeightConstraint.constant = determinate ? [self sciSubtitleRowHeight] : 0.0;
	self.progressHeightConstraint.constant = determinate ? [self sciProgressBarHeight] : 0.0;
...
- (void)refreshSizeAnimated:(BOOL)animated {
	...
	CGFloat textW = MAX(SCITextWidth(self.titleText, self.titleLabel.font), SCITextWidth(self.subtitleText, self.subtitleLabel.font));
	...
	CGFloat targetW = SCIClamp(ceil(textW) + fixedW + 6.0, minW, maxW);
...
- (void)sciPan:(UIPanGestureRecognizer *)pan {
	...
	switch (pan.state) {
		...
		case UIGestureRecognizerStateChanged: {
			...
			self.alpha = 1.0 - (SCIClamp(dismissTrans / 60.0, 0.0, 1.0) * 0.5);
			break;
		}
