...
static char kRYGPickerSelfRetainKey;
static UISheetPresentationControllerDetentIdentifier const kRYGPickerFitDetentID = @"ryg_color_picker_fit";

@interface RYGColorPickerSheet () <UIColorPickerViewControllerDelegate, UIAdaptivePresentationControllerDelegate>
@property (nonatomic, assign) RYGColorPickerSheetMode mode;
@property (nonatomic, strong) UIColor *startColor;
@property (nonatomic, strong, nullable) UIColor *endColor;
@property (nonatomic, copy) RYGColorPickerSheetApplyHandler applyHandler;
...
@implementation RYGColorPickerSheet
...
+ (instancetype)sheetWithMode:(RYGColorPickerSheetMode)mode
	...
	RYGColorPickerSheet *vc = [RYGColorPickerSheet new];
	vc.mode = mode;
...
- (void)configureSheetForViewController:(UIViewController *)vc {
	...
	// and working on the iOS 15 SDK (RYGCustomSheetDetent returns nil there).
	UISheetPresentationControllerDetent *fit =
		RYGCustomSheetDetent(kRYGPickerFitDetentID, ^CGFloat(CGFloat max) {
	...
	if (fit) {
		...
		sheet.selectedDetentIdentifier = kRYGPickerFitDetentID;
	} else {
		sheet.detents = @[
...
- (UIColorPickerViewController *)makePickerSeededWith:(UIColor *)seed {
	...
	picker.title = RYGLocalized(@"Colors");
...
- (UIColorPickerViewController *)makeStandalonePickerSeededWith:(UIColor *)seed {
	...
	objc_setAssociatedObject(picker, &kRYGPickerSelfRetainKey, self, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
...
- (void)tearDownStandalonePicker:(UIColorPickerViewController *)picker {
	if (!picker) return;
	picker.delegate = nil;
	objc_setAssociatedObject(picker, &kRYGPickerSelfRetainKey, nil, OBJC_ASSOCIATION_ASSIGN);
...
- (void)presentFromViewController:(UIViewController *)presenter {
	...
	if (_mode == RYGColorPickerSheetModeSolid) {
...
- (void)viewDidLoad {
	...
	if (_mode != RYGColorPickerSheetModeGradient) return;
...
- (void)buildSwatchRow {
	...
	_swatchRow = [[UIStackView alloc] initWithArrangedSubviews:@[
		[self makeSwatchColumnWithTitle:RYGLocalized(@"Start") swatch:_startSwatch],
		[self makeSwatchColumnWithTitle:RYGLocalized(@"End") swatch:_endSwatch]
	]];
...
- (void)applyPickedColor:(UIColor *)color throttle:(BOOL)throttle {
	...
	if (_mode == RYGColorPickerSheetModeGradient) {
...
- (void)fireApplyForced:(BOOL)forced {
	...
	if (_applyHandler) {
		_applyHandler(_mode, _startColor, (_mode == RYGColorPickerSheetModeGradient) ? _endColor : nil);
	}
}
...
- (void)viewDidDisappear:(BOOL)animated {
	...
	if (_mode == RYGColorPickerSheetModeGradient && self.isBeingDismissed) {
		[self fireApplyForced:YES];
	}
}
