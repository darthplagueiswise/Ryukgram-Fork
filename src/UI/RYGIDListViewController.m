#import "RYGIDListViewController.h"
#import "RYGUserPickerViewController.h"
#import "Notification/RYGNotificationCenter.h"
#import "Notification/RYGNotificationActions.h"
#import "../Localization/RYGLocalization.h"

@implementation RYGIDListConfig
- (instancetype)init {
	if ((self = [super init])) {
		_allowsEdit = YES;
		_allowsAdd = YES;
...
@interface RYGIDListViewController () <UITableViewDataSource, UITableViewDelegate, UISearchBarDelegate>
@property (nonatomic, strong, readwrite) RYGIDListConfig *config;
...
@property (nonatomic, strong) RYGNotificationHandle *addHandle;
@property (nonatomic, strong) dispatch_source_t addSpinnerPoll;
@end

@implementation RYGIDListViewController
...
- (void)viewDidLoad {
	...
	_searchBar.placeholder = self.config.searchPlaceholder ?: RYGLocalized(@"Search");
	...
	if (self.config.allowsEdit) {
		_editBtn = [[UIBarButtonItem alloc] initWithTitle:RYGLocalized(@"Select") style:UIBarButtonItemStylePlain target:self action:@selector(toggleEdit)];
...
- (void)reload {
	...
	self.countLabel.text = [NSString stringWithFormat:@"%lu %@", (unsigned long)count, count == 1 ? RYGLocalized(@"item") : RYGLocalized(@"items")];
	self.emptyLabel.text = count ? @"" : (q.length ? RYGLocalized(@"No results found.") : RYGLocalized(@"Nothing here yet."));
...
- (void)toggleEdit {
	...
	self.editBtn.title = editing ? RYGLocalized(@"Done") : RYGLocalized(@"Select");
...
- (void)refreshBatchToolbar {
	...
	UIBarButtonItem *remove = [[UIBarButtonItem alloc] initWithTitle:RYGLocalized(@"Remove") style:UIBarButtonItemStylePlain target:self action:@selector(tapBatchRemove)];
	remove.tintColor = UIColor.systemRedColor;
	remove.enabled = selected.count > 0;

	UIBarButtonItem *count = [[UIBarButtonItem alloc] initWithTitle:[NSString stringWithFormat:@"%lu %@", (unsigned long)selected.count, RYGLocalized(@"selected")] style:UIBarButtonItemStylePlain target:nil action:nil];
	count.enabled = NO;
...
- (void)tapSort {
	...
	UIAlertController *sheet = [UIAlertController alertControllerWithTitle:RYGLocalized(@"Sort by") message:nil preferredStyle:UIAlertControllerStyleActionSheet];
...
// Resolving a picked user into a thread is a network round-trip; show the loading
// pill until the consumer presents its confirm/error UI or reloads.
- (void)startAddLoading {
	if (self.addHandle) return;
	self.addHandle = RYGNotifyLoading(RYG_NOTIF_GENERIC, RYGLocalized(@"Adding…"), nil);

	__weak typeof(self) weak = self;
	__block int ticks = 0;
	dispatch_source_t t = dispatch_source_create(DISPATCH_SOURCE_TYPE_TIMER, 0, 0, dispatch_get_main_queue());
	dispatch_source_set_timer(t, dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.15 * NSEC_PER_SEC)), (uint64_t)(0.15 * NSEC_PER_SEC), (uint64_t)(0.05 * NSEC_PER_SEC));
	dispatch_source_set_event_handler(t, ^{ if (!weak || weak.presentedViewController || ++ticks > 80) [weak stopAddLoading]; });
	self.addSpinnerPoll = t;
	dispatch_resume(t);
}

- (void)stopAddLoading {
	if (self.addSpinnerPoll) { dispatch_source_cancel(self.addSpinnerPoll); self.addSpinnerPoll = nil; }
	[self.addHandle dismiss];
	self.addHandle = nil;
}
...
- (void)tapAdd {
	...
	if (self.config.onAddResolvedUser) {
		__weak typeof(self) weak = self;
		[RYGUserPickerViewController presentFromViewController:self title:self.config.addAlertTitle idLabel:self.config.addIDLabel onPick:^(NSDictionary *user) {
			if (weak.config.onAddResolvedUser) weak.config.onAddResolvedUser(user, ^{ [weak reload]; });
		}];
		return;
	}

	if (self.config.useUserPickerForAdd && self.config.onAddRequest) {
		__weak typeof(self) weak = self;
		[RYGUserPickerViewController presentFromViewController:self title:self.config.addAlertTitle idLabel:self.config.addIDLabel onPick:^(NSDictionary *user) {
			NSString *un = user[@"username"], *pk = user[@"pk"];
			NSString *text = un.length ? un : pk;
			if (!text.length || !weak.config.onAddRequest) return;
			[weak startAddLoading];
			weak.config.onAddRequest(text, weak, ^{ [weak stopAddLoading]; [weak reload]; });
		}];
		return;
	}
	...
	UIAlertController *alert = [UIAlertController alertControllerWithTitle:self.config.addAlertTitle ?: RYGLocalized(@"Add") message:self.config.addAlertMessage preferredStyle:UIAlertControllerStyleAlert];
	[alert addTextFieldWithConfigurationHandler:^(UITextField *tf) {
		tf.placeholder = self.config.addAlertPlaceholder ?: RYGLocalized(@"Enter value");
	...
	__weak typeof(self) weak = self;
	[alert addAction:[UIAlertAction actionWithTitle:RYGLocalized(@"Continue") style:UIAlertActionStyleDefault handler:^(__unused UIAlertAction *_) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)ip {
	static NSString *reuse = @"rygIDListCell";
...
- (UISwipeActionsConfiguration *)tableView:(__unused UITableView *)tableView trailingSwipeActionsConfigurationForRowAtIndexPath:(NSIndexPath *)ip {
	...
	id item = self.filtered[ip.row];
	__weak typeof(self) weak = self;
	UIContextualAction *remove = [UIContextualAction contextualActionWithStyle:UIContextualActionStyleDestructive title:RYGLocalized(@"Remove") handler:^(__unused UIContextualAction *a, __unused UIView *v, void (^done)(BOOL)) {
