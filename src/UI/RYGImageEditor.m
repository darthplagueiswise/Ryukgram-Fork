#import "RYGImageEditor.h"
#import <Vision/Vision.h>

// iOS 17+ subject-lift API; forward-declared so we build against older SDKs.
#if __IPHONE_OS_VERSION_MAX_ALLOWED < 170000
@interface VNInstanceMaskObservation : VNObservation
@property (nonatomic, readonly) NSIndexSet *allInstances;
- (CVPixelBufferRef)generateMaskedImageOfInstances:(NSIndexSet *)instances
                                fromRequestHandler:(VNImageRequestHandler *)handler
                          croppedToInstancesExtent:(BOOL)cropped
...
static UIImage *rygNormalizeUp(UIImage *img) {
...
static void rygRemoveBackground(UIImage *image, void (^done)(UIImage *result)) {
    if (@available(iOS 17.0, *)) {
        UIImage *img = rygNormalizeUp(image);
...
@interface RYGImageEditorController : UIViewController <UIScrollViewDelegate>
@property (nonatomic, strong) UIImage *sourceImage;
@property (nonatomic) CGFloat fixedAspect;
@property (nonatomic, copy) void (^onDone)(UIImage *edited);
@end

@implementation RYGImageEditorController {
...
- (void)viewDidLoad {
    ...
    _aspect = [[UISegmentedControl alloc] initWithItems:@[ RYGLocalized(@"Free"), RYGLocalized(@"Original"), @"1:1", @"4:5", @"9:16" ]];
    ...
    [_removeBGButton setTitle:RYGLocalized(@"Remove background") forState:UIControlStateNormal];
    ...
    [_resetButton setTitle:RYGLocalized(@"Reset") forState:UIControlStateNormal];
    ...
    [_doneButton setTitle:RYGLocalized(@"Done") forState:UIControlStateNormal];
...
- (void)removeBGTapped {
    ...
    rygRemoveBackground(current, ^(UIImage *result) {
        __strong typeof(ws) ss = ws;
        if (!ss) return;
        [ss->_spinner stopAnimating];
        ss.view.userInteractionEnabled = YES;
        if (!result) {
            UIAlertController *a = [UIAlertController alertControllerWithTitle:RYGLocalized(@"No subject found") message:nil preferredStyle:UIAlertControllerStyleAlert];
...
+ (void)presentForImage:(UIImage *)image from:(UIViewController *)presenter fixedAspect:(CGFloat)fixedAspect onDone:(void (^)(UIImage *))onDone {
    if (image && !image.CGImage) {
        UIImage *n = rygNormalizeUp(image);
        if (n.CGImage) image = n;
    }
    if (!image || !image.CGImage || !presenter) return;
    RYGImageEditorController *vc = [RYGImageEditorController new];
