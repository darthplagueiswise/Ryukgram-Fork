#import "RYGStickerSearchPicker.h"
#import "../Networking/RYGInstagramAPI.h"
#import <ImageIO/ImageIO.h>
#import <objc/message.h>
#import <objc/runtime.h>

static UIImage *rygDecodeStickerData(NSData *data) {
...
static void rygCollectStickerURLs(id node, NSString *key, NSMutableArray *out) {
    if ([node isKindOfClass:NSDictionary.class]) {
        for (NSString *k in (NSDictionary *)node) rygCollectStickerURLs(node[k], k, out);
    } else if ([node isKindOfClass:NSArray.class]) {
        for (id v in (NSArray *)node) rygCollectStickerURLs(v, key, out);
...
@interface RYGStickerCell : UICollectionViewCell
@property (nonatomic, strong) UIImageView *imageView;
@property (nonatomic, copy) NSString *loadingURL;
@end
@implementation RYGStickerCell
...
@interface RYGStickerSearchController : UIViewController <UICollectionViewDataSource, UICollectionViewDelegateFlowLayout, UITextViewDelegate>
@property (nonatomic, copy) void (^onPick)(UIImage *);
@end

@implementation RYGStickerSearchController {
...
- (void)viewDidLoad {
    ...
    [_collection registerClass:RYGStickerCell.class forCellWithReuseIdentifier:@"c"];
    [self.view addSubview:_collection];

    _empty = [UILabel new];
    _empty.text = RYGLocalized(@"No stickers yet");
    ...
    _iosHint.text = RYGLocalized(@"Tap any sticker on your keyboard\nto add it to your drawing");
...
- (void)fetch {
    ...
    [RYGInstagramAPI sendRequestWithMethod:@"GET" path:@"creatives/sticker_tray/" body:nil completion:^(NSDictionary *resp, NSError *err) {
        ...
        rygCollectStickerURLs(resp[@"sticker_tray"] ?: resp, nil, found);
...
@implementation RYGStickerSearchPicker
+ (void)presentFrom:(UIViewController *)presenter onPick:(void (^)(UIImage *))onPick {
    if (!presenter) return;
    RYGStickerSearchController *vc = [RYGStickerSearchController new];
    vc.onPick = onPick;
