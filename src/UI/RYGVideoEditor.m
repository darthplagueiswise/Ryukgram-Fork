#import "RYGVideoEditor.h"
#import <AVFoundation/AVFoundation.h>
#import "../Features/Instants/RYGInstantsPath.h"
#import "../RYGTempFiles.h"
#import "../Localization/RYGLocalization.h"

static const NSTimeInterval kRYGMinTrim = 1.0;
static const CGFloat kRYGTrimBarH = 56.0;
static const NSTimeInterval kRYGStripWindowSecs = 25.0;

@interface RYGVideoEditorController : UIViewController <UIScrollViewDelegate>
...
@implementation RYGVideoEditorController {
	AVURLAsset *_asset;
	AVAssetTrack *_track;
...
- (void)configureStrip {
	...
	CGFloat factor = MAX(1.0, _duration / kRYGStripWindowSecs);
...
- (void)handlePan:(UIPanGestureRecognizer *)pan {
	...
	CGFloat Wc = _stripScroll.contentSize.width;
	CGFloat minDist = (_duration > 0.001) ? (CGFloat)(kRYGMinTrim * Wc / _duration) : 20.0;
...
- (void)updateInfoLabel {
	...
	NSString *total = [NSString stringWithFormat:@"    %@", [NSString stringWithFormat:RYGLocalized(@"Total: %@"), [self timeText:_duration]]];
...
- (void)viewDidLayoutSubviews {
	...
	UIBezierPath *hole = squircle ? RYGInstantsSquirclePathInRect(crop) : [UIBezierPath bezierPathWithRoundedRect:crop cornerRadius:14];
...
- (void)useTapped {
	...
	NSURL *out = [RYGTempFiles claimWithExt:@"mp4" ttl:900 tag:@"video-edit"];
	...
	[session exportAsynchronouslyWithCompletionHandler:^{
		dispatch_async(dispatch_get_main_queue(), ^{
			...
			if (!ok) { NSLog(@"[RyukGram][VideoEditor] export failed %ld %@", (long)session.status, session.error); [RYGTempFiles releaseURL:out]; }
...
+ (void)presentForVideoURL:(NSURL *)url from:(UIViewController *)presenter maxDuration:(NSTimeInterval)maxDuration aspectW:(CGFloat)aspectW aspectH:(CGFloat)aspectH onDone:(void (^)(NSURL *))onDone {
	if (!url) return;
	RYGVideoEditorController *vc = [RYGVideoEditorController new];
	vc.assetURL = url;
