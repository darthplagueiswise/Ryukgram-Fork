...
- (NSString *)activeSymbol {
    return self.hasSource ? [SCIActionIcon effectiveSymbolNameForSource:self.source]
                          : [SCIActionIcon symbolName];
}
...
- (void)viewDidLoad {
    ...
    if (self.hasSource) {
        self.navigationItem.rightBarButtonItem =
            [[UIBarButtonItem alloc] initWithTitle:SCILocalized(@"Use default")
...
- (void)viewDidAppear:(BOOL)animated {
    ...
    NSUInteger idx = [self.icons indexOfObject:[self activeSymbol]];
    if (idx != NSNotFound) {
...
- (void)collectionView:(UICollectionView *)cv didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if ([picked isEqualToString:[self activeSymbol]]) {
        [cv deselectItemAtIndexPath:indexPath animated:YES];
        return;
    }
    if (self.hasSource) {
        [SCIActionIcon setOverride:picked forSource:self.source];
    } else {
...
- (void)resetToDefault {
    [SCIActionIcon setOverride:@"" forSource:self.source];
